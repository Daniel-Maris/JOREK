subroutine tg02a(ix,n,u,s,d,x,v)
  ...
  integer :: j
  logical :: found
  real*8  :: h00, h10, h01, h11, theta, theta2, theta3, t
  found = .false.
  eps = 1.d-16
  if (x .lt. u(1)) then
    ...
    if (ix .gt. 0.d0) j = min(max(ix,n),1)
    if (ix .lt. 0.d0) j = (x-u(1))/(u(n)-u(1))*(n-1)+1
    do while  (.not. found)
      if (x .gt. u(j+1)) then
        j = j + 1
  ...
  v(2) = 6*(theta2 - theta)*s(j) + (3*theta2 - 4*theta + 1)*h*d(j) + 6*(-theta2 + theta)*s(j+1) + (3*theta2 - 2*theta)*h*d(j+1)
  v(2) = v(2)/h
  v(3) = (12*theta - 6)*s(j) + (6*theta - 4)*h*d(j) + (-12*theta + 6)*s(j+1) + (6*theta - 2)*h*d(j+1)
  v(3) = v(3)/(h**2)
  v(4) = 6*(2*s(j) - 2*s(j+1) + h*d(j) + h*d(j+1))/(h**3)
