...
do iv = 1, 4, 3  ! loop over axis nodes only, 1 and 4 are axis nodes
   ...
   nodes(iv)%values(i_harm,dof2,ivar_in)  = nodes(iv)%x(1,dof2,1) * node_list%node(vg)%values(i_harm,dof2,ivar_in) &
                                          + nodes(iv)%x(1,dof2,2) * node_list%node(vg)%values(i_harm,dof4,ivar_in)
...
subroutine old2new_basis_on_the_axis_poisson(nodes, ELM, RHS, ivar_in, ivar_out, i_harm)
use data_structure
use phys_module
implicit none
type(type_node),       intent(inout) :: nodes(n_vertex_max)
...
end subroutine old2new_basis_on_the_axis_poisson
