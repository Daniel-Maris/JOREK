...
if (periodic) then
  nel   = (n_plane) * (nsub-1)*(nsub-1)*element_list%n_elements
else
  nel   = (n_plane-1) * (nsub-1)*(nsub-1)*element_list%n_elements
endif
    ...
    if ( (periodic) .and. (m .eq. n_plane)) then
      ...
      do j=1,nsub-1
        do k=1,nsub-1
          ...
          ien(5,ielm) = ien(1,ielm) - n_points * (n_plane-1)
          ien(6,ielm) = ien(2,ielm) - n_points * (n_plane-1)
          ien(7,ielm) = ien(3,ielm) - n_points * (n_plane-1)
          ien(8,ielm) = ien(4,ielm) - n_points * (n_plane-1)
