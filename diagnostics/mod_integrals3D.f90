...
real*8, dimension(n_plane,n_gauss,n_gauss) :: x_g, x_s, x_t, x_p, x_ss, x_tt, x_st, y_g, y_s, y_t, y_p, y_ss, y_tt, y_st
...
real*8  :: xjac, xjac_R, xjac_Z, BigR, wst, P_int, C_intern, zj0, w0, ps0, r0, T0, T0e, Vol, Volume, Area, Bgeo, area1
real*8  :: psi_as_coord
...
real*8  :: vperp_disp_tot, vpar_disp_tot, vperp_disp, vpar_disp, viscoperp_dissip_tot, viscopar_dissip_tot, source_tot, heating_tot
real*8  :: H_int, H_ext, S_int, S_ext, heating_in, heating_out, source_in, source_out
real*8  :: psi_xpoint(2),R_xpoint(2),Z_xpoint(2),s_xpoint(2),t_xpoint(2)
real*8  :: dTdx, dTdy, drhodx, drhody, dPdx, dPdy, dpsidx, dpsidy, dpsidp, dudx, dudy, dudp, drhondx, drhondy
real*8  :: source_volume, source_pellet, eta_T, eta_T_ohm, visco_T
...
!$omp          H_int, H_ext, S_int, S_ext, psi_xpoint, F0, VP_tot, eta, visco, visco_T_dependent, T_0, T_min, &
...
!$omp          mag_wk_tot, vperp_disp_tot, vpar_disp_tot, area1, mag_src_tot, momentum_x, momentum_y, &
!$omp          eta_ohmic,  &
#if (defined WITH_Neutrals) || (defined WITH_Impurities)
...
!$omp           thm_wk, mag_wk, eta_T, visco_T, vperp_disp, vpar_disp, p0_p, T0_corr, r0_corr, u0_p, &
    ...
    do ms=1, n_gauss
      do mt=1, n_gauss
        ...
        if (visco_T_dependent) then
          ...
          if (xpoint .and. (T0 .lt. T_min)) visco_T = visco*(max(T0,T_min)/T_0)**(-1.5d0)
        else
          visco_T = visco
        end if
  ...
  if (use_imp_adas) then  ! use open adas by default
    ...
    if ( trim(imp_type) == 'Ar') then ! Hard-coded fitting exists for argon
      ...
      stop
        ...
        vperp_disp_tot = vperp_disp_tot + vperp_disp * BigR * xjac * wst * delta_phi 
        ...
        if (using_spi) then
          ...
          do spi_i = 1, n_spi
            ...
            call neutral_source(pellets(spi_i)%spi_abl,pellets(spi_i)%spi_R,pellets(spi_i)%spi_Z,pellets(spi_i)%spi_phi,&
                                  ...
                                  ns_tor_norm, A_Dmv,K_Dmv,V_Dmv,P_Dmv,t_ns,L_tube,x_g(mp,ms,mt),y_g(mp,ms,mt),     &
          ...
          call neutral_source(ns_amplitude,ns_R,ns_Z,ns_phi,ns_radius,ns_sig,ns_deltaphi,ns_tor_norm,        &
                                A_Dmv,K_Dmv,V_Dmv,P_Dmv,t_ns,L_tube,x_g(mp,ms,mt),y_g(mp,ms,mt),phi,source_neutral,t_now, &
        ...
        if (using_spi) then
          ...
          call inj_source(ns_amplitude,ns_R,ns_Z,ns_phi,ns_radius,ns_sig,ns_deltaphi,ns_tor_norm,        &
                                A_Dmv,K_Dmv,V_Dmv,P_Dmv,t_ns,L_tube,x_g(mp,ms,mt),y_g(mp,ms,mt),phi,source_tmp,t_now, &
                                JET_MGI,ASDEX_MGI,central_density,central_mass)
...
call MPI_AllReduce(vperp_disp_tot, viscoperp_dissip_tot,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
...
viscoperp_dissip_tot = vperp_disp_tot
...
viscoperp_dissip_tot = n_period * viscoperp_dissip_tot * fact_flux
...
if (my_id .eq. 0) then 
  ...
  loop_expr: do iexpr = 1, expr_list%n_expr
    ...
    select case ( trim(expr_list%expr(iexpr)%name) )
      ...
      case ( 'Viscperp_diss' )
        res(iexpr+1) = viscoperp_dissip_tot
  ...
  if (index_now > 0 ) then
    ...
    viscoperp_dissip_tot_t(index_now) = viscoperp_dissip_tot
