...
module hdf5_io_module
  ...
  !---------------------------------------- 
  ! HDF5 saving for a 1D array of real*4. Parallelization
  ! based on the MPI gather of all data by the master task
  ! the data are written by the master task in HDF5 file.
  !----------------------------------------
  ! inputs:
  ...
  !   compress_level:     (integer) level of data compression to be used
  !----------------------------------------
  subroutine HDF5_array1D_saving_r4_gatherv(file_id,array1D,dim1_all_tasks,dim1_tot,&
    displs,dsetname,mpi_rank,n_cpu,mpi_comm_loc,start,compress_level)
    use mpi
    implicit none
    integer(HID_T)            , intent(in) :: file_id   ! file identifier
  ...
  end subroutine HDF5_array1D_saving_r4_gatherv
  ...
  !---------------------------------------- 
  ! HDF5 saving for a 1D array. Parallelization
  ! based on the MPI gather of all data by the master task
  ! the data are written by the master task in HDF5 file.
  !----------------------------------------
  ! inputs:
  ...
  !   compress_level:     (integer) level of data compression to be used
  !----------------------------------------
  subroutine HDF5_array1D_saving_gatherv(file_id,array1D,dim1_all_tasks,dim1_tot,&
    displs,dsetname,mpi_rank,n_cpu,mpi_comm_loc,start,compress_level)
    use mpi
    implicit none
    integer(HID_T)            , intent(in) :: file_id   ! file identifier
  ...
  end subroutine HDF5_array1D_saving_gatherv
  ...
  !---------------------------------------- 
  ! HDF5 saving for a 2D array. Parallelization
  ! based on the MPI gather of all data by the master task
  ! the data are written by the master task in HDF5 file.
  !----------------------------------------
  ! inputs:
  ...
  !   compress_level:     (integer) level of data compression to be used
  !----------------------------------------
  subroutine HDF5_array2D_saving_gatherv(file_id,array2D,dim1_all_tasks,dim1_tot,&
    dim2_tot,displs,dsetname,mpi_rank,n_cpu,mpi_comm_loc,start,compress_level)
    use mpi
    implicit none
    integer(HID_T)            , intent(in) :: file_id   ! file identifier
  ...
  end subroutine HDF5_array2D_saving_gatherv
  ...
  subroutine HDF5_array3D_saving_gatherv(file_id,array3D,dim1_all_tasks,dim1_tot,&
    ...
    do jj=1,dim3_tot
      do ii=1,dim2_tot
        ...
        array3D_tot(:,ii,jj),dim1_all_tasks,displs,MPI_REAL8,&
        master_task,mpi_comm_loc,ierr)
      enddo
    enddo
    ! Write the global array to HDF5 file
