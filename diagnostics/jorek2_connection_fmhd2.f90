program jorek2_poincare
  ...
  do i = local_elm_start, local_elm_end
    ...
    do i_dir = -1,1,2
      ...
      call find_RZ(node_list,element_list,R_start,Z_start,R_out,Z_out,i_elm,s_ini,t_ini,ifail)
      if ( ifail .ne. 0 ) cycle
...
!                .or.( (xcase .eq. DOUBLE_NULL ) .and. (Z_turn(1,1) .gt. ES%Z_xpoint(1)) .and. (Z_turn(1,1) .lt. ES%Z_xpoint(2)) ) ) ) then
          ...
          if (R_turn(i_turn,i_dir) .gt. 0.d0) then
            ...
            if (ikeep .le. n_points_max) then
              if (Rtheta_plot) then
                ...
                call interp(node_list,element_list,ielm_tmp,1,1,s_tmp,t_tmp,psi,P0_s,P0_t,P0_st,P0_ss,P0_tt)
                RZkeep(1,ikeep)          = (psi-ES%psi_axis)/(ES%psi_bnd-ES%psi_axis)!small_r
  ...
  do i=1,i_strike
    ...
    if (PS0_strike(i) .gt. ES%psi_bnd) then
      R_strike(i) = 0.d0
      Z_strike(i) = 0.d0
    endif
  enddo
