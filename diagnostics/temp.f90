subroutine temp(node_list,element_list,A_tem,A_den,A_jen,A_jec)
!---------------------------------------------------------------
...
real*8     :: A_jen(n_tor), A_jec(n_tor)
...
! eccd current
   ...
   if(jorek_model == 305) then
     ...
     jec = p0  ! when not applying eccd, this last array is global pressure
        ...
        A_tem(in) = A_tem(in) + t0* wst
        A_den(in) = A_den(in) + r0* wst
        A_jen(in) = A_jen(in) + zj0*wst
        A_jec(in) = A_jec(in) + jec*wst
