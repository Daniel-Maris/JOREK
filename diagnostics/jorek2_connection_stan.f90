...
program jorek2_poincare
  ...
  real*8  :: psi_xpoint(2),R_xpoint(2),Z_xpoint(2),s_xpoint(2),t_xpoint(2), value_out, psi_bnd, psi_bnd2
  integer :: i_elm_xpoint, elm_start, elm_end, elm_delta, local_elm_start, local_elm_end
  ...
  !!!!  	C_all(i_line) = min(C_all(i_line),total_length)
  	    C_all(i_line)   = C_all(i_line)+total_length
  	  ...
  	  if (R_turn(i_turn,1) .gt. 0.d0) then
  	    ...
  	    if ( (PSI_turn(1,1) .lt. psi_bnd2) &
  		 .and. (    ( (xcase .eq. 1) .and. (Z_turn(1,1) .gt. Z_xpoint(1)) ) &
  			...
  			.or.( (xcase .eq. 3) .and. (Z_turn(1,1) .gt. Z_xpoint(1)) .and. (Z_turn(1,1) .lt. Z_xpoint(2)) ) ) &
  		 .and. (Z_turn(i_turn,1) .lt. 10.d0)	     ) then
  
  	      if (n_turn_max(1) .lt. n_turns) then
  		ikeep = ikeep + 1
  		RZkeep(1,ikeep) 	   = R_turn(i_turn,1)
  		RZkeep(2,ikeep) 	   = Z_turn(i_turn,1)
  !!!!  	    scalars(ikeep,1:n_scalars) = (/ min(zl1,zl2),T_turn(1,1) /)
  		scalars(ikeep,1:n_scalars) = (/ C_all(i_line),T_turn(1,1) /)
  	      else
  !		 ikeep = ikeep + 1
  !		 RZkeep(1,ikeep)	    = R_turn(i_turn,1)
  !		 RZkeep(2,ikeep)	    = Z_turn(i_turn,1)
  !		 scalars(ikeep,1:n_scalars) = (/ maxval(partial),T_turn(1,1) /)
  	      endif 
  	...
  	do i_turn=1,n_turn_max(2)+1	     ! keep only field lines starting inside the plasma
  	  ...
  	  if (R_turn(i_turn,2) .gt. 0.d0) then
  	    ...
  	    if ( (PSI_turn(1,2) .lt. psi_bnd2) &
  		 .and. (    ( (xcase .eq. 1) .and. (Z_turn(1,2) .gt. Z_xpoint(1)) ) &
  			.or.( (xcase .eq. 2) .and. (Z_turn(1,2) .lt. Z_xpoint(2)) ) &
  			.or.( (xcase .eq. 3) .and. (Z_turn(1,2) .gt. Z_xpoint(1)) .and. (Z_turn(1,2) .lt. Z_xpoint(2)) ) ) &
  		 .and. (Z_turn(i_turn,1) .lt. 10.d0)	     ) then
  ...
  !!!!  	    scalars(ikeep,1:n_scalars) = (/ min(zl1,zl2),T_turn(1,2) /)
  		scalars(ikeep,1:n_scalars) = (/ C_all(i_line),T_turn(1,2) /)
  	      else
  !		 ikeep = ikeep + 1
  !		 RZkeep(1,ikeep)	    = R_turn(i_turn,2)
  !		 RZkeep(2,ikeep)	    = Z_turn(i_turn,2)
  !		 scalars(ikeep,1:n_scalars) = (/ maxval(partial),T_turn(1,2) /)
  	      endif
  ...
  n_scalars = 2 	    ! number of scalars to write to the VTK output file
  
  allocate(scalar_names(n_scalars))
  
  scalar_names  = (/ 'length	 ','T_start	'/)
  ...
  !  if (PS0_strike(i) .gt. psi_xpoint) then		 ! to exclude points started outside the plasma
  !    R_strike(i) = 0.d0
  !    Z_strike(i) = 0.d0
  !  endif
  enddo
