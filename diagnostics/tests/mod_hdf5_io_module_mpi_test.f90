...
subroutine test_HDF5_array3D_saving_r8()
  ...
  if(rank_loc.eq.master_rank) call HDF5_open_or_create(trim(filename),file_id,ierr=ifail_loc) 
  call HDF5_array3D_saving_native_or_gatherv(file_id,result_array,sum(elements_all),&
  n_elements(2),n_elements(3),datasetname,.true.,dim1_all_tasks=elements_all,&
