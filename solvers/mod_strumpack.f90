  ...
  contains
    ...
    subroutine strumpack_set_mat_core(spss,a_mat)
        ...
        if ((.not. dflag).and.(n_cpu.gt.1)) then
          ...
          call distribute_rows_core(a_mat%ng,n_cpu,a_mat%block_size,distr)
        ...
        elseif (n_cpu.eq.1) then
          call distribute_rows_core(a_mat%ng,1,a_mat%block_size,distr)
          n_d = a_mat%ng
          nnz_d = a_mat%nnz    
    ...
    subroutine distribute_rows_core(n,n_cpu,block_size,distr)
      !> Distribute rows between members of MPI group
        use, intrinsic :: iso_c_binding
        use mod_integer_types
        implicit none

        integer, intent(in) :: n_cpu, block_size
        ...
        nr = block_size*((n/block_size)/n_cpu)
        nr(n_cpu) = nr(n_cpu) + (n - sum(nr))
