...
subroutine solve_newton(solver, a_mat, rhs_vec, deltas, mhd_sim,tag)
  ...
  newton_loop: do inewton=1, newton_solver%maxNewton
    ...
    if (tol_Newton.le.(solve_tol)) then
      converged = .true.
      if (verbose) then
        write(*,*) "Newton converged:", inewton, tol_Newton
      endif
    
      call node_list_restore(mhd_sim%node_list, store_value, store_delta)
      call update_values(mhd_sim%element_list, mhd_sim%node_list, deln)
