...
#include "r3_info.h"
  ...
  INTEGER                  :: n_cpu
  INTEGER                  :: my_id
  ! global index_min, index_max for this cpu
  ...
  REAL*8                   :: t_analysis_0, t_analysis_1
  REAL*8                   :: t_fact_0, t_fact_1, t_comm_0, t_comm_1
  ...
  if (gmres) then
     if (.not. use_murge_element) then
        CALL MPI_Allreduce(m_loc,mumps_par%N,1,MPI_INTEGER,MPI_SUM,            &
             &             MPI_COMM_N,ierr)
        CALL MPI_Allreduce(mumps_par%NZ_loc,mumps_par%nz,1,MPI_INTEGER,        &
  ...
  if (.not. use_murge_element) then
     IF (ALLOCATED(column_scaling))                                            &
          call tr_deallocate(column_scaling,"column_scaling",CAT_PRECOND)
     IF (ALLOCATED(column_local))                                              &
          call tr_deallocate(column_local,"column_local",CAT_PRECOND)
     ...
     CALL MPI_AllReduce(column_local,column_scaling,mumps_par%N,               &
  ...
  IF (my_id .EQ. 0)                                                            &
       WRITE(*,'(A,f8.3)') ' PASTIX, scale     : ',t_scale_1-t_scale_0
  ...
  IF (.NOT. use_murge_element) THEN
     ...
     IF (ALLOCATED(counts))                                                    &
          call tr_deallocate(counts,"counts",CAT_PRECOND)
     IF (ALLOCATED(displacements))                                             &
          call tr_deallocate(displacements,"displacements",CAT_PRECOND)

     call tr_allocate(counts,1,n_cpu,"counts",CAT_PRECOND)
     call tr_allocate(displacements,1,n_cpu,"displacements",CAT_PRECOND)

     CALL MPI_Allgather(mumps_par%nz_loc,1,MPI_INTEGER,counts,1,MPI_INTEGER,   &
          &             MPI_COMM_WORLD,ierr)
     ...
     CALL MPI_AllgatherV(A_glob,mumps_par%nz_loc,MPI_DOUBLE_PRECISION,         &
          &              mumps_par%A, counts, displacements,                   &
  ...
  IF ((.NOT. use_murge_element) .AND. (.NOT. murge_initialised)) THEN
     ...
     CALL coicsr(mumps_par%N, mumps_par%NZ, 1, mumps_par%A,                    &
          &      mumps_par%IRN,mumps_par%JCN,sparskit_work)

     CALL CPU_TIME(t_comm_1)

     IF (my_id .EQ. 0)                                                         &
          WRITE(*,'(A,f8.3)') ' PASTIX, comm      : ',t_comm_1-t_comm_0
     ...
     IF (murge_solver == MURGE_SOLVER_PASTIX) THEN
        ...
        CALL MURGE_SetOptionINT(murge_id, IPARM_MATRIX_VERIFICATION, API_YES,  &
             &                  ierr)
...
!$omp end parallel
  ...
  IF ((.NOT. use_murge_element) .AND. (.NOT. pastix_analysed)) THEN
     ...
     CALL MURGE_GraphGlobalCSC(murge_id, mumps_par%n, mumps_par%jcn,           &
          &                    mumps_par%irn, -1, ierr)
     call system_clock(count=t1)
     nb_periods = t1-t0
     if (t1<t0) nb_periods = nb_periods + nb_periodes_max
     write(*,FMT_TIMING) my_id,                                                &
          ' system_clock elapsed time in MURGE_GraphGlobalCSC ',               &
          REAL(nb_periods)/nb_periodes_sec

     call system_clock(count=t0)
     CALL MURGE_MatrixGlobalCSC(murge_id, mumps_par%n, mumps_par%jcn,          &
          &                     mumps_par%irn, mumps_par%A, -1,                &
          &                     MURGE_ASSEMBLY_OVW, murge_sym, ierr)
     call system_clock(count=t1)
     nb_periods = t1-t0
     if (t1<t0) nb_periods = nb_periods + nb_periodes_max
     write(*,FMT_TIMING) my_id,                                                &
          ' system_clock elapsed time in MURGE_MatrixGlobalCSC ',              &
          REAL(nb_periods)/nb_periodes_sec
...
!      call tr_deallocate(rhs_tmp,"rhs_tmp",CAT_PRECOND)
  ...
  if (t1<t0) nb_periods = nb_periods + nb_periodes_max
  write(*,FMT_TIMING) my_id,                                                   &
       ' system_clock elapsed time in MURGE_SetGlobalRhs ',                    &
       REAL(nb_periods)/nb_periodes_sec
...
!     write(9, '(I8.8,1X,E20.12)'), k, mumps_par%rhs(k)
!  end do
!  close(unit=9)
!
  call system_clock(count=t1)
  ...
  if (t1<t0) nb_periods = nb_periods + nb_periodes_max
  write(*,FMT_TIMING) my_id,                                                   &
       ' system_clock elapsed time in MURGE_GetGlobalSolution ',               &
       REAL(nb_periods)/nb_periodes_sec
  ...
  if (.not. gmres) then
     ...
     if (my_id_n .eq. 0) then
        ...
        if (my_id .eq. 0 ) then
           ...
           rhs_tmp(1:ndof_glob:n_tor) =                                        &
                mumps_par%rhs(1:murge_global_n*murge_ndof)

        else

           rhs_tmp(2*murge_harmonic-2:ndof_glob:n_tor) =                       &
                mumps_par%rhs(1:murge_global_n*murge_ndof:2)
           rhs_tmp(2*murge_harmonic-1:ndof_glob:n_tor) =                       &
                mumps_par%rhs(2:murge_global_n*murge_ndof:2)

        endif

        call MPI_AllReduce(RHS_tmp,deltas,ndof_glob,MPI_DOUBLE_PRECISION,      &
             &             MPI_SUM,MPI_COMM_MASTER,ierr)
