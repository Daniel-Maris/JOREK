...
function initial_run () {
  ${codedir}/util/setinput.sh input nstep_n=0 tstep_n=0.                                  || exit 1
  $MPIRUN 1 ./jorek_model${jorekmodel}_1 < input | tee logfile_initial                    || exit 1
  ${codedir}/util/setinput.sh input nstep_n=10,10,10 tstep_n=10.,100.,1000. restart=.t.   || exit 1
...
function restart_run () {
  ${codedir}/util/setinput.sh input restart=.f. restart_particles=.t. nstep_n=1 tstep_n=1000. nout=1 || exit 1
