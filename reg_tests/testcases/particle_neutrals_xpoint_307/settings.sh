...
function initial_run () {
  ${codedir}/util/setinput.sh input restart_particles=.f. restart=.f. nstep_n=10,10,10 tstep_n=10.,100.,1000. use_ncs=.f. || exit 1
  $MPIRUN 1 ./jorek_model${jorekmodel}_1 < input | tee logfile_initial                                                    || exit 1
  ${codedir}/util/setinput.sh input restart_particles=.f. restart=.t. nstep_n=10 tstep_n=10. use_ncs=.t.                  || exit 1
