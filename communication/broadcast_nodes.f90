...
#ifdef fullmhd
bufsize = node_list%n_nodes * ((n_tor*(n_order+1)*n_dim + 2*n_tor*(n_order+1)*n_var+2*(n_order+1)+2)*IDBL_EXT + (n_order+1 + 1+3+1)*INT_EXT + (2)*ILOG_EXT)
#elif altcs
bufsize = node_list%n_nodes * ((n_tor*(n_order+1)*n_dim + 2*n_tor*(n_order+1)*n_var+2*(n_order+1)+2)*IDBL_EXT + (n_order+1 + 1+3+1)*INT_EXT + (2)*ILOG_EXT)
#else
bufsize = node_list%n_nodes * ((n_tor*(n_order+1)*n_dim + 2*n_tor*(n_order+1)*n_var+2)*IDBL_EXT + (n_order+1 + 1+3+1)*INT_EXT + (2)*ILOG_EXT)
#endif
...
if (my_id .eq. 0) then
  ...
  do i=1,node_list%n_nodes
    ...
    call MPI_PACK(anode%x              ,n_tor*(n_order+1)*n_dim      ,MPI_DOUBLE_PRECISION,buffer,bufsize,position,MPI_COMM_WORLD,ierr)
...
if (my_id .ne. 0) then
  ...
  do i=1,node_list%n_nodes
    ...
    call MPI_UNPACK(buffer,bufsize,position,anode%x              ,n_tor*(n_order+1)*n_dim      ,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr)
