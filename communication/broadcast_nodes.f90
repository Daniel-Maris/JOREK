...
#ifdef fullmhd
bufsize = node_list%n_nodes * ((n_coord_tor*(n_order+1)*(n_dim+2*3) + 2*n_tor*(n_order+1)*n_var+3*(n_order+1)+2)*IDBL_EXT + (n_order+1 + 1+3+1)*INT_EXT + (2)*ILOG_EXT)
#elif altcs                                                   
bufsize = node_list%n_nodes * ((n_coord_tor*(n_order+1)*(n_dim+2*3) + 2*n_tor*(n_order+1)*n_var+3*(n_order+1)+2)*IDBL_EXT + (n_order+1 + 1+3+1)*INT_EXT + (2)*ILOG_EXT)
#else                                
bufsize = node_list%n_nodes * ((n_coord_tor*(n_order+1)*(n_dim+2*3) + 2*n_tor*(n_order+1)*n_var+2 + n_order+1)*IDBL_EXT + (n_order+1 + 1+3+1)*INT_EXT + (2)*ILOG_EXT)
#endif
