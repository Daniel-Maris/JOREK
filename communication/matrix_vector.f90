...
subroutine matrix_vector(x_vec, a_mat, y_vec)
  ...
  counts = x_vec%n
  call MPI_BCAST(x_vec%val,counts,MPI_DOUBLE_PRECISION,0,a_mat%comm,ierr)
...
!$omp   shared(a_mat, x_vec, n_blocks, n_blocksize, index_offset) &
...
! --- End: Two different kernels for matrix-vector multiplication depending on n_tor
  ...
  call mpi_allgatherv(y_tmp, ndof_local, MPI_DOUBLE_PRECISION, y_vec, recv_counts, recv_disp, MPI_DOUBLE_PRECISION, a_mat%comm,ierr)
