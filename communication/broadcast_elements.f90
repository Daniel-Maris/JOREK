...
bufsize = element_list%n_elements * ((2*n_vertex_max+8+5)*INT_EXT + n_vertex_max*(n_order+1)*IDBL_EXT + ILOG_EXT)
...
if (my_id .eq. 0) then
  ...
  do ife=1,element_list%n_elements
    ...
    call MPI_PACK(anelement%axis_element,1,MPI_LOGICAL,buffer,bufsize,position,MPI_COMM_WORLD,ierr)
...
if (my_id .ne. 0) then
  ...
  do ife=1,element_list%n_elements
    ...
    call MPI_UNPACK(buffer,bufsize,position,anelement%axis_element,1,MPI_LOGICAL,MPI_COMM_WORLD,ierr)
