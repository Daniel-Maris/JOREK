...
parser.add_argument("-p", "--phi", type=list, default=[147, 150], help="Phi coordinate")
...
def visualise():
    if not(args.bezier):
        ...
        #values
        val_tor = ids.process[0].ggd.array[0].ion[0].emissivity.array[0].coefficients
        a = np.shape(val_tor)
        n_tor = a[0]
        ...
        val_tor1 = np.array([ids.process[0].ggd.array[0].ion[0].emissivity.array[0].coefficients])
        a = np.shape(val_tor1)
        n_tor = a[1]
        valu = np.reshape(val_tor1, (1, n_tor, 4, len(xyz0)))
        ...
        nam = ["Radiation (W/m^3)", "u", "j", "w", "rho", "T", "v_par"]
        for i in range(1):
