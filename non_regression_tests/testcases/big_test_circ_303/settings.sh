# --- General settings
jorekmodel="303"
description="Tearing mode, circular plasma, model$jorekmodel, n_tor=9."
mpitasks=10
binaries="jorek_model${jorekmodel}_9 rst_bin2hdf5 rst_hdf52bin"
...
function compile_jorek () {
  ...
  ./util/config.sh model=$jorekmodel n_tor=9 n_plane=32 n_period=1                    || exit 1
  make cleanall                                                                      || exit 1
  make $compilopt jorek_model${jorekmodel} rst_bin2hdf5 rst_hdf52bin                 || exit 1
  mv jorek_model${jorekmodel} jorek_model${jorekmodel}_9                             || exit 1
}
...
function initial_run () {
  ...
  ${MPIRUN} $mpitasks ./jorek_model${jorekmodel}_9 < input | tee -a logfile          || exit 1
}
