module mod_equations
  ...
  type(algexpr), parameter, private :: visco      = algexpr(basic=.true.,var=25)
  ! Auxiliary variables
  ...
  type(algexpr), parameter, private :: tjx        = algexpr(basic=.true.,var=31)
  type(algexpr), parameter, private :: tjy        = algexpr(basic=.true.,var=32)
  type(algexpr), parameter, private :: tjp        = algexpr(basic=.true.,var=33)
  type(algexpr), parameter, private :: tjchi      = algexpr(basic=.true.,var=34)
  
  type(const), private :: tstep, zeta, theta, visco_num
  ...
  type(algexpr), private :: a_Bv2, a_j0x, a_j0y, a_j0p, a_j0chi, a_tjx, a_tjy, a_tjp, a_tjchi
  
  integer, parameter :: n_rhs = 3, n_amat = 14, n_aux = 12
  
  type(algexpr), private :: rhs1e, rhs2e, rhs5e
  ...
  type(algexpr), private :: ea_j0chi, ea_tjchi, ea_Bv2x, ea_Bv2y, ea_Bv2p
  ...
  subroutine init_equations()
    use phys_module, only: time_evol_zeta, time_evol_theta, Ivisco_num => visco_num, Itstep => tstep
    implicit none
    integer       :: i
    ...
    a_j0x =  -(zj0 + 2.d0*dx(Psi0)/R)*dx(chi) + Bv_parderiv(dx(Psi0)) - gradprod(Psi0,dx(chi)) !+ dy(F)/R
    a_j0y =  -(zj0 + 2.d0*dx(Psi0)/R)*dy(chi) + Bv_parderiv(dy(Psi0)) - gradprod(Psi0,dy(chi)) !- dx(F)/R
...
!           - (visco*T_0**(1.5d0))*gradprod(v,w0)/(sqrtT0c*sqrtT0c*sqrtT0c) - visco_num*Lap(v)*Lap(w0)/R) &
...
!    else
      amat11 = (1.d0 + zeta)*v*Bv_pbrack(psi_v,Psi) &
             + tstep*theta*(Bv_pbrack(Psi,Phi0)*Bv_pbrack(v,psi_v)/Bv2 + eta*(dx(v)*(dy(psi_v)*tjp - dp(psi_v)*tjy/R) &
...
!    end if
    ...
    amat52e = Dexpand(deepcopy(amat52)); 
    amat55e = Dexpand(deepcopy(amat55))
  ...
  subroutine init_eq_struct()
    ...
    if (.not. allocated(thread_eq)) then
      ...
      do i=1,nbthreads
        allocate(thread_eq(i)%eq(2*n_var+22,0:n_order-1,0:n_order-1,0:n_order-1))
#ifdef DEBUG
  ...
  subroutine get_aux(aux,varnames)
    ...
    aux = (/ a_Bv2, ea_Bv2x, ea_Bv2y, ea_Bv2p, a_j0x, a_j0y, a_j0p, ea_j0chi, a_tjx, a_tjy, a_tjp, ea_tjchi /)
    varnames(1) = "eq(26,0,0,0)"; varnames(2) = "eq(26,1,0,0)"; varnames(3) = "eq(26,0,1,0)"; varnames(4) = "eq(26,0,0,1)"
    do i=5,n_aux
      write(num,'(I2)') ((i-3)+2*n_var+13)
