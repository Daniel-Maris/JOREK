...
do ms=1, n_gauss
 ...
 do mt=1, n_gauss
   ...
   do mp = 1, n_plane
     ...
     eq(n_var+1:2*n_var,1,0,0) = (y_t(mp,ms,mt)*delta_s(mp,:,ms,mt) - y_s(mp,ms,mt)*delta_t(mp,:,ms,mt))/xjac
     eq(n_var+1:2*n_var,0,1,0) = (-x_t(mp,ms,mt)*delta_s(mp,:,ms,mt) + x_s(mp,ms,mt)*delta_t(mp,:,ms,mt))/xjac
     eq(n_var+1:2*n_var,0,0,1) = delta_p(mp,:,ms,mt) - eq(n_var+1:2*n_var,1,0,0)*x_p(mp,ms,mt) - eq(n_var+1:2*n_var,0,1,0)*y_p(mp,ms,mt)
     
     eq(2*n_var+3,:,:,:) = get_chi(x_g(mp,ms,mt),y_g(mp,ms,mt),phi)  ! Vacuum scalar magnetic potential (chi) and field (grad chi)
     eq(2*n_var+4,0,0,0) = x_g(mp,ms,mt); eq(2*n_var+4,1,0,0) = 1.d0 ! Cylindrical R coordinate
     
     eq(2*n_var+5,0,0,0) = press_gvec(ms,mt)            ! Pressure, as imported from GVEC
     eq(2*n_var+6:2*n_var+8,0,0,0) = B_gvec(:,mp,ms,mt) ! Magnetic field, as imported from GVEC
...
#ifdef DEBUG
     eq(2*n_var+9,0,0,0) = eval(thread_eq(tid)%aBv2seq);  eq(2*n_var+9,1,0,0) = eval(thread_eq(tid)%aBv2xseq)
     eq(2*n_var+9,0,1,0) = eval(thread_eq(tid)%aBv2yseq); eq(2*n_var+9,0,0,1) = eval(thread_eq(tid)%aBv2pseq)
     eq(2*n_var+10,0,0,0) = eval(thread_eq(tid)%aB2seq)
#else
     ...
     do i=1,n_vertex_max
       ...
       do j=1,n_order+1
         ...
         do im=i_tor_min,i_tor_max
           ...
           eq(2*n_var+1,1,0,0) = (y_t(mp,ms,mt)*h_s(i,j,ms,mt) - y_s(mp,ms,mt)*h_t(i,j,ms,mt))*element%size(i,j)*HZ(im,mp)/xjac
           eq(2*n_var+1,0,1,0) = (-x_t(mp,ms,mt)*h_s(i,j,ms,mt) + x_s(mp,ms,mt)*h_t(i,j,ms,mt))*element%size(i,j)*HZ(im,mp)/xjac
           eq(2*n_var+1,0,0,1) = H(i,j,ms,mt)*element%size(i,j)*HZ_p(im,mp) - eq(2*n_var+1,1,0,0)*x_p(mp,ms,mt) - eq(2*n_var+1,0,1,0)*y_p(mp,ms,mt)
           ...
           rhs_ij_5 = 0.d0 ! eval(thread_eq(tid)%rhs5seq)*BigR*xjac
           ...
           do k=1,n_vertex_max
             ...
             do l=1,n_order+1
               ...
               do in=i_tor_min,i_tor_max
                 ...
                 eq(2*n_var+2,1,0,0) = (y_t(mp,ms,mt)*h_s(k,l,ms,mt) - y_s(mp,ms,mt)*h_t(k,l,ms,mt))*element%size(k,l)*HZ(in,mp)/xjac
                 eq(2*n_var+2,0,1,0) = (-x_t(mp,ms,mt)*h_s(k,l,ms,mt) + x_s(mp,ms,mt)*h_t(k,l,ms,mt))*element%size(k,l)*HZ(in,mp)/xjac
                 eq(2*n_var+2,0,0,1) = H(k,l,ms,mt)*element%size(k,l)*HZ_p(in,mp) - eq(2*n_var+2,1,0,0)*x_p(mp,ms,mt) - eq(2*n_var+2,0,1,0)*y_p(mp,ms,mt)
...
!---------------------------------------------------------------- equation 2
                 ...
                 amat_23 = 0.d0 ! eval(thread_eq(tid)%amat23seq)*BigR*xjac/F0
                 amat_24 = 0.d0 ! eval(thread_eq(tid)%amat24seq)*BigR*xjac
                 amat_25 = 0.d0 ! eval(thread_eq(tid)%amat25seq)*BigR*xjac
                 amat_26 = 0.d0 ! eval(thread_eq(tid)%amat26seq)*BigR*xjac

!---------------------------------------------------------------- equation 3
                 amat_31 = 0.d0 ! eval(thread_eq(tid)%amat31seq)*BigR*xjac/F0
...
!---------------------------------------------------------------- equation 5
                 amat_51 = 0.d0 ! eval(thread_eq(tid)%amat51seq)*BigR*xjac/F0
                 amat_52 = 0.d0 ! eval(thread_eq(tid)%amat52seq)*BigR*xjac
