     ...
     do i=1,n_vertex_max
       ...
       do j=1,n_order+1
         ...
         do im=i_tor_min,i_tor_max
           ...
           eq(n_var+1,0,0,2) = H(i,j,ms,mt)*element%size(i,j)*HZ_pp(im,mp) - x_pp(mp,ms,mt)*eq(n_var+1,1,0,0) - 2.d0*(x_p(mp,ms,mt)*v_xp &
                               + y_p(mp,ms,mt)*v_yp) - y_pp(mp,ms,mt)*eq(n_var+1,0,1,0) + x_p(mp,ms,mt)**2*eq(n_var+1,2,0,0)             &
                               + 2.d0*x_p(mp,ms,mt)*y_p(mp,ms,mt)*eq(n_var+1,1,1,0) + y_p(mp,ms,mt)**2*eq(n_var+1,0,2,0)
           eq(n_var+1,1,0,1) = v_xp - x_p(mp,ms,mt)*eq(n_var+1,2,0,0) - y_p(mp,ms,mt)*eq(n_var+1,1,1,0)
           eq(n_var+1,0,1,1) = v_yp - y_p(mp,ms,mt)*eq(n_var+1,0,2,0) - x_p(mp,ms,mt)*eq(n_var+1,1,1,0)
           ...
           do k=1,n_vertex_max
             ...
             do l=1,n_order+1
               ...
               do in=i_tor_min,i_tor_max
                 ...
                 eq(n_var+2,0,0,2) = H(k,l,ms,mt)*element%size(k,l)*HZ_pp(in,mp) - x_pp(mp,ms,mt)*eq(n_var+2,1,0,0) - 2.d0*(x_p(mp,ms,mt)*u_xp &
                                     + y_p(mp,ms,mt)*u_yp) - y_pp(mp,ms,mt)*eq(n_var+2,0,1,0) + x_p(mp,ms,mt)**2*eq(n_var+2,2,0,0)             &
                                     + 2.d0*x_p(mp,ms,mt)*y_p(mp,ms,mt)*eq(n_var+2,1,1,0) + y_p(mp,ms,mt)**2*eq(n_var+2,0,2,0)
                 eq(n_var+2,1,0,1) = u_xp - x_p(mp,ms,mt)*eq(n_var+2,2,0,0) - y_p(mp,ms,mt)*eq(n_var+2,1,1,0)
                 eq(n_var+2,0,1,1) = u_yp - y_p(mp,ms,mt)*eq(n_var+2,0,2,0) - x_p(mp,ms,mt)*eq(n_var+2,1,1,0)
