...
subroutine init_live_data_model(file_handle)
  ...
  integer :: i
  real*8  :: psin, FFp, dFFp_dpsi, dens, dn_dpsi, temp, dT_dpsi, S_rho, S_T, S_Te, d_perp, zk_perp, zk_iperp, zk_eperp
  real*8  :: d, d1, d2, d3, d4, d5, d6, d7, d8 ! dummies
  ...
  if (with_TiTe) then
    ...
    write(file_handle,'(A)') '@input_profiles: %"psin"       "rho"    "drho/dpsin"   "S_rho"     "S_T"     "S_Te"        "D_perp"    "ZK_iperp"       "ZK_eperp"'
  else
  ...
  do i = 0, 200
    ...
    if (with_TiTe) then
      call sources    (xpoint,xcase,0.d0,(/-99.d0,-99.d0/),psin,0.d0,1.d0,S_rho,S_T, S_Te)
    else
    ...
    if (with_TiTe) then
      ...
      write(file_handle,'(a,20es13.4e3)') '@input_profiles: ', psin, dens, dn_dpsi, S_rho, S_T, S_Te, d_perp, zk_iperp, zk_eperp
    else
