...
subroutine boundary_conditions( my_id, node_list, element_list, bnd_node_list, local_elms,    & 
  ...
  ! Internal parameters
  real*8  :: zbig
  real*8  :: T0, Vpar0, dT0_ds, dVpar0_ds, dBigR_ds
  ...
  real*8  :: R_s, R_t, Z_s, Z_t, ps0_s, ps0_t, ps0_x, ps0_y
  real*8  :: direction, xjac
  real*8  :: Vpar0_pol_R, Vpar0_pol_Z, Vpol_R, Vpol_Z, 
  real*8  :: Btot
  real*8  :: grad_psi, u0_s, u0_t, u0_x, u0_y
  integer :: i_bnd, i, ife, inode1, inode2, knode, j, l
  integer :: index_i, index_large_i, index_large_k, index_node2, ierr
  integer :: ijA_position2, kv, kT, ku, ilarge_vv, ilarge_vT, ilarge_vus
			  ...
			  if ( (xcase2 .eq. 3) .and. (node_list%node(inode)%x(1,2) .gt. (Z_xpoint(1)+Z_xpoint(2))/2.d0) ) direction = -direction
                          ...
                          if (use_murge .and. use_murge_element) then
                             ...
                             if (is_local) then
                                ...
                                if (only_count = .false.) then
                                   Rhs_loc( n_tor*n_var * (index_node-1) +   &
                                            (kv-1)*n_tor + in) =             &
                                        Zbig * ( - Vpar0 + BigR**2 *         &
                                                 U0_s /ps0_s +               &
                          ...
                          if (use_murge .and. use_murge_element) then
                             ...
                             if (is_local) then
                                ...
                                call murge_add_one_entry( index_node2, kv, in, &
                                     ...
                                     &   dT0_ds * direction, solve_only,       &
                                     &                    gmres,               &
                                     &                    cnt, cnt_prod,       &
                                     &                    only_count)
                                if (only_count = .false.) then
                                   Rhs_loc( n_tor * n_var * (index_node2-1) + &
                                            (kv-1)*n_tor + in) =              &
                                            Zbig*(-dVpar0_ds +  0.5d0 / Btot *&
                                            GAMMA / sqrt(GAMMA*T0) * dT0_ds * &
                                            direction)
                                end if
                             end if
                          else
