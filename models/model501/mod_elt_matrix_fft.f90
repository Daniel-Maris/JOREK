...
do ms=1, n_gauss
 ...
 do mt=1, n_gauss
   ...
   do mp = 1, n_plane
     ...
     dr0_corr_dr0 = dcorr_neg_temp_dT(r0) ! Improve the correction
     d2r0_corr_dr02 = d2corr_neg_temp_dT2(r0,(/1.d-8,1.d-5/))
...
!----------------- simplified version of 2nd derivatives (for some unknown reason this is more stable!)
     ...
     if ( visco_T_dependent ) then       
       ...
       d2visco_d2T = (  visco * (3.75d0) * T0_corr**(-3.5d0) * T_0**(1.5d0) ) * d2T0_corr_dT2
     else
       visco_T   = visco
       dvisco_dT = 0.d0
       d2visco_d2T = 0.d0
...
!###################################################################################################
         ...
         rhs_ij_5_k =  - (D_par-D_prof) * BigR / BB2 * Bgrad_rho_k_star * (Bgrad_rho-Bgrad_rhon)   * xjac * tstep &
...
!###################################################################################################
             ...
             amat_51_k = - (D_par-D_prof) * BigR * BB2_psi/ BB2**2 * Bgrad_rho_k_star * (Bgrad_rho-Bgrad_rhon)         * xjac * theta * tstep &
             ...
             amat_58_kn = - (D_par-D_prof) * BigR / BB2 * Bgrad_rho_k_star * Bgrad_rho_rhon_n       * xjac * theta * tstep &
