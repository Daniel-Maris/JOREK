...
module domains
  ...
  private
  public which_domain, in_plasma, in_sol, in_private, which_surface
  ...
  !> Determine whether the element is within a given tracked flux surfaces.
  integer function which_surface(node_list, element_list, R, Z, psi, xpoint, xcase, R_xpoint,       &
    Z_xpoint, psi_xpoint, psi_limit, R_axis, Z_axis, psi_axis)
    
    use phys_module, only: DOMAIN_PLASMA, DOMAIN_SOL, DOMAIN_OUTER_SOL, DOMAIN_UPPER_PRIVATE,        &
      DOMAIN_LOWER_PRIVATE, LOWER_XPOINT, UPPER_XPOINT, DOUBLE_NULL, id_surfaces, psi_surfaces
    ...
    if ( xpoint ) then
      ...
      if ( xcase == LOWER_XPOINT ) then
        ...
        if ( ( sign_corr*psi < sign_corr*psi_xpoint(1) ) .and. ( Z < Z_xpoint(1) ) ) then
          ...
          which_surface = DOMAIN_OUTER_SOL
        end if
