...
integer    :: inode, j
integer    :: n_bnd(4)                   ! 4 sides of the grid
integer    :: bnd_list(4,n_boundary_max) ! 4 sides of the grid
integer    :: bnd_list_tmp(n_boundary_max) ! tmp copy to re-order node list
  ...
  do i=1,node_list%n_nodes
    ...
    do nj=1,n_jropes
      ...
      if (rr .lt. 1.00*ww) then
        zjz        =      rho_jropes(nj) * (1.0 - (rr/ww)**2 )**2
        dj_dR      = -4.* rho_jropes(nj) * rr   /ww**2 * (1.0 - (rr/ww)**2 ) * drr_dR
        dj_dZ      = -4.* rho_jropes(nj) * rr   /ww**2 * (1.0 - (rr/ww)**2 ) * drr_dZ
        dj_dR_dR   = -4.* rho_jropes(nj)        /ww**2 * (1.0 - (rr/ww)**2 ) * drr_dR**2 & 
                     -4.* rho_jropes(nj) * rr   /ww**2 * (1.0 - (rr/ww)**2 ) * drr_dR2   & 
                     +8.* rho_jropes(nj) * rr**2/ww**4                       * drr_dR**2 
        dj_dZ_dZ   = -4.* rho_jropes(nj)        /ww**2 * (1.0 - (rr/ww)**2 ) * drr_dZ**2 & 
                     -4.* rho_jropes(nj) * rr   /ww**2 * (1.0 - (rr/ww)**2 ) * drr_dZ2   & 
                     +8.* rho_jropes(nj) * rr**2/ww**4                       * drr_dZ**2 
        dj_dR_dZ   = -4.* rho_jropes(nj)        /ww**2 * (1.0 - (rr/ww)**2 ) * drr_dR*drr_dZ & 
                     -4.* rho_jropes(nj) * rr   /ww**2 * (1.0 - (rr/ww)**2 ) * drr_dRdZ      & 
                     +8.* rho_jropes(nj) * rr**2/ww**4                       * drr_dR*drr_dZ 
      endif
...
! ---------------------------------------
! --- Important! This assumes we are on a rectangular grid!

! --- First, get a list of the boundaries on each side
n_bnd = 0
! --- Just redo the loop from grid_bezier_square.f90
inode = 0
do j=1,n_Z
  do i=1,n_R
    inode = inode + 1
    if (i .eq. 1) then
      n_bnd(1) = n_bnd(1) + 1
      bnd_list(1,n_bnd(1)) = inode
    endif
    if (i .eq. n_R) then
      n_bnd(2) = n_bnd(2) + 1
      bnd_list(2,n_bnd(2)) = inode
    endif
    if (j .eq. 1) then
      n_bnd(3) = n_bnd(3) + 1
      bnd_list(3,n_bnd(3)) = inode
    endif
    if (j .eq. n_Z) then
      n_bnd(4) = n_bnd(4) + 1
      bnd_list(4,n_bnd(4)) = inode
    endif
  enddo
enddo
! --- copy node indices
