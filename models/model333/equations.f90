...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_2
  ...
  amat(2,5)   = amat(2,5)														&
                ...
                + tau_IC * v * R**4 * u0_xy * (  rho_xx*T0 + 2.d0*rho_x*T0_x + rho*T0_xx						&
                                               - rho_yy*T0 - 2.d0*rho_y*T0_y - rho*T0_yy )			* xjac * theta * tstep	&                                          
                - tau_IC * v * R**4 * (rho_xy*T0 + rho_x*T0_y + rho_y*T0_x + rho*T0_xy) * (u0_xx-u0_yy)		* xjac * theta * tstep  &
	        ! --- Inverse diamagnetic terms
  ...
  amat(2,6)   = amat(2,6)														&
                ...
                + tau_IC * v * R**4 * u0_xy * ( r0_xx*T + 2.d0*r0_x*T_x + r0*T_xx							&
                                               -r0_yy*T - 2.d0*r0_y*T_y - r0*T_yy )				* xjac * theta * tstep	&                                        
                - tau_IC * v * R**4 * (r0_xy*T + r0_x*T_y + r0_y*T_x + r0*T_xy) * (u0_xx-u0_yy)               	* xjac * theta * tstep  &
	        ! --- Inverse diamagnetic terms
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_4
  ...
  ! --- The RHS term (diamagnetic part)	      
  !rhs(4) = rhs(4)											&
  !	   - tau_IC * W_dia / r0_corr2 * ( v_x  * P0_x + v_y  * P0_y)		* R * xjac	
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_4
  ...
  ! --- The LHS terms (diamagnetic part)
  amat(4,5) = amat(4,5)									        				&
              + tau_IC * W_dia / r0_corr2                * (v_x   * rho  *T0_x + v_y   * rho  *T0_y) * R  	* xjac 	&
              + tau_IC * W_dia / r0_corr2                * (v_x   * rho_x*T0   + v_y   * rho_y*T0  ) * R  	* xjac 	&
              - tau_IC * W_dia * rho / r0_corr**2       * (v_x   * P0_x       + v_y   * P0_y      ) * R    	* xjac 	

  amat(4,6) = amat(4,6)									        	    			&
              + tau_IC * W_dia / r0_corr2                * (v_x   * r0   *T_x  + v_y   * r0   *T_y ) * R   	* xjac &
              + tau_IC * W_dia / r0_corr2                * (v_x   * r0_x *T    + v_y   * r0_y *T   ) * R   	* xjac 	
