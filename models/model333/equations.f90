...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_1
  ...
  rhs(1) = rhs(1)											&
           - v * tau_IC/(rho_corr*BB2) * F0**2/R**2 * (ps0_x*p0_y - ps0_y*p0_x)		* xjac * tstep  & 		  
           + v * tau_IC/(rho_corr*BB2) * F0**3/R**3 * eps_cyl * p0_p			* xjac * tstep  
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_1
  ...
  amat(1,1)   = amat(1,1)													&
                - v * tau_IC/(r0*BB2**2) * BB2_psi * F0**2/R**2 * (ps0_x*p0_y - ps0_y*p0_x)		* xjac * theta * tstep	&
                + v * tau_IC/(r0*BB2**2) * BB2_psi * F0**3/R**3 * eps_cyl * p0_p			* xjac * theta * tstep	&
                + v * tau_IC/(r0*BB2) * F0**2/R**2 * (psi_x * p0_y - psi_y * p0_x)			* xjac * theta * tstep
  
  amat(1,5)   = amat(1,5)													&
                + v * tau_IC/(r0*BB2) * F0**2/R**2 * T0  * (ps0_x*rho_y - ps0_y*rho_x)			* xjac * theta * tstep	&
	        + v * tau_IC/(r0*BB2) * F0**2/R**2 * rho * (ps0_x*T0_y  - ps0_y*T0_x )			* xjac * theta * tstep	&
	        - v * tau_IC/(r0*BB2) * F0**3/R**3 * eps_cyl * rho * T0_p				* xjac * theta * tstep	&
		- v * tau_IC * rho /(r0**2 * BB2) * F0**2/R**2 * (ps0_x*p0_y - ps0_y*p0_x)		* xjac * theta * tstep	&		    
		+ v * tau_IC * rho /(r0**2 * BB2) * F0**3/R**3 * eps_cyl * p0_p				* xjac * theta * tstep 
  
  amat_n(1,5) = amat_n(1,5)													&
                - v * tau_IC/(r0*BB2) * F0**3/R**3 * eps_cyl * T0 * rho_p				* xjac * theta * tstep
  
  amat(1,6)   = amat(1,6)													&
                + v * tau_IC/(r0*BB2) * F0**2/R**2 * r0 * (ps0_x*T_y  - ps0_y*T_x ) 			* xjac * theta * tstep	&
	        + v * tau_IC/(r0*BB2) * F0**2/R**2 * T  * (ps0_x*r0_y - ps0_y*r0_x)			* xjac * theta * tstep	&
		- v * tau_IC/(r0*BB2) * F0**3/R**3 * eps_cyl * T * r0_p					* xjac * theta * tstep 

  amat_n(1,6) = amat_n(1,6)													&
                - v * tau_IC/(r0*BB2) * F0**3/R**3 * eps_cyl * r0 * T_p					* xjac * theta * tstep 
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_2
  ...
  amat(2,5)   = amat(2,5)														&
                ! --- Diamagnetic terms
                + tau_IC * v * R**4 * T0  * (rho_x  * w0_y - rho_y  * w0_x)                                     * xjac * theta * tstep  &
                + tau_IC * v * R**4 * rho * (T0_x   * w0_y - T0_y   * w0_x)                                     * xjac * theta * tstep  &
                + tau_IC     * R**3 * (T0_y*rho + T0*rho_y) * (v_x*u0_x + v_y*u0_y)                             * xjac * theta * tstep  &
                + tau_IC * v * R**4 * ( u0_xy        * (rho_xx*T0 + 2.d0*rho_x*T0_x + rho*T0_xx                                         &
	        ...
	        + tau_IC * W_dia * R**4 / rho_corr * (rho_x * P0_x + rho_y * P0_y)     * (v_x*u0_y - v_y*u0_x)	* xjac * theta * tstep	&
	        + tau_IC * W_dia * R**4 / rho_corr * ( r0_x * (rho_x*T0 + rho*T0_x) 							&
		                                     + r0_y * (rho_y*T0 + rho*T0_y) )  * (v_x*u0_y - v_y*u0_x)	* xjac * theta * tstep	&
	        - tau_IC * W_dia * R**4 * rho/rho_corr**2.d0 * (r0_x*P0_x + r0_y*P0_y) * (v_x*u0_y - v_y*u0_x)	* xjac * theta * tstep	&
	        ! --- Neoclassical term
                - amu_neo_prof * BB2 / (Btheta2+epsil)**2 * (ps0_x*v_x + ps0_y*v_y) * R							&
  ...
  amat(2,6)   = amat(2,6)														&
                ! --- Diamagnetic terms
                + tau_IC * v * R**4 * r0  * (T_x*w0_y - T_y*w0_x)                                               * xjac * theta * tstep  &
                + tau_IC * v * R**4 * T  * (r0_x*w0_y - r0_y*w0_x)                                              * xjac * theta * tstep  &
                + tau_IC     * R**3 * (r0_y*T + r0*T_y) * (v_x*u0_x + v_y*u0_y)                                 * xjac * theta * tstep  &
                + tau_IC * v * R**4 * ( u0_xy        * (r0_xx*T + 2.d0*r0_x*T_x + r0*T_xx                                               &
	        ...
	        + tau_IC * W_dia * R**4 / rho_corr * ( r0_x * (r0_x*T + r0*T_x)									&
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_4
  ...
  ! --- The RHS term (diamagnetic part)	      
  rhs(4) = rhs(4)											&
  	   - tau_IC * W_dia / rho_corr * ( v_x  * P0_x + v_y  * P0_y)		* R * xjac * tstep	
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_4
  ...
  ! --- The LHS terms (diamagnetic part)
  amat(4,5) = amat(4,5)									        				&
              + tau_IC * W_dia / rho_corr                * (v_x   * rho  *T0_x + v_y   * rho  *T0_y) * R  	* xjac * tstep	&
              + tau_IC * W_dia / rho_corr                * (v_x   * rho_x*T0   + v_y   * rho_y*T0  ) * R  	* xjac * tstep	&
              - tau_IC * W_dia * rho / rho_corr**2.d0    * (v_x   * P0_x       + v_y   * P0_y      ) * R  	* xjac * tstep	

  amat(4,6) = amat(4,6)									        	    			&
              + tau_IC * W_dia / rho_corr                * (v_x   * r0   *T_x  + v_y   * r0   *T_y ) * R    	* xjac * tstep	&
              + tau_IC * W_dia / rho_corr                * (v_x   * r0_x *T    + v_y   * r0_y *T   ) * R    	* xjac * tstep	
