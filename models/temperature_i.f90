...
real*8,  intent(out) :: dTi_dpsi,  dTi_dz                                             ! 1st order derivatives
real*8,  intent(out) :: dTi_dpsi2, dTi_dz2, dTi_dpsi_dz                               ! 2nd order derivatives
real*8,  intent(out) :: dTi_dpsi3, dTi_dpsi_dz2, dTi_dpsi2_dz,  dTi_dz3               ! 2rd order derivatives
real*8,  intent(out) :: dTi_dpsi4, dTi_dpsi_dz3, dTi_dpsi2_dz2, dTi_dpsi3_dz, dTi_dz4 ! 4th order derivatives
...
if (xpoint2) then
  ...
  dTi_dpsi4        = dprof1_dpsi4 *    atn_z * atn_z_u  

  dTi_dz           = prof1        * ( datn_z * atn_z_u +          atn_z * datn_z_u)
  dTi_dz2          = prof1        * (d2atn_z * atn_z_u + 2.d0 *  datn_z * datn_z_u        +   atn_z * d2atn_z_u)  
  dTi_dz3          = prof1        * (d3atn_z * atn_z_u + 4.d0 * d2atn_z * datn_z_u + 4.d0 *  datn_z * d2atn_z_u  + atn_z * d3atn_z_u)  
  dTi_dz4          = prof1        * (d4atn_z * atn_z_u + 4.d0 * d3atn_z * datn_z_u + 6.d0 * d2atn_z * d2atn_z_u &
  ...
  dTi_dpsi_dz3     = dprof1_dpsi  * (d3atn_z * atn_z_u + 4.d0 * d2atn_z * datn_z_u + 4.d0 *  datn_z * d2atn_z_u  + atn_z * d3atn_z_u)

  dTi_dpsi2_dz     = dprof1_dpsi2 * ( datn_z * atn_z_u +         atn_z * datn_z_u)
  dTi_dpsi2_dz2    = dprof1_dpsi2 * (d2atn_z * atn_z_u + 2.d0 * datn_z * datn_z_u  + atn_z * d2atn_z_u)

  dTi_dpsi3_dz     = dprof1_dpsi3 * ( datn_z * atn_z_u +         atn_z * datn_z_u)
  
else
  
  temperature_i_profile = prof1
  dTi_dpsi              = dprof1_dpsi
  dTi_dpsi2             = dprof1_dpsi2
  dTi_dpsi3             = dprof1_dpsi3
  dTi_dpsi4             = dprof1_dpsi4
  dTi_dz                = 0.d0
  dTi_dz2               = 0.d0
  dTi_dz3               = 0.d0
  dTi_dz4               = 0.d0
...
if (freeboundary_equil .and. num_Ti) then                       !if the temperature profile is given in a file and there is freeboundary equilibrium
  ...
  dTi_dpsi4             = dTi_dpsi4             * current_FB_fact
  dTi_dz                = dTi_dz                * current_FB_fact
  dTi_dz2               = dTi_dz2               * current_FB_fact
  dTi_dz3               = dTi_dz3               * current_FB_fact
  dTi_dz4               = dTi_dz4               * current_FB_fact
  dTi_dpsi_dz           = dTi_dpsi_dz           * current_FB_fact
  dTi_dpsi_dz2          = dTi_dpsi_dz2          * current_FB_fact
  dTi_dpsi_dz3          = dTi_dpsi_dz3          * current_FB_fact
  dTi_dpsi2_dz          = dTi_dpsi2_dz          * current_FB_fact
  dTi_dpsi2_dz2         = dTi_dpsi2_dz2         * current_FB_fact
  dTi_dpsi3_dz          = dTi_dpsi3_dz          * current_FB_fact
