program algexpr2fort
  ...
  if (n_aux2 .ne. 0) call get_aux2(aux2, varname_aux2)
  ...
  if (n_aux2 .ne. 0) then
    open(40, file="models/model"//model_num//"/aux2_unreadable.h", action="write", status="replace")
    do i=1,n_aux2
      full = varname_aux2(i) // "=" // gencode(aux2(i), varname)
      length = len(full)
      n = length/line_length
      if (n .gt. 1) then
        e = merge(n-1,n,n*line_length.eq.length)
        write(40,'(A,A)') full(1:line_length), "&"
        do j=2,e
          write(40,'(A,A,A)') "&", full((j-1)*line_length+1:j*line_length), "&"
        end do
        write(40,'(A,A)') "&", full(e*line_length+1:length)
      else
        write(40,'(A)') full
      end if
    end do
