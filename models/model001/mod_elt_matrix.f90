    ...
    do ms=1, n_gauss
      do mt=1, n_gauss
        ...
        do k=1,n_var
          ...
          eq_g(mp,k,ms,mt) = eq_g(mp,k,ms,mt) + nodes(i)%values(in,j,k) * esize(i,j) * BasFun(i,j,ms,mt)  
          eq_s(mp,k,ms,mt) = eq_s(mp,k,ms,mt) + nodes(i)%values(in,j,k) * esize(i,j) * BasFun_s(i,j,ms,mt)
          eq_t(mp,k,ms,mt) = eq_t(mp,k,ms,mt) + nodes(i)%values(in,j,k) * esize(i,j) * BasFun_t(i,j,ms,mt)

          eq_ss(mp,k,ms,mt) = eq_ss(mp,k,ms,mt) + nodes(i)%values(in,j,k) * esize(i,j) * BasFun_ss(i,j,ms,mt)
          eq_st(mp,k,ms,mt) = eq_st(mp,k,ms,mt) + nodes(i)%values(in,j,k) * esize(i,j) * BasFun_st(i,j,ms,mt)
          eq_tt(mp,k,ms,mt) = eq_tt(mp,k,ms,mt) + nodes(i)%values(in,j,k) * esize(i,j) * BasFun_tt(i,j,ms,mt)

          delta_g(mp,k,ms,mt) = delta_g(mp,k,ms,mt) + nodes(i)%deltas(in,j,k) * esize(i,j) * BasFun(i,j,ms,mt)  
          delta_s(mp,k,ms,mt) = delta_s(mp,k,ms,mt) + nodes(i)%deltas(in,j,k) * esize(i,j) * BasFun_s(i,j,ms,mt)
          delta_t(mp,k,ms,mt) = delta_t(mp,k,ms,mt) + nodes(i)%deltas(in,j,k) * esize(i,j) * BasFun_t(i,j,ms,mt) 
	...
	v_yy = (v_ss * x_t(ms,mt)**2 - 2.d0*v_st * x_s(ms,mt)*x_t(ms,mt) + v_tt * x_s(ms,mt)**2   &	        
        ...
        do k=1,n_vertex_max
          ...
          do l=1,n_order+1
            ...
            ! change basis function for axis elements
            u    = BasFun(k,l,ms,mt)    * esize(k,l)
            u_s  = BasFun_s(k,l,ms,mt)  * esize(k,l)
            u_t  = BasFun_t(k,l,ms,mt)  * esize(k,l) 
            u_ss = BasFun_ss(k,l,ms,mt) * esize(k,l) 
            u_tt = BasFun_tt(k,l,ms,mt) * esize(k,l) 
            u_st = BasFun_st(k,l,ms,mt) * esize(k,l)
		 
            u_x = (   y_t(ms,mt) * BasFun_s(k,l,ms,mt) - y_s(ms,mt) * BasFun_t(k,l,ms,mt) ) / xjac * esize(k,l) 
            u_y = ( - x_t(ms,mt) * BasFun_s(k,l,ms,mt) + x_s(ms,mt) * BasFun_t(k,l,ms,mt) ) / xjac * esize(k,l)
