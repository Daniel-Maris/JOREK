...
do i=1,2
  ...
  do j=1,2
    ...
    do ms=1, n_gauss
	  !----------------------changed size(i,j) to size(vertex(i),j)     NEDDS TO BE CHECKED!  should j also change to jdir?????

      x_g(ms)  = x_g(ms)  + nodes(i)%x(j,1) * element%size(vertex(i),j) * H1(i,j,ms)
      x_s(ms)  = x_s(ms)  + nodes(i)%x(j,1) * element%size(vertex(i),j) * H1_s(i,j,ms)

      y_g(ms)  = y_g(ms)  + nodes(i)%x(j,2) * element%size(vertex(i),j) * H1(i,j,ms)
      y_s(ms)  = y_s(ms)  + nodes(i)%x(j,2) * element%size(vertex(i),j) * H1_s(i,j,ms)
	  ...
	  !----------------------changed size(i,j) to size(vertex(i),j)     NEDDS TO BE CHECKED!  should j also change to jdir?????
            ...
            eq_g(mp,k,ms)  = eq_g(mp,k,ms)  + nodes(i)%values(in,j,k) * element%size(vertex(i),j) * H1(i,j,ms)   * HZ(in,mp)

            eq_s(mp,k,ms)  = eq_s(mp,k,ms)  + nodes(i)%values(in,j,k) * element%size(vertex(i),j) * H1_s(i,j,ms) * HZ(in,mp)

            eq_p(mp,k,ms)  = eq_p(mp,k,ms)  + nodes(i)%values(in,j,k) * element%size(vertex(i),j) * H1(i,j,ms)   * HZ_p(in,mp)

            eq_ss(mp,k,ms) = eq_ss(mp,k,ms) + nodes(i)%values(in,j,k) * element%size(vertex(i),j) * H1_ss(i,j,ms)* HZ(in,mp)

            delta_g(mp,k,ms) = delta_g(mp,k,ms) + nodes(i)%deltas(in,j,k) * element%size(vertex(i),j) * H1(i,j,ms)   * HZ(in,mp)
            delta_s(mp,k,ms) = delta_s(mp,k,ms) + nodes(i)%deltas(in,j,k) * element%size(vertex(i),j) * H1_s(i,j,ms) * HZ(in,mp)
...
do ms=1, n_gauss
   ...
   do mp = 1, n_plane
     ...
     gamma_sheeth = -3.d0 !* ps0_s / abs(ps0_s)
     ...
     do i=1,2                ! loop over nodes
       ...
       do j=1,2              ! loop over basis functions
         ...
         do im=1,n_tor
           ...
           v   =  H1(i,j,ms) * element%size(vertex(i),j) * HZ(im,mp)         ! test function

           rhs_ij_6 = v * gamma_sheeth * r0 * T0 * vpar0 * ps0_s * tstep     ! right hand side equation 6
