...
TG_num1    = TGNUM(1); TG_num2    = TGNUM(2); TG_num5    = TGNUM(5); TG_num6    = TGNUM(6); TG_num7    = TGNUM(7); TG_num8    = 0!TGNUM(8); !#############################
        ...
        do mp = 1, n_plane
          ...
          if (Wdia) then
            W_dia = + tauIC /r0_corr    * (pi0_xx + pi0_x/bigR + pi0_yy) &
                    - tauIC /r0_corr**2 * (r0_x*pi0_x + r0_y*pi0_y)
          else
            W_dia = 0.d0
          endif
...
!--------------------------------------------------------
          ...
          do im=n_tor_start, n_tor_end
            ...
            rhs_ij(var_psi) = v * eta_T  * (zj0 - current_source(ms,mt) - Jb)/ BigR * xjac * tstep &
                      ...
                      - v * tauIC/(r0_corr*BB2) * F0**2/BigR**2 * (ps0_s * pe0_t - ps0_t * pe0_s) * tstep &
                      + v * tauIC/(r0_corr*BB2) * F0**3/BigR**3 * pe0_p       * xjac * tstep &
              ...
              if (NEO) then
                rhs_ij(var_vpar) =  rhs_ij(var_vpar)  + amu_neo_prof(ms,mt)*BB2/(Btheta2+epsil)  &
                          * v * ( r0 * (ps0_x * u0_x + ps0_y * u0_y)               &
                                + tauIC   * (ps0_x * Pi0_x + ps0_y * Pi0_y)        &
                                + aki_neo_prof(ms,mt) * tauIC * r0 * (ps0_x * Ti0_x + ps0_y * Ti0_y) - r0 * Vpar0 * Btheta2) * xjac * tstep * BigR
              endif
            ...
            if ( with_TiTe ) then
              ...
              rhs_ij(var_T) =  v * BigR * heat_source_e(ms,mt)                                    * xjac * tstep &
                         ...
                         - (ZKe_par_T-ZKe_prof) * BigR / BB2 * Bgrad_T_star * Bgrad_T      * xjac * tstep &
                         - ZKe_prof * BigR * (v_x*T0_x + v_y*T0_y                   )     * xjac * tstep &
              ...
              rhs_ij_k(var_T) = - (ZKe_par_T-ZKe_prof) * BigR / BB2 * Bgrad_T_k_star * Bgrad_T * xjac * tstep &
                            - ZKe_prof * BigR * (                + v_p*T0_p /BigR**2 )     * xjac * tstep &
            ...
            do k=1,n_vertex_max
              ...
              do l=1,n_order+1
                ...
                do in = n_tor_start, n_tor_end
                  ...
                  if (Wdia) then
                    W_dia_rho = - tauIC *     rho/r0_corr**2 * (pi0_xx     + pi0_x    /bigR + pi0_yy    ) &
                                + tauIC          /r0_corr    * (pi0_xx_rho + pi0_x_rho/bigR + pi0_yy_rho) &
                                + tauIC * 2.0*rho/r0_corr**3 * (r0_x *pi0_x     + r0_y *pi0_y    )    &
                                - tauIC          /r0_corr**2 * (rho_x*pi0_x     + rho_y*pi0_y    )    &
                                - tauIC          /r0_corr**2 * (r0_x *pi0_x_rho + r0_y *pi0_y_rho)
                    W_dia_Ti  = + tauIC          /r0_corr    * (pi0_xx_Ti + pi0_x_Ti/bigR + pi0_yy_Ti) &
                                - tauIC          /r0_corr**2 * (r0_x  *pi0_x_Ti + r0_y  *pi0_y_Ti)
                  else
                    W_dia_rho = 0.d0
                    W_dia_Ti  = 0.d0
                  endif
                  ...
                  amat(var_psi,var_rho) = + v * tauIC/(r0_corr*BB2) * F0**2/BigR**2 * Te0 * (ps0_s * rho_t - ps0_t * rho_s) * theta * tstep &
                              + v * tauIC/(r0_corr*BB2) * F0**2/BigR**2 * rho * (ps0_s * Te0_t - ps0_t * Te0_s) * theta * tstep &
                              - v * tauIC/(r0_corr*BB2) * F0**3/BigR**3 * rho * Te0_p  * xjac          * theta * tstep &

                              - v * tauIC * rho /(r0_corr**2 * BB2) * F0**2/BigR**2 * (ps0_s * pe0_t - ps0_t * pe0_s) * theta * tstep &
                              + v * tauIC * rho /(r0_corr**2 * BB2) * F0**3/BigR**3 * pe0_p * xjac                    * theta * tstep 

                  amat_n(var_psi,var_rho) = - v * tauIC/(r0_corr*BB2) * F0**3/BigR**3 * Te0  * rho_p            * xjac * theta * tstep 
                  ...
                  amat_n(var_psi,var_Te) = - v * tauIC/(r0_corr*BB2) * F0**3/BigR**3 * r0 * Te_p                * xjac * theta * tstep 
                  ...
                  if (NEO) then
                    amat(var_u,var_psi) = amat(var_u,var_psi) &
                         ...
                         * (r0 * (ps0_x*u0_x + ps0_y*u0_y) + tauIC * (ps0_x*Pi0_x + ps0_y*Pi0_y) &
                         + aki_neo_prof(ms,mt) * tauIC * r0 * (ps0_x*Ti0_x + ps0_y*Ti0_y) -r0 * Vpar0 * Btheta2) * BigR * xjac * theta * tstep &                         
                         - amu_neo_prof(ms,mt) * BB2/((Btheta2+epsil)**2) * (ps0_x*v_x + ps0_y*v_y) * (r0*(psi_x*u0_x + psi_y*u0_y) &
                              + tauIC * (psi_x*Pi0_x + psi_y*Pi0_y)                                     &
                  ...
                  if ( NEO ) then
                    amat(var_u,var_rho) = amat(var_u,var_rho) &
                         ...
                         + tauIC * (ps0_x*(rho_x*Ti0 + rho*Ti0_x) + ps0_y*(rho_y*Ti0 + rho*Ti0_y))                                   &
                         + aki_neo_prof(ms,mt) * tauIC * rho * (ps0_x*Ti0_x + ps0_y*Ti0_y) - rho * Vpar0 * Btheta2) * BigR * xjac * tstep * theta
                  endif
                  ...
                  amat(var_rho,var_Ti) = - v * 2.d0 * tauIC * (Ti_y * r0 + Ti*r0_y) * BigR                           * xjac * theta * tstep
                  ...
                  if ( with_vpar ) then
                    ...
                    amat_k(var_rho,var_vpar) = + TG_num5 * 0.25d0 / BigR * 2.d0*vpar0*vpar                                 &
                                     * (r0_x * ps0_y - r0_y * ps0_x + F0 / BigR * r0_p)                       &
                                     * (                            + F0 / BigR * v_p) * xjac * theta * tstep * tstep 
                  ...
                  if ( with_vpar ) then
                    ...
                    if ( NEO ) then
                      amat(var_vpar,var_rho) = amat(var_vpar,var_rho) - v * amu_neo_prof(ms,mt)*BB2/(Btheta2+epsil) &
                            * (rho * (ps0_x*u0_x + ps0_y*u0_y) + tauIC*(ps0_x * (rho_x*Ti0 + rho*Ti0_x) + ps0_y*(rho_y*Ti0 + rho*Ti0_y)) &
                            + aki_neo_prof(ms,mt) * tauIC * rho*(ps0_x*Ti0_x + ps0_y*Ti0_y) - rho * Vpar0 * Btheta2) * BigR * xjac * tstep * theta
                      
                      amat(var_vpar,var_Ti) = amat(var_vpar,var_Ti) -v*amu_neo_prof(ms,mt)*BB2/(Btheta2+epsil)           &
                                * (tauIC * (ps0_x * (r0_x*Ti + r0*Ti_x) + ps0_y*(r0_y*Ti + r0*Ti_y)) &
                                + aki_neo_prof(ms,mt) * tauIC * r0 * (ps0_x*Ti_x + ps0_y*Ti_y)) * BigR * xjac * tstep * theta
                  ...
                  if ( with_TiTe ) then
                    ...
                    amat_k(var_Ti,var_rho) = + TG_num6 * 0.25d0 / BigR * vpar0**2                                                &
                                     * Ti0 * (rho_x * ps0_y - rho_y * ps0_x                    )                      &
                                     * (                            + F0 / BigR * v_p) * xjac * theta * tstep * tstep &
                                + TG_num6 * 0.25d0 / BigR * vpar0**2                                                  &
                                     * rho * (Ti0_x * ps0_y - Ti0_y * ps0_x + F0 / BigR * Ti0_p)                      &
                    ...
                    amat_kn(var_Ti,var_rho) = + TG_num6 * 0.25d0 / BigR * vpar0**2                 &
                                     * Ti0 * (+ F0 / BigR * rho_p)                      &
                                     * (      + F0 / BigR * v_p) * xjac * theta * tstep * tstep
  ...
  !!!!                            -v * Te * (gamma-1.d0) * deta_dT_ohm * (zj0 / BigR)**2.d0 * BigR * xjac * theta * tstep &
                    ...
                    amat_k(var_Te,var_rho) = + TG_num8 * 0.25d0 / BigR * vpar0**2                                                &
                                     * Te0 * (rho_x * ps0_y - rho_y * ps0_x                    )                      &
                                     * (                            + F0 / BigR * v_p) * xjac * theta * tstep * tstep &
                                + TG_num8 * 0.25d0 / BigR * vpar0**2                                                  &
                                     * rho * (Te0_x * ps0_y - Te0_y * ps0_x + F0 / BigR * Te0_p)                      &
                    ...
                    amat_kn(var_Te,var_rho) = + TG_num8 * 0.25d0 / BigR * vpar0**2                 &
                                     * Te0 * (+ F0 / BigR * rho_p)                      &
                                     * (      + F0 / BigR * v_p) * xjac * theta * tstep * tstep
                    ...
                    amat(var_T,var_psi) = - (ZKe_par_T-ZKe_prof) * BigR * BB2_psi / BB2**2 * Bgrad_T_star     * Bgrad_T     * xjac * theta * tstep &
                                + (ZKe_par_T-ZKe_prof) * BigR / BB2              * Bgrad_T_star_psi * Bgrad_T     * xjac * theta * tstep &
                                + (ZKe_par_T-ZKe_prof) * BigR / BB2              * Bgrad_T_star     * Bgrad_T_psi * xjac * theta * tstep &
                    ...
                    amat_k(var_T,var_psi) = - (ZKe_par_T-ZKe_prof) * BigR * BB2_psi / BB2**2 * Bgrad_T_k_star * Bgrad_T     * xjac * theta * tstep &
                                  + (ZKe_par_T-ZKe_prof) * BigR / BB2              * Bgrad_T_k_star * Bgrad_T_psi * xjac * theta * tstep &
                    ...
                    amat_k(var_T,var_rho) = + TG_num6 * 0.25d0 / BigR * vpar0**2                                                &
                                     * T0 * (rho_x * ps0_y - rho_y * ps0_x                    )                      &
                                     * (                            + F0 / BigR * v_p) * xjac * theta * tstep * tstep &
                                + TG_num6 * 0.25d0 / BigR * vpar0**2                                                  &
                                     * rho * (T0_x * ps0_y - T0_y * ps0_x + F0 / BigR * T0_p)                      &
                    ...
                    amat_kn(var_T,var_rho) = + TG_num6 * 0.25d0 / BigR * vpar0**2                 &
                                     * T0 * (+ F0 / BigR * rho_p)                      &
                                     * (      + F0 / BigR * v_p) * xjac * theta * tstep * tstep
                    ...
                    amat(var_T,var_T) = v * r0_corr * T  * BigR * xjac * (1.d0 + zeta)     &
                              ...
                              + (ZKe_par_T-ZKe_prof) * BigR / BB2 * Bgrad_T_star * Bgrad_T_T * xjac * theta * tstep &
                              + ZKe_prof * BigR * ( v_x*T_x + v_y*T_y )                      * xjac * theta * tstep &
  
                              + dZKe_par_dT * T * BigR / BB2 * Bgrad_T_star * Bgrad_T       * xjac * theta * tstep &
                    ...
                    amat_k(var_T,var_T) = + (ZKe_par_T-ZKe_prof) * BigR / BB2 * Bgrad_T_k_star * Bgrad_T_T * xjac * theta * tstep  &
                                  + dZKe_par_dT * T     * BigR / BB2 * Bgrad_T_k_star * Bgrad_T    * xjac * theta * tstep  &
                    ...
                    amat_n(var_T,var_T) = + (ZKe_par_T-ZKe_prof) * BigR / BB2 * Bgrad_T_star   * Bgrad_T_T_n  * xjac * theta * tstep &
                    ...
                    amat_kn(var_T,var_T) = + (ZKe_par_T-ZKe_prof) * BigR / BB2 * Bgrad_T_k_star * Bgrad_T_T_n * xjac * theta * tstep &
                                   + ZKe_prof * BigR   * (v_p*T_p /BigR**2 )                           * xjac * theta * tstep &
