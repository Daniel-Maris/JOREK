...
real*8     :: eta_num_T, eta_R, eta_Z, eta_p
real*8     :: eta_T_T, eta_R_T, eta_Z_T, eta_p_T__p, eta_p_T__n
real*8     :: eta_T_ohm, deta_dT_ohm
real*8     :: Zki_par_T, dZKi_par_dT, Zke_par_T, dZKe_par_dT, Zk_par_T, dZK_par_dT
...
real*8     :: drho0_corr_dn, dTi0_corr_dT, dTe0_corr_dT, dT0_corr_dT
...
real*8     :: rho_imp_bg                                      ! Background impurity density in JOREK unit
...
! for viscous heating
real*8     :: gradU(3,3)
real*8     :: gradU_UR__p(3,3), gradU_UR__n(3,3)
real*8     :: gradU_UZ__p(3,3), gradU_UZ__n(3,3)
real*8     :: gradU_Up__p(3,3), gradU_Up__n(3,3)
      ...
      do mt=1, n_gauss
        ...
        do mp = 1, n_plane
          ...
          if(with_TiTe)then
            ...
            BgradPe   = rho0*BgradT   + T0*BgradRho  ! Note that the same variable name is used as two temperature model
          endif
          ...
          do im=n_tor_start, n_tor_end
            ...
            if(with_TiTe)then
              ...
              gradT_gradVstar__p  = 0.d0
              gradT_gradVstar__k  = 0.d0
            else
              gradTi_gradVstar__p = 0.d0 
              gradTi_gradVstar__k = 0.d0 

              gradTe_gradVstar__p = 0.d0 
              gradTe_gradVstar__k = 0.d0 
            ...
            gradU(1,1:3)  = (/UR0_R, UR0_Z, UR0_p/R-Up0/R /)
            gradU(2,1:3)  = (/UZ0_R, UZ0_Z, UZ0_p/R /)
            gradU(3,1:3)  = (/Up0_R, Up0_Z, Up0_p/R+UR0/R /)

            Qvisc_T     =   visco_T * sum(gradU * gradU)
            ...
            Qvec_p(var_UR)    = + Qconv_UR              &
                                ...
                                + JxB_UR__p             &
                                + visco_T * Qvisc_UR__p &
                                - v * PneoR             &   ! check diamagnetic terms
            ...
            if(with_TiTe)then
              ...
              if(with_impurities)then
                ...
                Qvec_p(var_Te) = Qvec_p(var_Te) &
                               ...
                               - v * rho0 * frad_bg                                                              &
                               + v * (gamma-1.d0) * ( - rhoimp0 * dE_ion_dT * UgradTe - E_ion * UgradRhoimp )    &
              ...
              if(with_impurities)then
                ...
                Qvec_p(var_T) = Qvec_p(var_T) &
                              ...
                              - v * (rho0 + alpha_e*rhoimp0) * rhoimp0 * Lrad                         &
                              - v * rho0 * frad_bg                                                    &
                              + v * (gamma-1.d0) * ( - rhoimp0 * dE_ion_dT * UgradT - E_ion * UgradRhoimp )       &
                              - v * (gamma-1.d0) * rhoimp0 * E_ion * divU &
                              - (gamma-1.d0) * E_ion * D_prof_imp * gradRhoimp_gradVstar__p                     &
                              - (gamma-1.d0) * E_ion * (D_par_imp-D_prof_imp) * BgradVstar__p * BgradRhoimp / BB2
            ...
            do k=1,n_vertex_max
              ...
              do l=1,n_degrees
                ...
                do in =  n_tor_start, n_tor_end
                  ...
                  ! use the same variable names in both mdoels for this term
                  if(with_TiTe)then
                    ...
                    BgradPe_AR__p  = rho0*BgradT_AR__p + T0*BgradRho_AR__p
                    BgradPe_AR__n  = rho0*BgradT_AR__n + T0*BgradRho_AR__n
                    BgradPe_AZ__p  = rho0*BgradT_AZ__p + T0*BgradRho_AZ__p
                    BgradPe_AZ__n  = rho0*BgradT_AZ__n + T0*BgradRho_AZ__n
                    BgradPe_A3     = rho0*BgradT_A3    + T0*BgradRho_A3
                    BgradPe_rho__p = rho*BgradT        + T0*BgradRho_rho__p
                    BgradPe_rho__n =                   + T0*BgradRho_rho__n
                    BgradPe_Te__p  = rho0*BgradT_T__p  + T *BgradRho
                    BgradPe_Te__n  = rho0*BgradT_T__n
                  endif
                  ...
                  Qconv_Up_T__p  =                                           - v * rho0 * ( VdiaGradUp_Ti__p  + VdiaP0_T__p *UR0 / R )
                  Qconv_Up_T__n  =                                           - v * rho0 * ( VdiaGradUp_Ti__n  + VdiaP0_T__n *UR0 / R )
                  ...
                  if(with_TiTe)then
                    ...
                    gradT_gradVstar_T__p  = 0.d0 
                    gradT_gradVstar_T__kn = 0.d0 
                  else
                    gradTi_gradVstar_Ti__p  = 0.d0 
                    gradTi_gradVstar_Ti__kn = 0.d0 

                    gradTe_gradVstar_Te__p  = 0.d0 
                    gradTe_gradVstar_Te__kn = 0.d0 
                  ...
                  gradU_UR__p(1,1:3) = (/UR_R, UR_Z, 0.d0/)
                  gradU_UR__p(2,1:3) = (/0.d0, 0.d0, 0.d0/)
                  gradU_UR__p(3,1:3) = (/0.d0, 0.d0, UR / R /)
                  gradU_UR__n(1,1:3) = (/0.d0, 0.d0, UR_p/R /)

                  gradU_UZ__p(1,1:3) = (/0.d0, 0.d0, 0.d0/)
                  gradU_UZ__p(2,1:3) = (/UZ_R, UZ_Z, 0.d0/)
                  gradU_UZ__p(3,1:3) = (/0.d0, 0.d0, 0.d0/)
                  gradU_UZ__n(2,1:3) = (/0.d0, 0.d0, UZ_p/R /)

                  gradU_Up__p(1,1:3) = (/0.d0, 0.d0, -Up/R /)
                  gradU_Up__p(2,1:3) = (/0.d0, 0.d0, 0.d0/)
                  gradU_Up__p(3,1:3) = (/Up_R, Up_Z, 0.d0/)
                  gradU_Up__n(3,1:3) = (/0.d0, 0.d0, Up_p/R /)

                  Qvisc_T_UR__p   =  visco_T * 2.d0 * sum(gradU * gradU_UR__p)
                  Qvisc_T_UR__n   =  visco_T * 2.d0 * sum(gradU * gradU_UR__n)

                  Qvisc_T_UZ__p   =  visco_T * 2.d0 * sum(gradU * gradU_UZ__p)
                  Qvisc_T_UZ__n   =  visco_T * 2.d0 * sum(gradU * gradU_UZ__n)

                  Qvisc_T_Up__p   =  visco_T * 2.d0 * sum(gradU * gradU_Up__p)
                  Qvisc_T_Up__n   =  visco_T * 2.d0 * sum(gradU * gradU_Up__n)

                  Qvisc_T_T__p    =  dvisco_dT * T * sum(gradU * gradU)
                  ...
                  if(with_TiTe)then
                    ...
                    Qjac_p (var_Ti,var_UR)  = + v * ( - rho0 * UgradTi_UR - Ti0 * UgradRho_UR  -  gamma * pi0 * divU_UR) &
                                              ...
                                              + v * (gamma-1.d0) * 0.5d0 * vv2_UR * particle_source(ms,mt)
                    ...
                    Qjac_p (var_Ti,var_UZ)  = + v * ( - rho0 * UgradTi_UZ - Ti0 * UgradRho_UZ  -  gamma * pi0 * divU_UZ) &
                                              ...
                                              + v * (gamma-1.d0) * 0.5d0 * vv2_UZ * particle_source(ms,mt)
                    ...
                    Qjac_p (var_Ti,var_Up)  = + v * ( - rho0 * UgradTi_Up - Ti0 * UgradRho_Up                             ) &
                                              ...
                                              + v * (gamma-1.d0) *0.5d0 * vv2_Up * particle_source(ms,mt)
                    ...
                    if(with_impurities)then
                      ...
                      Qjac_p (var_Te,var_Te)  = Qjac_p (var_Te,var_Te)                          &
                       ...
                       Qjac_p (var_Te,var_rhoimp)= Qjac_p (var_Te,var_rhoimp)                           &
                                                 ...
                                                 + v * (gamma-1.d0) * deta_drhoimp0_ohm * rhoimp * JJ2  & ! LO: not defined
...
subroutine neutrals_modeling()
  ...
  ! --- Source of neutrals, e.g. from MGI/SPI
  source_neutral = max(source_neutral,0.d0) + source_pellet
...
subroutine impurities_modeling()
  ...
  ne_SI       = (rho0_corr + alpha_e * rhoimp0_corr) * 1.d20 * central_density ! electron density (SI)
  ne_JOREK    = rho0_corr  + alpha_e * rhoimp0_corr ! Electron density in JOREK unit
  ne_JOREK    = max(ne_JOREK,1.d-12)
  ...
  if ( (Z_eff >= 1.d0) .and. (Z_eff <= imp_adas(1)%n_Z) ) then
     ...
     dZ_eff_drhoimp0  = dZ_eff_drhoimp0 - 1.
     do ion_i=1, imp_adas(1)%n_Z
...
subroutine construct_radiation_parameters()
  ...
  if (use_imp_adas .or. with_impurities) then  ! use open adas by default
    ...
    do i_imp =1, n_adas
      ...
      rho_imp_bg = nimp_bg(i_imp) / (1.d20 * central_density)  ! Background impurity density in JU     
      if (ne_SI > ne_SI_min .and. Te_eV > Te_eV_min .and. rho_imp_bg > 0) then
        Lrad_imp_bg = 0.0
        dLrad_imp_bg_dT = 0.0
      ...
      frad_bg = frad_bg + rho_imp_bg * Lrad_imp_bg
      dfrad_bg_dT =  dfrad_bg_dT + rho_imp_bg * dLrad_imp_bg_dT 
