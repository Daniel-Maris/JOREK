...
!!!! WARNING: gmres already defined in phys_module!!! Hence we use phys_module, ONLY...
  ...
  ! Internal parameters
  real*8  :: zbig, zbig_backup,  Ti0, Te0, Vpar0, bigR, dTi0_ds, dTe0_ds, dVpar0_ds, dBigR_ds, psi_1, R_1, Z_1
  real*8  :: R_s, R_t, Z_s, Z_t, ps0_s, ps0_t, ps0_x, ps0_y, direction, xjac
  real*8  :: Btot, alpha, dTi0_dt, dTe0_dt, dVpar0_dt, dBigR_dt, R_inside, Z_inside
  real*8  :: grad_psi, u0_s, u0_t, u0_x, u0_y
  integer :: i, in, iv, inode, k, index_tmp
  integer :: index_large_i, index_node, index_node2, ielm
  integer :: ijA_position,ijA_position2, ilarge2, kv, kTi, kTe, ku, kn, ilarge_vv, ilarge_vT, ilarge_vus, ilarge_vn
...
!======================================= end RMPs ==================================
                       ...
                       if (        apply_psi_BC      &
                         ...
                         if ( (k .eq. 7) .and. (.not. mach_one_bnd_integral) ) then
                            ...
                            dTi0_ds   = node_list%node(inode)%values(1,2,var_Ti)
                            dTe0_ds   = node_list%node(inode)%values(1,2,var_Te)
                            dVpar0_ds = node_list%node(inode)%values(1,2,var_vpar)
                            dBigR_ds  = node_list%node(inode)%x(2,1)

                            ps0_s     = node_list%node(inode)%values(1,2,var_psi)
                            ps0_t     = node_list%node(inode)%values(1,3,var_psi)

                            U0_s      = node_list%node(inode)%values(1,2,var_u)
                            U0_t      = node_list%node(inode)%values(1,3,var_u)
                               ...
                               if (in .eq. 1) then
                                  call boundary_conditions_add_RHS(              &
                                       ...
                                       Zbig * (-Vpar0 + BigR**2 * U0_s/ps0_s +   & 
                                              direction * sqrt(GAMMA*(Ti0+Te0)) / Btot),& 
                                       i_tor_min, i_tor_max)
                            ...
                            call boundary_conditions_add_one_entry(   &
                               ...
                               if (in .eq. 1) then
                                  call boundary_conditions_add_RHS(                  &
                                       ...
                                       GAMMA / sqrt(GAMMA*(Ti0+Te0)) * (dTi0_ds+dTe0_ds) * direction), &
                                       i_tor_min, i_tor_max)
                               else
                                  call boundary_conditions_add_RHS(                  &
                                       index_node2, kv, in,                          &
...
!======================================= end RMPs ==================================
                         ...
                         if ( (k .eq. 7) .and. (.not. mach_one_bnd_integral) ) then
                            ...
                            Vpar0     = node_list%node(inode)%values(1,1,var_vpar)
                            BigR      = node_list%node(inode)%x(1,1)
                            dTi0_dt   = node_list%node(inode)%values(1,3,var_Ti)
                            dTe0_dt   = node_list%node(inode)%values(1,3,var_Te)
                            dVpar0_dt = node_list%node(inode)%values(1,3,var_vpar)
                            ...
                            call boundary_conditions_add_one_entry(   &
                               ...
                               if (in .eq. 1) then
                                  call boundary_conditions_add_RHS(       &
                                       ...
                                       /ps0_t + direction*sqrt(GAMMA*(Ti0+Te0))  &
                                       / Btot),                           &
                                       i_tor_min, i_tor_max)
                               else
                                  call boundary_conditions_add_RHS(       &
