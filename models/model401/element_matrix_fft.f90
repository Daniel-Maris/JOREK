...
integer    :: ij1, ij2, ij3, ij4, ij5, ij6, ij7, kl1, kl2, kl3, kl4, kl5, kl6, kl7
...
real*8     :: eta_T,	visco_T,    deta_dT, d2eta_d2T, dvisco_dT
real*8     :: eta_numm, visco_numm, visco_par_numm, D_perp_numm, K_perp_numm, K_perp_numm2
real*8     :: K_par_num, K_par_num2
real*8     :: T0_ps0_x, T_ps0_x, T0_psi_x, T0_ps0_y, T_ps0_y, T0_psi_y, v_ps0_x, v_psi_x, v_ps0_y, v_psi_y
real*8     :: D_prof, ZK_prof, K_par, dZK_par
...
real*8     :: xjac, xjac_s, xjac_t, xjac_x, xjac_y, BigR,  BigR_x
...
real*8     :: Bgrad_T,   Bgrad_T_star,   Bgrad_T_psi,   Bgrad_T_star_psi,   Bgrad_T_T,     Bgrad_T_T_n,     Bgrad_T_k_star

real*8     :: rhs_ij_1,   rhs_ij_2,   rhs_ij_3,   rhs_ij_4, rhs_ij_5, rhs_ij_6, rhs_ij_7
real*8     :: rhs_ij_5_k, rhs_ij_6_k
real*8     :: amat_11, amat_12, amat_13, amat_16
real*8     :: amat_21, amat_22, amat_23, amat_24, amat_25, amat_26
real*8     :: amat_31, amat_33
real*8     :: amat_42, amat_44
real*8     :: amat_51, amat_52, amat_55, amat_57
real*8     :: amat_61, amat_62, amat_65, amat_66, amat_67
real*8     :: amat_71, amat_72, amat_75, amat_76, amat_77
real*8     :: amat_12_n
real*8     :: amat_23_n
real*8     :: amat_51_k, amat_55_k, amat_55_n, amat_55_kn, amat_57_n
real*8     :: amat_61_k, amat_66_k, amat_66_n,  amat_66_kn,  amat_67_n
real*8     :: amat_75_n, amat_76_n
...
do ms=1, n_gauss
  ...
  do mt=1, n_gauss
    ...
    current_source = 0.d0 ; particle_source = 0.d0 ; heat_source = 0.d0
    ...
    do mp = 1, n_plane
      ...
      do i=1,n_vertex_max
 	  ...
 	  do in=1,n_tor
            ...
            T0       = T0       + nodes(i)%values(in,j,6) * element%size(i,j) * H(i,j,ms,mt)  * HZ(in,mp)
            T0_p     = T0_p     + nodes(i)%values(in,j,6) * element%size(i,j) * H(i,j,ms,mt)  * HZ_p(in,mp)
            T0_s     = T0_s     + nodes(i)%values(in,j,6) * element%size(i,j) * H_s(i,j,ms,mt)* HZ(in,mp)
            T0_t     = T0_t     + nodes(i)%values(in,j,6) * element%size(i,j) * H_t(i,j,ms,mt)* HZ(in,mp)
            T0_ss    = T0_ss    + nodes(i)%values(in,j,6) * element%size(i,j) * H_ss(i,j,ms,mt)*HZ(in,mp)
            T0_tt    = T0_tt    + nodes(i)%values(in,j,6) * element%size(i,j) * H_tt(i,j,ms,mt)*HZ(in,mp)
            T0_st    = T0_st    + nodes(i)%values(in,j,6) * element%size(i,j) * H_st(i,j,ms,mt)*HZ(in,mp)
            T0_pp    = T0_pp    + nodes(i)%values(in,j,6) * element%size(i,j) * H(i,j,ms,mt)  * HZ_pp(in,mp)
            
	    if(in .ne. 1) then
	      T3     = T3       + nodes(i)%values(in,j,6) * element%size(i,j) * H(i,j,ms,mt)  * HZ(in,mp)
              T3_ss  = T3_ss    + nodes(i)%values(in,j,6) * element%size(i,j) * H_ss(i,j,ms,mt)*HZ(in,mp)
              T3_tt  = T3_tt    + nodes(i)%values(in,j,6) * element%size(i,j) * H_tt(i,j,ms,mt)*HZ(in,mp)
              T3_st  = T3_st    + nodes(i)%values(in,j,6) * element%size(i,j) * H_st(i,j,ms,mt)*HZ(in,mp)
	    endif
...
!	endif
	...
	ZK_prof = ZK_prof * 1.d0
      ...
      K_par_num  = 0.d0!1.d-10
      K_par_num2 = 0.d0!1.d-10
!      if ( T0 .lt. T_1 ) then
!        K_par   = ZK_par           * T_1**(2.5d0) 
!        dZK_par = 0.d0
!      else
...
!      endif
      ...
      K_perp_numm    = 0.d0!ZK_perp_num		     ! hyperconductivity
      K_perp_numm2   = ZK_perp_num		     ! hyperconductivity
 	...
 	do j=1,n_order+1
	  ...
	  do im=1,n_tor_loop
	    ...
	    v_xx = (v_ss * y_t**2 - 2.d0*v_st * y_s*y_t + v_tt * y_s**2   &	        
		  + v_s * (y_st*y_t - y_tt*y_s )			  &    
	          + v_t * (y_st*y_s - y_ss*y_t ) )  / xjac**2		  &	    
		  - xjac_x * (v_s * y_t - v_t * y_s) / xjac**2

	    v_yy = (v_ss * x_t**2 - 2.d0*v_st * x_s*x_t + v_tt * x_s**2   &	        
		  + v_s * (x_st*x_t - x_tt*x_s )			  &    
	          + v_t * (x_st*x_s - x_ss*x_t ) )     / xjac**2	  &	    
		  - xjac_y * (- v_s * x_t + v_t * x_s ) / xjac**2

            v_xy = (- v_ss * y_t*x_t - v_tt * x_s*y_s                     &
     	            + v_st * (y_s*x_t  + y_t*x_s  )                       &        
                    - v_s  * (x_st*y_t - x_tt*y_s )			  &	     
	            - v_t  * (x_st*y_s - x_ss*y_t )  )  / xjac**2	  &		  
                    - xjac_x * (- v_s * x_t + v_t * x_s )   / xjac**2	   	
	    ...
	    Bgrad_rho_star   = ( v_x  * ps0_y - v_y  * ps0_x ) / BigR			      ! only the part without	 d/d(phi)
 	    ...
 	    BB2 = (F0*F0 + ps0_x * ps0_x + ps0_y * ps0_y )/BigR**2
    	    ...
    	    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    	    rhs_ij_1 = + v * eta_T  * (zj0 - current_source)/ BigR	   * xjac * tstep &
            ...
            rhs_ij_3 = - ( v_x * ps0_x  + v_y * ps0_y + v*zj0) / BigR * xjac * tstep
    	    ...
    	    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    	    rhs_ij_6 =   v * BigR * heat_source						   * xjac * tstep &
    	    ...
    	    rhs_ij_6_k = - (K_par-ZK_prof) * BigR / BB2 * Bgrad_T_k_star * Bgrad_T  	   * xjac * tstep &
	    ...
	    if (n_tor .gt. 3) then
 	    ...
 	    do k=1,n_vertex_max
 	      ...
 	      do l=1,n_order+1
	          ...
	          if (n_tor .gt. 3) then
	            ...
	            HHZ_pp(in,mp) = HZ_pp(in,mp)
                  ...
                  psi_pp = h(k,l,ms,mt)    * element%size(k,l) * HHZ_pp(in,mp)
                  ...
                  psi_xx = (psi_ss * y_t**2 - 2.d0*psi_st * y_s*y_t + psi_tt * y_s**2   &	        
		          + psi_s * (y_st*y_t - y_tt*y_s )				&      
	                  + psi_t * (y_st*y_s - y_ss*y_t ) )	/ xjac**2		&	    
		          - xjac_x * (psi_s * y_t - psi_t * y_s) / xjac**2

	          psi_yy = (psi_ss * x_t**2 - 2.d0*psi_st * x_s*x_t + psi_tt * x_s**2   &	        
		          + psi_s * (x_st*x_t - x_tt*x_s )				&      
	                  + psi_t * (x_st*x_s - x_ss*x_t ) )	/ xjac**2		&	    
		          - xjac_y * (- psi_s * x_t + psi_t * x_s ) / xjac**2
           
	          psi_xy = (- psi_ss * y_t*x_t - psi_tt * x_s*y_s                       &
     	                    + psi_st * (y_s*x_t  + y_t*x_s  )                           &	     
                            - psi_s  * (x_st*y_t - x_tt*y_s )				&    
	                    - psi_t  * (x_st*y_s - x_ss*y_t )  )  / xjac**2		&	  
                            - xjac_x * (- psi_s * x_t + psi_t * x_s )   / xjac**2
                  ...
                  u_t  = psi_t; zj_t  = psi_t; w_t  = psi_t ; rho_t  = psi_t ; T_t  = psi_t ; Vpar_t  = psi_t
                                               w_ss = psi_ss; rho_ss = psi_ss; T_ss = psi_ss; Vpar_ss = psi_ss
                                               w_tt = psi_tt; rho_tt = psi_tt; T_tt = psi_tt; Vpar_tt = psi_tt
                                               w_st = psi_st; rho_st = psi_st; T_st = psi_st; Vpar_st = psi_st
					                                       T_xx = psi_xx
									       T_yy = psi_yy
									       T_xy = psi_xy
									       T_pp = psi_pp
		  ...
		  if (r0 .lt. rho_1) then
		    ...
		    ELM(ij5,kl7) = ELM(ij5,kl7) + wst * amat_57 + wst * amat_57_n
		  endif
		    ...
		    ELM(ij6,kl7) = ELM(ij6,kl7) + wst * amat_67                   + wst * amat_67_n
		  endif
		    ...
		    ELM(ij7,kl5) = ELM(ij7,kl5) + wst * amat_75                   + wst * amat_75_n
