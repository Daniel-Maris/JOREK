...
subroutine init_live_data_model(file_handle)
  ...
  integer :: i
  real*8  :: psin, FFp, dFFp_dpsi, dens, dn_dpsi, temp, dT_dpsi, S_rho, S_T, S_Ti, S_Te, d_perp, zk_perp, zk_i_perp, zk_e_perp
  ...
  if (with_TiTe) then
    write(file_handle,'(A)') '@input_profiles: %"psin"       "rho"    "drho/dpsin"   "S_rho"     "S_Ti"   "S_Te"   "D_perp"    "ZK_i_perp"     "ZK_e_perp"'
  else
  ...
  do i = 0, 200
    ...
    if (with_TiTe) then
      ...
      write(file_handle,'(a,20es13.4e3)') '@input_profiles: ', psin, dens, dn_dpsi, S_rho, S_Ti, S_Te, d_perp, zk_i_perp, zk_e_perp
    else
