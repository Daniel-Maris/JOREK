...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_1
  ...
  rhs(1) = 											&
	   ...
	   + zeta * v / BigR							* xjac * delta_g(1)&
           ! --- Resistive term
           + v * eta_Te  * (zj0 - current_source)/ BigR				* xjac * tstep  &
           ! --- VxB
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_1
  ...
  amat(1,5)   = amat(1,5)													&
                + v * tau_IC/(r0*BB2) * F0**2/BigR**2 * Te0 * (ps0_x*rho_y - ps0_y*rho_x)		* xjac * theta * tstep	&
  ...
  amat_n(1,5) = amat_n(1,5)													&
                - v * tau_IC/(r0*BB2) * F0**3/BigR**3 * eps_cyl * Te0 * rho_p				* xjac * theta * tstep
  ...
  amat_n(1,8) = amat_n(1,8)													&
                - v * tau_IC/(r0*BB2) * F0**3/BigR**3 * eps_cyl * r0 * Te_p				* xjac * theta * tstep 
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_2
  ...
  rhs(2) = rhs(2)												&
	   ...
	   - tau_IC * BigR**4 * (Pi0_xx       + Pi0_yy)       * (v_x * u0_y - v_y * u0_x)	* xjac * tstep	&
	   - tau_IC * BigR**4 * (r0_x * Pi0_x + r0_y * Pi0_y) * (v_x * u0_y - v_y * u0_x)	* xjac * tstep	&
	   ! --- Neoclassic term
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_2
  ...
  amat(2,2)   = amat(2,2)														&
	        ...
	        + tau_IC * BigR**4 * (Pi0_xx       + Pi0_yy)       * (v_x * u_y - v_y * u_x)			* xjac * theta * tstep	&
	        + tau_IC * BigR**4 * (r0_x * Pi0_x + r0_y * Pi0_y) * (v_x * u_y - v_y * u_x)			* xjac * theta * tstep	&
                - amu_neo_prof * BB2 / (Btheta2+epsil)**2 * (ps0_x*v_x + ps0_y*v_y) * BigR						&
  ...
  amat(2,5)   = amat(2,5)														&
		...
		+ tau_IC * v * BigR**4 * ( u0_xy        * (rho_xx*Ti0 + 2.d0*rho_x*Ti0_x + rho*Ti0_xx  					&
	        ...
	        + tau_IC * BigR**4 * (rho_x * Pi0_x + rho_y * Pi0_y)                * (v_x * u0_y - v_y * u0_x)	* xjac * theta * tstep	&
	        + tau_IC * BigR**4 * ( r0_x * (rho_x*Ti0 + rho*Ti0_x) 									&
  ...
  amat(2,6)   = amat(2,6)														&
		...
		+ tau_IC * v * BigR**4 * ( u0_xy        * (r0_xx*Ti + 2.d0*r0_x*Ti_x + r0*Ti_xx      					&
	        ...
	        + tau_IC * BigR**4 * ( r0_x * (r0_x*Ti0 + r0*Ti0_x)									&
		                     + r0_y * (r0_y*Ti0 + r0*Ti0_y) )         * (v_x * u0_y - v_y * u0_x)	* xjac * theta * tstep	&
                - amu_neo_prof * BB2 / (Btheta2+epsil)**2 * (ps0_x*v_x + ps0_y*v_y) * BigR						&
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_4
  ...
  ! --- The RHS term (diamagnetic part)	      
  !rhs(4) = rhs(4)										&
  !	    - tau_IC / r0 * ( v_x  * Pi0_x + v_y  * Pi0_y)         	* BigR * xjac * tstep	
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_5
  ...
  rhs(5)   =   											&
	     ...
	     - v * r0    * (vpar0_x * ps0_y - vpar0_y * ps0_x)			* xjac * tstep	&
             ! --- Source
             + v * BigR * particle_source					* xjac * tstep	&
             ! --- Diffusivity
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_5
  ...
  amat(5,6)    = amat(5,6)												&
                 - tau_IC * v * 2.d0 * (Ti_y*r0 + Ti*r0_y) * BigR				* xjac * theta * tstep 
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_6
  ...
  rhs(6) =   												&
	     ...
	     - Ki_par_num * (v_ps0_x   * ps0_y - v_ps0_y   * ps0_x)					&
             ...
             - Ki_perp_numm * (v_xx   + v_x  /Bigr + v_yy  )						&
	                    * (Ti0_xx + Ti0_x/Bigr + Ti0_yy) * BigR 			* xjac * tstep  
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_6
  ...
  amat(6,6)    = - v * r0 * BigR**2 * (Ti_x * u0_y  - Ti_y * u0_x)					* xjac * theta * tstep	&
		 ...
		 + Ki_prof * BigR * (v_x*Ti_x + v_y*Ti_y )						* xjac * theta * tstep	& 
                 + Ki_perp_numm * (v_xx  + v_x /BigR + v_yy )									&
		                * (Ti_xx + Ti_x/BigR + Ti_yy) * BigR 					* xjac * theta * tstep	
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_8
  ...
  rhs(8) =   rhs(8) 											&
	     ...
	     - Ke_par_num * (v_ps0_x   * ps0_y - v_ps0_y   * ps0_x)					&
             ...
             - Ke_perp_numm * (v_xx   + v_x  /Bigr + v_yy  )						&
	                    * (Te0_xx + Te0_x/Bigr + Te0_yy) * BigR 			* xjac * tstep  
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_8
  ...
  amat(8,8)    = - v * r0 * BigR**2 * (Te_x * u0_y  - Te_y * u0_x)					* xjac * theta * tstep	&
		 ...
		 + Ke_prof * BigR * (v_x*Te_x + v_y*Te_y )						* xjac * theta * tstep	& 
                 + Ke_perp_numm * (v_xx  + v_x /BigR + v_yy )									&
		                * (Te_xx + Te_x/BigR + Te_yy) * BigR 					* xjac * theta * tstep	
