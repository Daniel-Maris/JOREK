...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_1
  ...
  rhs(1) = rhs(1)                      &
           - v * tau_IC/(r0_corr2*BB2) * F0**2/R**2 * (ps0_x*Pe0_y - ps0_y*Pe0_x)    * xjac * tstep  &       
           + v * tau_IC/(r0_corr2*BB2) * F0**3/R**3 * eps_cyl * Pe0_p                * xjac * tstep  
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_1
 ...
 amat(1,8)   = - deta_dTe * v * Te * (zj0 - current_source - jb) / R          * xjac * theta * tstep  
  ...
  amat(1,1)   = amat(1,1)                          &
                - v * tau_IC/(r0_corr2*BB2**2) * BB2_psi * F0**2/R**2 * (ps0_x*Pe0_y - ps0_y*Pe0_x)      * xjac * theta * tstep  &
                + v * tau_IC/(r0_corr2*BB2**2) * BB2_psi * F0**3/R**3 * eps_cyl * Pe0_p                  * xjac * theta * tstep  &
                + v * tau_IC/(r0_corr2*BB2)              * F0**2/R**2 * (psi_x * Pe0_y - psi_y * Pe0_x)  * xjac * theta * tstep
  
  amat(1,5)   = amat(1,5)                          &
                + v * tau_IC/(r0_corr*BB2)             * F0**2/R**2 * Te0 * (ps0_x*rho_y - ps0_y*rho_x)   * xjac * theta * tstep  &
                + v * tau_IC/(r0_corr*BB2)             * F0**2/R**2 * rho * (ps0_x*Te0_y - ps0_y*Te0_x)   * xjac * theta * tstep  &
                - v * tau_IC/(r0_corr*BB2)             * F0**3/R**3 * eps_cyl * rho * Te0_p               * xjac * theta * tstep  &
                - v * tau_IC * rho /(r0_corr**2 * BB2) * F0**2/R**2 * (ps0_x*Pe0_y - ps0_y*Pe0_x)         * xjac * theta * tstep  &        
                + v * tau_IC * rho /(r0_corr**2 * BB2) * F0**3/R**3 * eps_cyl * Pe0_p                     * xjac * theta * tstep 
  
  amat_n(1,5) = amat_n(1,5)                          &
                - v * tau_IC/(r0_corr*BB2) * F0**3/R**3 * eps_cyl * Te0 * rho_p      * xjac * theta * tstep
  
  amat(1,8)   = amat(1,8)                          &
                + v * tau_IC/(r0_corr*BB2) * F0**2/R**2 * r0 * (ps0_x*Te_y - ps0_y*Te_x)     * xjac * theta * tstep  &
                + v * tau_IC/(r0_corr*BB2) * F0**2/R**2 * Te * (ps0_x*r0_y - ps0_y*r0_x)     * xjac * theta * tstep  &
                - v * tau_IC/(r0_corr*BB2) * F0**3/R**3 * eps_cyl * Te * r0_p                * xjac * theta * tstep 


  amat_n(1,8) = amat_n(1,8)                          &
                - v * tau_IC/(r0_corr*BB2) * F0**3/R**3 * eps_cyl * r0 * Te_p        * xjac * theta * tstep 
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_2
  ...
  rhs(2) = rhs(2)                            &
           ! --- Diamagnetic terms  
       - tau_IC * v * R**4         * (Pi0_x* w0_y - Pi0_y* w0_x)                                        * xjac * tstep  &
       - tau_IC     * R**3 * Pi0_y  * (v_x * u0_x + v_y * u0_y)                                         * xjac * tstep  &
       - tau_IC * v * R**4 * u0_xy * (Pi0_xx-Pi0_yy)                                                    * xjac * tstep  &
       + tau_IC * v * R**4 * Pi0_xy * (u0_xx-u0_yy)                                                     * xjac * tstep  &
           ! --- Diamagnetic viscosity
       + dvisco_dTe * R * W_dia * (v_x*Ti0_x + v_y*Ti0_y)                                              * xjac * tstep  &
       + visco_Te   * R * W_dia * (v_xx + v_x/R + v_yy)                                              * xjac * tstep  &
         ! --- Neoclassic term
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_2
  ...
  real*8 :: Pi0_x_T,   Pi0_xx_T,   Pi0_y_T,   Pi0_yy_T,   Pi0_xy_T
  ...
  Pi0_x_T  = r0_x*Ti + r0*Ti_x
  Pi0_xx_T = r0_xx*Ti + 2.0*r0_x*Ti_x + r0*Ti_xx
  Pi0_y_T  = r0_y*Ti + r0*Ti_y
  Pi0_yy_T = r0_yy*Ti + 2.0*r0_y*Ti_y + r0*Ti_yy
  Pi0_xy_T = r0_xy*Ti + r0*Ti_xy + r0_x*Ti_y + r0_y*Ti_x
 
if (Wdia) then
    W_dia_rho = - tau_IC *     rho/r0_corr2**2 * (Pi0_xx     + Pi0_x    /R + Pi0_yy    ) &
                + tau_IC          /r0_corr2    * (Pi0_xx_rho + Pi0_x_rho/R + Pi0_yy_rho) &
                + tau_IC * 2.0*rho/r0_corr2**3 * (r0_x *Pi0_x     + r0_y *Pi0_y    )    &
                - tau_IC          /r0_corr2**2 * (rho_x*Pi0_x     + rho_y*Pi0_y    )    &
                - tau_IC          /r0_corr2**2 * (r0_x *Pi0_x_rho + r0_y *Pi0_y_rho)
    W_dia_T   = + tau_IC          /r0_corr2    * (Pi0_xx_T + Pi0_x_T/R + Pi0_yy_T) &
                - tau_IC          /r0_corr2**2 * (r0_x  *Pi0_x_T + r0_y  *Pi0_y_T)
  else
    W_dia_rho = 0.d0
    W_dia_T   = 0.d0
  endif
  ...
  amat(2,2)   = amat(2,2)                            &
                ...
                + tau_IC * v * R**4 * u_xy  * (Pi0_xx-Pi0_yy)                                       * xjac * theta * tstep  &
                - tau_IC * v * R**4 * Pi0_xy * (u_xx -u_yy )                                        * xjac * theta * tstep  &
                ! --- Neoclassical term
                - amu_neo_prof * BB2 / (Btheta2+epsil)**2 * (ps0_x*v_x + ps0_y*v_y) * R              &
  ...
  amat(2,4)   = amat(2,4)                                                                                                               &
                ...
                + tau_IC * v * R**4         * (Pi0_x* w_y - Pi0_y* w_x)                                           * xjac * theta * tstep  

  amat(2,5)   = amat(2,5)                            &
                ! --- Diamagnetic terms
                + tau_IC * v * R**4 * Ti0                    * (rho_x * w0_y - rho_y * w0_x)                      * xjac * theta * tstep  &
                + tau_IC * v * R**4 * rho                   * (Ti0_x  * w0_y - Ti0_y  * w0_x)                     * xjac * theta * tstep  &
                + tau_IC     * R**3 * (Ti0_y*rho + Ti0*rho_y) * (v_x   * u0_x + v_y   * u0_y)                     * xjac * theta * tstep  &
                + tau_IC * v * R**4 * u0_xy * (  rho_xx*Ti0 + 2.d0*rho_x*Ti0_x + rho*Ti0_xx            &
                                               - rho_yy*Ti0 - 2.d0*rho_y*Ti0_y - rho*Ti0_yy )                     * xjac * theta * tstep  &                                          
                - tau_IC * v * R**4 * (rho_xy*Ti0 + rho_x*Ti0_y + rho_y*Ti0_x + rho*Ti0_xy) * (u0_xx-u0_yy)       * xjac * theta * tstep  &
                ! --- Diamagnetic viscosity
                - dvisco_dTe * R * W_dia_rho * (v_x*Ti0_x + v_y*Ti0_y)                                            * xjac * theta * tstep  &
                - visco_Te   * R * W_dia_rho * (v_xx + v_x/R + v_yy)                                              * xjac * theta * tstep  &
                ! --- Neoclassical term
                - amu_neo_prof * BB2 / (Btheta2+epsil)**2 * (ps0_x*v_x + ps0_y*v_y) * R              &
  ...
  amat(2,6)   = amat(2,6)                            &
                ! --- Diamagnetic terms
                + tau_IC * v * R**4 * Ti                 * (r0_x * w0_y - r0_y * w0_x)                     * xjac * theta * tstep  &
                + tau_IC * v * R**4 * r0                * (Ti_x  * w0_y - Ti_y  * w0_x)                    * xjac * theta * tstep  &
                + tau_IC     * R**3 * (r0_y*Ti + r0*Ti_y) * (v_x  * u0_x + v_y  * u0_y)                    * xjac * theta * tstep  &
                + tau_IC * v * R**4 * u0_xy * ( r0_xx*Ti + 2.d0*r0_x*Ti_x + r0*Ti_xx              &
                                               -r0_yy*Ti - 2.d0*r0_y*Ti_y - r0*Ti_yy )                     * xjac * theta * tstep  &                                        
                - tau_IC * v * R**4 * (r0_xy*Ti + r0_x*Ti_y + r0_y*Ti_x + r0*Ti_xy) * (u0_xx-u0_yy)        * xjac * theta * tstep  &
                ! --- Diamagnetic viscosity
                - d2visco_dTe2*Te * R * W_dia   * (v_x*Ti0_x + v_y*Ti0_y)                                  * xjac * theta * tstep  &
                - dvisco_dTe*Te   * R * W_dia   * (v_xx + v_x/R + v_yy)                                    * xjac * theta * tstep  &
                - dvisco_dTe      * R * W_dia_T * (v_x*Ti0_x + v_y*Ti0_y)                                  * xjac * theta * tstep  &
                - visco_Te        * R * W_dia_T * (v_xx + v_x/R + v_yy)                                    * xjac * theta * tstep  &
                - dvisco_dTe      * R * W_dia   * (v_x*Ti_x  + v_y*Ti_y )                                  * xjac * theta * tstep  &
  ! --- Neoclassical term
                - amu_neo_prof * BB2 / (Btheta2+epsil)**2 * (ps0_x*v_x + ps0_y*v_y) * R              &
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_4
  ...
  ! --- The RHS term (diamagnetic part)        
  !rhs(4) = rhs(4)                    &
  !     - tau_IC * W_dia / r0_corr2 * ( v_x  * Pi0_x + v_y  * Pi0_y)        * R * xjac 
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_5
  ...
  rhs_k(5) = &
       - (D_par-D_prof) * R / BB2 * Bgrad_rho_k_star * Bgrad_rho  * xjac * tstep  &
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_6
  ...
  rhs(6) =                          &
       ...
       + zeta * v * Ti0_corr * R                                 * xjac *delta_g(5)&
             ! --- Convective terms
       + v * r0 * R**2 * ( Ti0_x * u0_y - Ti0_y * u0_x)          * xjac * tstep  &
       + v * Ti0 * R**2 * ( r0_x * u0_y - r0_y * u0_x)           * xjac * tstep  &
       + v * r0 * GAMMA * Ti0 * u0_y * 2.d0 * R                  * xjac * tstep  &
       - v * r0 *         F0 / R * Vpar0 * Ti0_p                 * xjac * tstep  &
       - v * Ti0 *         F0 / R * Vpar0 * r0_p                 * xjac * tstep  &
       - v * r0 * GAMMA * F0 / R * Ti0    * vpar0_p              * xjac * tstep  &
       - v * r0 *      Vpar0 * (Ti0_x    * ps0_y - Ti0_y    * ps0_x)    * xjac * tstep  &
       - v * Ti0 *      Vpar0 * (r0_x    * ps0_y - r0_y    * ps0_x)     * xjac * tstep  &
       - v * r0 * GAMMA * Ti0 * (vpar0_x * ps0_y - vpar0_y * ps0_x)     * xjac * tstep  &
             ! --- Source
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_6
  ...
  amat(6,1)    = + v         * r0 * Vpar0 * (Ti0_x    * psi_y - Ti0_y    * psi_x)        * xjac * theta * tstep  &
     + v         * Ti0 * Vpar0 * (r0_x    * psi_y - r0_y    * psi_x)                     * xjac * theta * tstep  &
     + v * GAMMA * r0 * Ti0    * (vpar0_x * psi_y - vpar0_y * psi_x)                     * xjac * theta * tstep  &
  ...
  amat(6,2)    = - v * r0 * R**2 * ( Ti0_x * u_y - Ti0_y * u_x)            * xjac * theta * tstep  &
     - v * Ti0 * R**2 * ( r0_x * u_y - r0_y * u_x)                         * xjac * theta * tstep  &
     - v * 2.d0 * GAMMA * r0 * R * Ti0 * u_y                               * xjac * theta * tstep 

  amat(6,5)    = + v * rho * Ti0_corr * R                                  * xjac * (1.d0 + zeta)  &
     - v * rho * R**2    * (Ti0_x  * u0_y  - Ti0_y  * u0_x )               * xjac * theta * tstep  &
     - v * Ti0  * R**2    * (rho_x * u0_y  - rho_y * u0_x )                * xjac * theta * tstep  &
     + v * rho * Vpar0   * (Ti0_x  * ps0_y - Ti0_y  * ps0_x)               * xjac * theta * tstep  &
     + v * Ti0  * Vpar0   * (rho_x * ps0_y - rho_y * ps0_x)                * xjac * theta * tstep  &
     + v * rho * Vpar0   * F0/R * Ti0_p                                    * xjac * theta * tstep  &
     - v * GAMMA * rho * Ti0 * u0_y * 2.d0 * R                             * xjac * theta * tstep  &
     + v * GAMMA * rho * Ti0 * F0/R * Vpar0_p                              * xjac * theta * tstep  &
     + v * GAMMA * rho * Ti0 * (Vpar0_x * ps0_y - Vpar0_y * ps0_x)         * xjac * theta * tstep

  amat_n(6,5)  = + v * Ti0 *                F0 / R * Vpar0 * rho_p         * xjac * theta * tstep
  
  amat(6,6)    = v * r0_corr * Ti * R                                      * xjac * (1.d0 + zeta)  &
     - v * r0  * R**2    * (Ti_x  * u0_y - Ti_y  * u0_x)                   * xjac * theta * tstep  &
     - v * Ti  * R**2    * (r0_x * u0_y - r0_y * u0_x)                     * xjac * theta * tstep  &
     + v * Ti  *             F0 / R * Vpar0 * r0_p                         * xjac * theta * tstep  &
     + v * r0 * Vpar0   * (Ti_x  * ps0_y - Ti_y  * ps0_x)                  * xjac * theta * tstep  &
     + v * Ti * Vpar0   * (r0_x * ps0_y - r0_y * ps0_x)                    * xjac * theta * tstep  &
     - v * r0 * GAMMA * Ti * R * u0_y * 2.d0                               * xjac * theta * tstep  &
     + v * r0 * GAMMA * Ti * F0/R * Vpar0_p                                * xjac * theta * tstep  &
     + v * r0 * GAMMA * Ti * (Vpar0_x * ps0_y - Vpar0_y * ps0_x)           * xjac * theta * tstep  &
  ...
  amat(6,7)    = + v * r0 * F0/R * Vpar * Ti0_p                          * xjac * theta * tstep  &
     + v * Ti0 *                F0 / R * Vpar * r0_p                     * xjac * theta * tstep  &
     + v * r0 * Vpar * (Ti0_x * ps0_y - Ti0_y * ps0_x)                   * xjac * theta * tstep  &
     + v * Ti0 * Vpar * (r0_x * ps0_y - r0_y * ps0_x)                    * xjac * theta * tstep  &
     + v * GAMMA * r0 * Ti0 * (Vpar_x * ps0_y - Vpar_y * ps0_x)          * xjac * theta * tstep 
     
  amat_n(6,7)  = + v * GAMMA * r0 * Ti0 * F0/R * Vpar_p                  * xjac * theta * tstep       
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_7
  ...
  rhs(7) =                           &
       ...
       - v * (P0_x * ps0_y - P0_y * ps0_x)                    * xjac * tstep  &
             ! --- Density source term
       - v * particle_source * vpar0 * BB2 * R                * xjac * tstep  &
             ! --- Numerical viscosity
  ...
  if (normalized_velocity_profile) then
    ...
    rhs(7)   = rhs(7)                                                                                                                &
      - visco_par * v_x * ( vpar0_x * F0**2 / R**2 -2.d0 * vpar0 * F0**2 / R**3 - 2.d0 * PI * F0 * Omega_tor0_x ) * R * xjac * tstep &
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_7
  ...
  amat(7,1)   = + v * (P0_x * psi_y - P0_y * psi_x)                                     * xjac * theta * tstep  &
                ...
                + v * particle_source * vpar0 * BB2_psi * R                             * xjac * theta * tstep  &
  ...
  amat(7,7)   = + v * Vpar * r0_corr * F0**2 / R                             * xjac * (1.d0 + zeta)  &
                ...
                + v * particle_source  * vpar * BB2 * R                      * xjac * theta * tstep  &
                + visco_par_numm * (v_xx    + v_x   /R + v_yy   )                  &
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_8
  ...
  rhs(8) =                           &
             ! --- Time derivative
       + zeta * v * r0_corr * R                        * xjac *delta_g(6)&
             + zeta * v * Te0_corr * R                 * xjac *delta_g(5)&
             ! --- Convective terms
       + v * r0 * R**2 * ( Te0_x * u0_y - Te0_y * u0_x)        * xjac * tstep  &
       + v * Te0 * R**2 * ( r0_x * u0_y - r0_y * u0_x)         * xjac * tstep  &
       + v * r0 * GAMMA * Te0 * u0_y * 2.d0 * R                * xjac * tstep  &
       - v * r0 *         F0 / R * Vpar0 * Te0_p               * xjac * tstep  &
       - v * Te0 *         F0 / R * Vpar0 * r0_p               * xjac * tstep  &
       - v * r0 * GAMMA * F0 / R * Te0    * vpar0_p            * xjac * tstep  &
       - v * r0 *      Vpar0 * (Te0_x    * ps0_y - Te0_y    * ps0_x)   * xjac * tstep  &
       - v * Te0 *      Vpar0 * (r0_x    * ps0_y - r0_y    * ps0_x)    * xjac * tstep  &
       - v * r0 * GAMMA * Te0 * (vpar0_x * ps0_y - vpar0_y * ps0_x)    * xjac * tstep  &
             ! --- Source
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_8
  ...
  amat(8,1)    = + v         * r0 * Vpar0 * (Te0_x    * psi_y - Te0_y    * psi_x)        * xjac * theta * tstep  &
     + v         * Te0 * Vpar0 * (r0_x    * psi_y - r0_y    * psi_x)                     * xjac * theta * tstep  &
     + v * GAMMA * r0 * Te0    * (vpar0_x * psi_y - vpar0_y * psi_x)                     * xjac * theta * tstep  &
  ...
  amat(8,5)    = + v * rho * Te0_corr * R                                            * xjac * (1.d0 + zeta)  &
     ...
     + v * Te0  * Vpar0   * (rho_x * ps0_y - rho_y * ps0_x)                          * xjac * theta * tstep  &
     + v * rho * Vpar0   * F0/R * Te0_p                                              * xjac * theta * tstep  &
     - v * GAMMA * rho * Te0 * u0_y * 2.d0 * R                                       * xjac * theta * tstep  &
     + v * GAMMA * rho * Te0 * F0/R * Vpar0_p                                        * xjac * theta * tstep  &
     + v * GAMMA * rho * Te0 * (Vpar0_x * ps0_y - Vpar0_y * ps0_x)                   * xjac * theta * tstep

  amat_n(8,5)  = + v * Te0 *                F0 / R * Vpar0 * rho_p                   * xjac * theta * tstep
  

  amat(8,7)    = + v * r0 * F0/R * Vpar * Te0_p                           * xjac * theta * tstep  &
     + v * Te0 *                F0 / R * Vpar * r0_p                      * xjac * theta * tstep  &
     + v * r0 * Vpar * (Te0_x * ps0_y - Te0_y * ps0_x)                    * xjac * theta * tstep  &
     + v * Te0 * Vpar * (r0_x * ps0_y - r0_y * ps0_x)                     * xjac * theta * tstep  &
     + v * GAMMA * r0 * Te0 * (Vpar_x * ps0_y - Vpar_y * ps0_x)           * xjac * theta * tstep 
     
  amat_n(8,7)  = + v * GAMMA * r0 * Te0 * F0/R * Vpar_p                   * xjac * theta * tstep       
  
  amat(8,8)    = v * r0_corr * Te * R                                     * xjac * (1.d0 + zeta)  &
     - v * r0 * R**2    * (Te_x  * u0_y - Te_y  * u0_x)                   * xjac * theta * tstep  &
     - v * Te  * R**2    * (r0_x * u0_y - r0_y * u0_x)                    * xjac * theta * tstep  &
     + v * Te  *                F0 / R * Vpar0 * r0_p                     * xjac * theta * tstep  &
     + v * r0 * Vpar0   * (Te_x  * ps0_y - Te_y  * ps0_x)                 * xjac * theta * tstep  &
     + v * Te  * Vpar0   * (r0_x * ps0_y - r0_y * ps0_x)                  * xjac * theta * tstep  &
     - v * r0 * GAMMA * Te * R * u0_y * 2.d0                              * xjac * theta * tstep  &
     + v * r0 * GAMMA * Te * F0/R * Vpar0_p                               * xjac * theta * tstep  &
     + v * r0 * GAMMA * Te * (Vpar0_x * ps0_y - Vpar0_y * ps0_x)          * xjac * theta * tstep  &
