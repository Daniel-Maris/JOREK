...
!--------------------------------------------------------------------------
  ...
  call element_matrix(element, nodes, xpoint2, xcase2, psi_axis, psi_bnd, Z_xpoint, ELM, RHS, tid)
...
integer    :: i, j, ms, mt, mp, k, l, index_ij, index_kl, index, index_k, index_m, m, ik, id, xcase2, n_tor_loop, n_tor_loop2
...
real*8     :: rhs_ij_5_k, rhs_ij_6_k, rhs_ij_8_k
real*8     :: psi_norm
...
real*8     :: amat_12_n
real*8     :: amat_23_n
real*8     :: amat_51_k, amat_55_kn, amat_55_k, amat_55_n, amat_57_k, amat_57_n
real*8     :: amat_61_k, amat_65_k, amat_66_kn, amat_66_k, amat_66_n, amat_67_k, amat_67_n
...
do ms=1, n_gauss
  ...
  do mt=1, n_gauss
    ...
    do mp = 1, n_plane
      ...
      Te0_y  = ( - x_t(ms,mt) * eq_s(mp,8,ms,mt) + x_s(ms,mt) * eq_t(mp,8,ms,mt) ) / xjac
      Te0_p  = eq_p(mp,8,ms,mt)
 	    ...
 	    rhs_ij_2 = - 0.5d0 * vv2 * (v_x * r0_y_hat - v_y * r0_x_hat)	     * xjac * tstep &
 	  	  	 ...
 	  	  	 (    ( 	v_ss  * (x_t(ms,mt)**2+y_t(ms,mt)**2)			    & 
 	  	  	    ...
 	  	  	    * ( 	w0_ss * (x_t(ms,mt)**2+y_t(ms,mt)**2)			    &
 	  	  	       ...
 	  	  	       - 2.d0 * w0_st * (x_s(ms,mt)*x_t(ms,mt) + y_s(ms,mt)*y_t(ms,mt)) ) ) &
	  	  	 / xjac**3  * tstep                                                         &
 	    ...
 	    rhs_ij_5 = v * BigR * particle_source(ms,mt)			       * xjac * tstep &
 	  	  	 ...
 	  	  	 (    ( 	v_ss  * (x_t(ms,mt)**2 + y_t(ms,mt)**2) 		      &
 	  	  	    ...
 	  	  	    * ( 	r0_ss * (x_t(ms,mt)**2 + y_t(ms,mt)**2) 		      &
 	  	  	       ...
 	  	  	       - 2.d0 * r0_st * (x_s(ms,mt)*x_t(ms,mt) + y_s(ms,mt)*y_t(ms,mt)) ) )   &
	  	  	 / xjac**3 * theta*tstep                                                      &
 	    ...
 	    rhs_ij_6 =   v * BigR * heat_source_i(ms,mt)				   * xjac * tstep &
 	  	  	 ...
 	  	  	 (    ( 	v_ss   * (x_t(ms,mt)**2 + y_t(ms,mt)**2)			  &
 	  	  	    ...
 	  	  	    * ( 	Ti0_ss * (x_t(ms,mt)**2 + y_t(ms,mt)**2)			  &
 	  	  	       ...
 	  	  	       - 2.d0 * Ti0_st * (x_s(ms,mt)*x_t(ms,mt) + y_s(ms,mt)*y_t(ms,mt)) ) )	  &
	  	  	 / xjac**3 * theta*tstep                                                          &
 	    ...
 	    rhs_ij_7 = - v * F0 / BigR * P0_p						           * xjac * tstep &
 	  	  	 ...
 	  	  	 (    ( 	v_ss	 * (x_t(ms,mt)**2 + y_t(ms,mt)**2)		                  &
 	  	  	    ...
 	  	  	    * ( 	vpar0_ss * (x_t(ms,mt)**2 + y_t(ms,mt)**2)		                  &
 	  	  	       ...
 	  	  	       - 2.d0 * vpar0_st * (x_s(ms,mt)*x_t(ms,mt) + y_s(ms,mt)*y_t(ms,mt)) ) )            &
	  	  	 / xjac**3 * theta*tstep                                                                  &
 	    ...
 	    rhs_ij_8 =   v * BigR * heat_source_e(ms,mt)				   * xjac * tstep &
 	  	  	 ...
 	  	  	 (    ( 	v_ss   * (x_t(ms,mt)**2 + y_t(ms,mt)**2)			  &
 	  	  	    ...
 	  	  	    * ( 	Te0_ss * (x_t(ms,mt)**2 + y_t(ms,mt)**2)			  &
 	  	  	       ...
 	  	  	       - 2.d0 * Te0_st * (x_s(ms,mt)*x_t(ms,mt) + y_s(ms,mt)*y_t(ms,mt)) ) )	  &
	  	  	 / xjac**3 * theta*tstep                                                          &
 	    ...
 	    do k=1,n_vertex_max
 	      ...
 	      do l=1,n_order+1
	          ...
	          if (n_tor .gt. 3) then
 	    	  ...
 	    	  w_ss    = h_ss(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)
 	    	  w_tt    = h_tt(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)
 	    	  w_st    = h_st(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)
 	  	  	      
 	    	  Vpar_ss = h_ss(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)
 	    	  Vpar_tt = h_tt(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)
 	    	  Vpar_st = h_st(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)

 	    	  rho_ss  = h_ss(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)
 	    	  rho_tt  = h_tt(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)
 	    	  rho_st  = h_st(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)

 	    	  Ti_ss   = h_ss(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)
 	    	  Ti_tt   = h_tt(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)
 	    	  Ti_st   = h_st(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)

 	    	  Te_ss   = h_ss(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)
 	    	  Te_tt   = h_tt(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)
 	    	  Te_st   = h_st(k,l,ms,mt) * element%size(k,l) * HHZ(in,mp)
 	    	  ...
 	    	  amat_24 = r0_hat * BigR**2 * w  * ( v_s * u0_t - v_t * u0_s)  			     * theta * tstep &
 	  	  	  ...
 	  	  	  + visco_numm  *										     &
 	  	  	    (	 (	   v_ss * (x_t(ms,mt)**2 + y_t(ms,mt)**2)					     & 
 	  	  		  ...
 	  	  		  - 2.d0 * w_st * (x_s(ms,mt)*x_t(ms,mt) + y_s(ms,mt)*y_t(ms,mt)) ) )                        &
	                    / xjac**3 * theta*tstep                                                                          &
 	    	  ...
 	    	  amat_55 = v * rho * BigR								   * xjac * (1.d0 + zeta) &
 	  	  	    ...
 	  	  	    + D_perp_num  *											  &
 	  	  	      (	   (	     v_ss   * (x_t(ms,mt)**2 + y_t(ms,mt)**2)						  &
 	  	  		    ...
 	  	  		    - 2.d0 * rho_st * (x_s(ms,mt)*x_t(ms,mt) + y_s(ms,mt)*y_t(ms,mt)) ) ) 			  &
			      / xjac**3 * theta * tstep                                                         		  &
	    	  ...
	    	  ! --- Equation 6 (Ti - Ion energy)
            	  ! --------------------------------
 	    	  amat_61 = + v * r0 * Vpar0 * (Ti0_s * psi_t - Ti0_t * psi_s)  				       * theta * tstep &
 	    	  ...
 	    	  amat_77 = v * Vpar * abs(R0) * F0**2 / BigR					   * xjac * (1.d0 + zeta) &
 	  	  	    ...
 	  	  	    + visco_par_numm  *										  &
 	  	  	      (	   (	    v_ss    * (x_t(ms,mt)**2 + y_t(ms,mt)**2)					  &
 	  	  		    ...
 	  	  		    -2.d0 * Vpar_st * (x_s(ms,mt)*x_t(ms,mt) + y_s(ms,mt)*y_t(ms,mt)) ) ) 		  &
	  	  	      / xjac**3  * theta*tstep									  &
