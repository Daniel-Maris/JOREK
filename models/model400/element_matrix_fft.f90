...
real*8     :: Bgrad_rho_rho_n, Bgrad_Ti_Ti_n, Bgrad_Te_Te_n, Bgrad_rho_k_star, Bgrad_Ti_k_star, Bgrad_Te_k_star
...
real*8     :: amat_12_n
real*8     :: amat_23_n
real*8     :: amat_51_k, amat_55_kn, amat_55_k, amat_55_n, amat_57_n
real*8     :: amat_61_k, amat_66_kn, amat_66_k, amat_66_n, amat_67_n
real*8     :: amat_75_n, amat_76_n, amat_78_n
real*8     :: amat_81_k, amat_81_n, amat_85_n, amat_87_n, amat_88_k, amat_88_n, amat_88_kn
     ...
     do i=1,n_vertex_max
       ...
       do j=1,n_order+1
         ...
         rhs_ij_2 = - 0.5d0 * vv2 * (v_x * r0_y_hat - v_y * r0_x_hat)     * xjac * tstep &
                      ...
                      + visco_num_Te * (v_s * w0_s + v_t * w0_t)                 * tstep &
         ...
         rhs_ij_6 = v * BigR * heat_source_i(ms,mt)                                           * xjac * tstep &
		    ...
		    - kappa_par_num  * ( (v_ss * (x_t(ms,mt)**2+y_t(ms,mt)**2) + v_tt*(x_s(ms,mt)**2+y_s(ms,mt)**2)      &
	            ...
	            + zeta * v * delta_g(mp,6,ms,mt) * BigR
         ...
         rhs_ij_7 = - v * F0 / BigR * P0_p                                      * xjac * tstep &
                    ...
                    + zeta * v * delta_g(mp,7,ms,mt) * BigR                       * xjac  
         ...
         do k=1,n_vertex_max
           ...
           do l=1,n_order+1
             ...
             psi_x  = (   y_t(ms,mt) * h_s(k,l,ms,mt) - y_s(ms,mt) * h_t(k,l,ms,mt) ) / xjac * element%size(k,l)      !!!!!HEYYYYYYYY!!!!! WHY x and not y?????????
	     ...
	     psi_xp = (   y_t(ms,mt) * h_s(k,l,ms,mt) - y_s(ms,mt) * h_t(k,l,ms,mt) ) / xjac * element%size(k,l) 
	     psi_yp = ( - x_t(ms,mt) * h_s(k,l,ms,mt) + x_s(ms,mt) * h_t(k,l,ms,mt) ) / xjac * element%size(k,l) 
	     ...
	     Ti_ss = h_ss(k,l,ms,mt) * element%size(k,l) 
             Ti_tt = h_tt(k,l,ms,mt) * element%size(k,l) 
             Ti_st = h_st(k,l,ms,mt) * element%size(k,l) 
	     
	     Te_ss = h_ss(k,l,ms,mt) * element%size(k,l) 
             Te_tt = h_tt(k,l,ms,mt) * element%size(k,l) 
             Te_st = h_st(k,l,ms,mt) * element%size(k,l) 
...
!------------------------------------------------------------ equation 2
             ...
             amat_24 = r0_hat * BigR**2 * w  * ( v_s * u0_t - v_t * u0_s)  * theta * tstep  &
                     ...
                     - visco_num_Te * (v_s * w_s + v_t * w_t)                      * tstep
...
!------------------------------------------------------------ equation 6
             ...
             Bgrad_Ti_Ti       = ( Ti_x * ps0_y - Ti_y * ps0_x ) / BigR	      ! F0 due to absence of normalisation
             ...
             amat_61_k = - (ZK_i_par-ZK_i_prof) * BigR * BB2_psi / BB2**2 * Bgrad_Ti_k_star * Bgrad_Ti       * xjac * theta * tstep &
                         ...
                         !- dZK_i_prof * psi * BigR / BB2 * Bgrad_Ti_k_star * Bgrad_Ti		             * xjac * theta * tstep 	 
	     ...
	     amat_68 = - v * dQ_dTe * Te * (Te0 - Ti0)      * xjac * theta * tstep & 
...
!---------------------------------------------------------------- equation 8   
			   ...
			   - (ZK_e_par-ZK_e_prof) * BigR * BB2_psi / BB2**2 * Bgrad_Te_star * Bgrad_Te     * xjac * theta * tstep &
                 ...
                 amat_81_n = - (1.5d0) * sigma * v / BigR * Te0_x * psi_xp                                   * xjac * theta * tstep & 
		 ...
		 amat_85_n = - v * sigma * (GAMMA-1.d0) * Te0 / (BigR*r0) * zj0 * rho_p              * xjac * theta * tstep 
		 ...
		 amat_88_n = + (1.5d0) * v * r0 * Vpar0 * F0/BigR * Te_p                          * xjac * theta * tstep &
	     ...
	     ELM_p(mp,ij8,kl8)  =  ELM_p(mp,ij8,kl8)  + wst * amat_88
             ELM_k(mp,ij6,kl8)  =  ELM_k(mp,ij8,kl8)  + wst * amat_88_k
             ELM_n(mp,ij6,kl8)  =  ELM_n(mp,ij8,kl8)  + wst * amat_88_n
             ELM_kn(mp,ij6,kl8) =  ELM_kn(mp,ij8,kl8) + wst * amat_88_kn
