...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_1
  ...
  rhs(1) = 											&
	   ...
	   + zeta * v / R							* xjac * delta_g(1)&
           ! --- Resistive term
           + v * eta_Te  * (zj0 - current_source)/ R				* xjac * tstep  &
           ! --- VxB
  ...
  rhs(1) = rhs(1)										&
           - v * Wdia/BB2 * F0**2/R**2 * (ps0_x*pe0_y - ps0_y*pe0_x)		* xjac * tstep  &		      
           + v * Wdia/BB2 * F0**3/R**3 * eps_cyl * pe0_p			* xjac * tstep  
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_1
  ...
  amat(1,1)   = amat(1,1)													&
                - v * Wdia/BB2**2 * BB2_psi * F0**2/R**2 * (ps0_x*pe0_y - ps0_y*pe0_x)			* xjac * theta * tstep  &
                + v * Wdia/BB2**2 * BB2_psi * F0**3/R**3 * eps_cyl * pe0_p				* xjac * theta * tstep  &
                + v * Wdia/BB2	            * F0**2/R**2 * (psi_x*pe0_y - psi_y*pe0_x)			* xjac * theta * tstep
  
  amat(1,5)   = amat(1,5)													&
                + v * Wdia/BB2 * F0**2/R**2 * Te0 * (ps0_x*rho_y - ps0_y*rho_x)				* xjac * theta * tstep	&
	        + v * Wdia/BB2 * F0**2/R**2 * rho * (ps0_x*Te0_y - ps0_y*Te0_x)				* xjac * theta * tstep	&
	        - v * Wdia/BB2 * F0**3/R**3 * eps_cyl * rho * Te0_p					* xjac * theta * tstep	
  
  amat_n(1,5) = amat_n(1,5)													&
                - v * Wdia/BB2 * F0**3/R**3 * eps_cyl * Te0 * rho_p					* xjac * theta * tstep
  
  amat(1,8)   = amat(1,8)													&
                + v * Wdia/BB2 * F0**2/R**2 * r0 * (ps0_x*Te_y - ps0_y*Te_x) 				* xjac * theta * tstep	&
	        + v * Wdia/BB2 * F0**2/R**2 * Te * (ps0_x*r0_y - ps0_y*r0_x)				* xjac * theta * tstep	&
		- v * Wdia/BB2 * F0**3/R**3 * eps_cyl * Te * r0_p					* xjac * theta * tstep 

  amat_n(1,8) = amat_n(1,8)													&
                - v * Wdia/BB2 * F0**3/R**3 * eps_cyl * r0 * Te_p					* xjac * theta * tstep 
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_2
  ...
  rhs(2) = rhs(2)												&
	   ...
	   - Wdia * R**3 * r0 * u0_y  * Pi0_y * v_y						* xjac * tstep	&
	   - Wdia * R**4 * r0 * u0_xy * Pi0_y * v_y						* xjac * tstep	&
	   - Wdia * R**4 * r0 * u0_xy * Pi0_x * v_x						* xjac * tstep	&
	   ! --- Inverse diamagnetic terms (needed when including diamagnetic vorticity directly into W - equation4)
	   + Wdia * R**4 * r0 * (Pi0_xx + Pi0_x/R + Pi0_yy)    * (v_x * u0_y - v_y * u0_x)	* xjac * tstep	&
	   - Wdia * R**4      * (r0_x * Pi0_x + r0_y * Pi0_y)  * (v_x * u0_y - v_y * u0_x)	* xjac * tstep	&
	   ! --- Neoclassic term
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_2
  ...
  amat(2,2)   = amat(2,2)														&
	        ! --- Main diamagnetic terms
	        + Wdia * R**3 * r0 * u_y  * Pi0_y * v_y								* xjac * theta * tstep	&
	        + Wdia * R**4 * r0 * u_xy * Pi0_y * v_y								* xjac * theta * tstep	&
	        + Wdia * R**4 * r0 * u_xy * Pi0_x * v_x								* xjac * theta * tstep	&
	        ! --- Inverse diamagnetic terms
	        - Wdia * R**4 * r0 * (Pi0_xx + Pi0_x/R + Pi0_yy) * (v_x * u_y - v_y * u_x)			* xjac * theta * tstep	&
	        + Wdia * R**4      * (r0_x * Pi0_x + r0_y * Pi0_y ) * (v_x * u_y - v_y * u_x)			* xjac * theta * tstep	&
	        ! --- Neoclassical term
                - amu_neo_prof * BB2 / (Btheta2+epsil)**2 * (ps0_x*v_x + ps0_y*v_y) * R							&
  ...
  amat(2,5)   = amat(2,5)														&
	        ! --- Main diamagnetic terms
	        + Wdia * R**3 * rho * u0_y  * Pi0_y * v_y							* xjac * theta * tstep	&
	        + Wdia * R**4 * rho * u0_xy * Pi0_y * v_y							* xjac * theta * tstep	&
	        + Wdia * R**4 * rho * u0_xy * Pi0_x * v_x							* xjac * theta * tstep	&
	        + Wdia * R**3 * r0  * u0_y  * (rho_y*Ti0 + rho*Ti0_y) * v_y					* xjac * theta * tstep	&
	        + Wdia * R**4 * r0  * u0_xy * (rho_y*Ti0 + rho*Ti0_y) * v_y					* xjac * theta * tstep	&
	        + Wdia * R**4 * r0  * u0_xy * (rho_x*Ti0 + rho*Ti0_x) * v_x					* xjac * theta * tstep	&
	        ! --- Inverse diamagnetic terms
	        - Wdia * R**4 * rho * ( pi0_xx	+ pi0_yy + pi0_x/R )                * (v_x * u0_y - v_y * u0_x)	* xjac * theta * tstep	&
	        - Wdia * R**4 * r0 * ( rho_xx*Ti0 + rho*Ti0_xx + 2.d0*rho_x*Ti0_x							&
		                     + rho_yy*Ti0 + rho*Ti0_yy + 2.d0*rho_y*Ti0_y							&
		                     + rho_x*Ti0/R + rho*Ti0_x/R	          ) * (v_x * u0_y - v_y * u0_x)	* xjac * theta * tstep	&
	        + Wdia * R**4    * (rho_x * Pi0_x + rho_y * Pi0_y)                  * (v_x * u0_y - v_y * u0_x)	* xjac * theta * tstep	&
	        + Wdia * R**4    * ( r0_x * (rho_x*Ti0 + rho*Ti0_x) 									&
  ...
  amat(2,6)   = amat(2,6)														&
	        ! --- Main diamagnetic terms
	        + Wdia * R**3 * r0  * u0_y  * (r0_y*Ti + r0*Ti_y) * v_y						* xjac * theta * tstep	&
	        + Wdia * R**4 * r0  * u0_xy * (r0_y*Ti + r0*Ti_y) * v_y						* xjac * theta * tstep	&
	        + Wdia * R**4 * r0  * u0_xy * (r0_x*Ti + r0*Ti_x) * v_x						* xjac * theta * tstep	&
	        ! --- Inverse diamagnetic terms
	        - Wdia * R**4 * r0 * ( r0_xx*Ti + r0*Ti_xx + 2.d0*r0_x*Ti_x       							&
		                     + r0_x*Ti/R + r0*Ti_x/R										&
		                     + r0_yy*Ti + r0*Ti_yy + 2.d0*r0_y*Ti_y ) * (v_x * u0_y - v_y * u0_x)	* xjac * theta * tstep	&
	        + Wdia * R**4      * ( r0_x * (r0_x*Ti + r0*Ti_x)									&
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_4
  ...
  ! --- The RHS term (diamagnetic part)	      
  !rhs(4) = rhs(4)									&
  !	    - Wdia * ( v_x  * Pi0_x + v_y  * Pi0_y) * R 		* xjac * tstep	
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_4
  ...
  ! --- The LHS terms (diamagnetic part)
  amat(4,5) = amat(4,5)									        			&
              + Wdia                        * (v_x   * rho  *Ti0_x + v_y   * rho  *Ti0_y) * R  		* xjac * tstep	&
              + Wdia                        * (v_x   * rho_x*Ti0   + v_y   * rho_y*Ti0  ) * R  		* xjac * tstep	

  amat(4,6) = amat(4,6)								        	    			&
              + Wdia                        * (v_x   * r0   *Ti_x  + v_y   * r0   *Ti_y ) * R    	* xjac * tstep	&
              + Wdia                        * (v_x   * r0_x *Ti    + v_y   * r0_y *Ti   ) * R    	* xjac * tstep	
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_rhs_5
  ...
  ! --- The RHS term (diamagnetic part)	      
  rhs(5)   = rhs(5)									 	&
	     + Wdia * v * R**2 * ( r0_x * pi0_y - r0_y * pi0_x)			* xjac * tstep	&
	     + Wdia * v * R    * r0 * 2.d0 * pi0_y				* xjac * tstep	
...
!DEC$ ATTRIBUTES FORCEINLINE :: ELM_main_lhs_5
  ...
  ! --- The LHS terms (diamagnetic part)
  amat(5,5)    = amat(5,5)												&
	         - Wdia * v * R**2 * ( rho_x * pi0_y     - rho_y * pi0_x     )			* xjac * theta * tstep	&
	         - Wdia * v * R**2 * ( r0_x  * rho_y*Ti0 - r0_y  * rho_x*Ti0 )			* xjac * theta * tstep	&
	         - Wdia * v * R**2 * ( r0_x  * rho*Ti0_y - r0_y  * rho*Ti0_x )			* xjac * theta * tstep	&
	         - Wdia * v * R    * rho * 2.d0 * pi0_y						* xjac * theta * tstep	&
	         - Wdia * v * R    * r0  * 2.d0 * (rho_y*Ti0 + rho*Ti0_y)			* xjac * theta * tstep	
  
  amat(5,6)    = amat(5,6)												&
	         - Wdia * v * R**2 * ( r0_x  * r0_y*Ti - r0_y  * r0_x*Ti )			* xjac * theta * tstep	&
	         - Wdia * v * R**2 * ( r0_x  * r0*Ti_y - r0_y  * r0*Ti_x )			* xjac * theta * tstep	&
	         - Wdia * v * R    * r0  * 2.d0 * (r0_y*Ti + r0*Ti_y)				* xjac * theta * tstep	
