...
contains
  ...
  subroutine boundary_conditions( my_id, node_list, element_list, bnd_node_list, local_elms,    & 
       ...
       &                          xcase2, R_axis, Z_axis, psi_axis, psi_bnd, R_xpoint, Z_xpoint, psi_xpoint, a_mat)
              ...
              if ((node_list%node(inode)%boundary == 1) .or. (node_list%node(inode)%boundary == 3)) then
                ...
                if ((k .eq. var_AR) .or. (k .eq. var_AZ) .or. (k .eq. var_A3) .or. (no_mach1_bc)) then
                  ...
                  if ((index_node .ge. index_min) .and. (index_node .le. index_max)) then
                    ...
                    ilarge2 = ijA_position - 1 + ((k-1)*n_tor_local +in-a_mat%i_tor_min) * n_var*n_tor_local + (k-1)*n_tor_local + in - a_mat%i_tor_min +1
                ...
                if ( (.not. Mach1_openBC) .and. (.not. no_mach1_bc) ) then 
                  if ( (k == var_uR) .or. (k == var_uZ) .or. (k == var_up) ) then 
                    ...
                    if ((index_node .ge. index_min) .and. (index_node .le. index_max)) then
                      ...
                      ilarge_v (k)      = ijA_position  - 1 + ((k-1)*n_tor_local +in-a_mat%i_tor_min) * n_var*n_tor_local + (k     -1)*n_tor_local + in - a_mat%i_tor_min +1
                      ilarge_v (var_Ti) = ijA_position  - 1 + ((k-1)*n_tor_local +in-a_mat%i_tor_min) * n_var*n_tor_local + (var_Ti-1)*n_tor_local + in - a_mat%i_tor_min +1
                      ilarge_v (var_Te) = ijA_position  - 1 + ((k-1)*n_tor_local +in-a_mat%i_tor_min) * n_var*n_tor_local + (var_Te-1)*n_tor_local + in - a_mat%i_tor_min +1

                      a_mat%irn(ilarge_v(k))   = n_tor_local * n_var * (index_node -1) + (k-1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%jcn(ilarge_v(k))   = n_tor_local * n_var * (index_node -1) + (k-1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%val(ilarge_v(k)) = Zbig * Mach1_U

                      a_mat%irn(ilarge_v(var_Ti))  = n_tor_local * n_var * (index_node -1) + (k     -1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%jcn(ilarge_v(var_Ti))  = n_tor_local * n_var * (index_node -1) + (var_Ti-1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%val(ilarge_v(var_Ti))= Zbig * Mach1_Ti

                      a_mat%irn(ilarge_v(var_Te))  = n_tor_local * n_var * (index_node -1) + (k     -1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%jcn(ilarge_v(var_Te))  = n_tor_local * n_var * (index_node -1) + (var_Te-1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%val(ilarge_v(var_Te))= Zbig * Mach1_Te

                      RHS_loc(n_tor_local*n_var * (index_node-1) + (k-1)*n_tor_local + in - a_mat%i_tor_min +1) = 0.d0
                      if (in .eq. 1) then
                         RHS_loc(n_tor_local*n_var * (index_node-1) + (k-1)*n_tor_local + in - a_mat%i_tor_min +1) = - Zbig * Mach1
                      endif
                    ...
                    if ((index_node2 .ge. index_min) .and. (index_node2 .le. index_max)) then
                      ...
                      ilarge_vs(k)      = ijA_position2 - 1 + ((k-1)*n_tor_local +in-a_mat%i_tor_min) * n_var*n_tor_local + (k     -1)*n_tor_local + in - a_mat%i_tor_min +1
                      ilarge_v (var_Ti) = ijA_position  - 1 + ((k-1)*n_tor_local +in-a_mat%i_tor_min) * n_var*n_tor_local + (var_Ti-1)*n_tor_local + in - a_mat%i_tor_min +1
                      ilarge_vs(var_Ti) = ijA_position2 - 1 + ((k-1)*n_tor_local +in-a_mat%i_tor_min) * n_var*n_tor_local + (var_Ti-1)*n_tor_local + in - a_mat%i_tor_min +1
                      ilarge_v (var_Te) = ijA_position  - 1 + ((k-1)*n_tor_local +in-a_mat%i_tor_min) * n_var*n_tor_local + (var_Te-1)*n_tor_local + in - a_mat%i_tor_min +1
                      ilarge_vs(var_Te) = ijA_position2 - 1 + ((k-1)*n_tor_local +in-a_mat%i_tor_min) * n_var*n_tor_local + (var_Te-1)*n_tor_local + in - a_mat%i_tor_min +1

                      a_mat%irn(ilarge_vs(k))   = n_tor_local * n_var * (index_node2-1) + (k-1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%jcn(ilarge_vs(k))   = n_tor_local * n_var * (index_node2-1) + (k-1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%val(ilarge_vs(k)) = Zbig * Mach1_s_Us

                      a_mat%irn(ilarge_v(var_Ti))  = n_tor_local * n_var * (index_node2-1) + (k     -1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%jcn(ilarge_v(var_Ti))  = n_tor_local * n_var * (index_node -1) + (var_Ti-1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%val(ilarge_v(var_Ti))= Zbig * Mach1_s_Ti

                      a_mat%irn(ilarge_vs(var_Ti))   = n_tor_local * n_var * (index_node2-1) + (k     -1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%jcn(ilarge_vs(var_Ti))   = n_tor_local * n_var * (index_node2-1) + (var_Ti-1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%val(ilarge_vs(var_Ti)) = Zbig * Mach1_s_Tis

                      a_mat%irn(ilarge_v(var_Te))  = n_tor_local * n_var * (index_node2-1) + (k     -1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%jcn(ilarge_v(var_Te))  = n_tor_local * n_var * (index_node -1) + (var_Te-1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%val(ilarge_v(var_Te))= Zbig * Mach1_s_Te

                      a_mat%irn(ilarge_vs(var_Te))   = n_tor_local * n_var * (index_node2-1) + (k     -1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%jcn(ilarge_vs(var_Te))   = n_tor_local * n_var * (index_node2-1) + (var_Te-1)*n_tor_local + in - a_mat%i_tor_min +1
                      a_mat%val(ilarge_vs(var_Te)) = Zbig * Mach1_s_Tes

                      Rhs_loc(n_tor_local * n_var * (index_node2-1) + (k-1)*n_tor_local + in - a_mat%i_tor_min +1) = 0.d0
                      if (in .eq. 1) then
                         Rhs_loc(n_tor_local * n_var * (index_node2-1) + (k-1)*n_tor_local + in - a_mat%i_tor_min +1) = - Zbig * Mach1_s
                      endif
