 ...
 !$OMP SIMD collapse(2)
    ...
    do ms=1, n_gauss
      do mt=1, n_gauss
        ...
        psi_n0(ms,mt) = psi_n0(ms,mt) + nodes(i)%values(1,j,var_A3) * element%size(i,j) * H(i,j,ms,mt)  * HZ(1,1)
      end do
    end do
...
do ms=1, n_gauss
  do mt=1, n_gauss
    if (keep_current_prof) then
      call current(xpoint2, xcase2, x_g(ms,mt),y_g(ms,mt), Z_xpoint, psi_n0(ms,mt),psi_axis,psi_bnd,current_source(ms,mt))
      ! --- Historically JOREK uses a negative current, so we need to reverse it.
      current_source(ms,mt) = - current_source(ms,mt)
    endif
    call sources(xpoint2, xcase2, y_g(ms,mt)           , Z_xpoint, psi_n0(ms,mt),psi_axis,psi_bnd,particle_source(ms,mt),heat_source(ms,mt))
  enddo
enddo
...
!$OMP  amat, Pjac, Qjac_p, Qjac_k, Qjac_n, Qjac_kn)
        ...
        do mp = 1, n_plane
          ...
          ! --- psi_norm
          psi_norm = get_psi_n(psi_n0(ms,mt), y_g(ms,mt))
