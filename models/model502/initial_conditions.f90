...
if (my_id .eq. 0) then
  ...
  do i=1,node_list%n_nodes
    ...
    node_list%node(i)%values(1,1,8) = 0.
    node_list%node(i)%values(1,2,8) = 0.
    node_list%node(i)%values(1,3,8) = 0.
    node_list%node(i)%values(1,4,8) = 0.
    ...
    node_list%node(i)%values(1,1,9) = zTe
    node_list%node(i)%values(1,2,9) = dTe_dpsi  * node_list%node(i)%values(1,2,1) + dTe_dz * node_list%node(i)%x(2,2)
    node_list%node(i)%values(1,3,9) = dTe_dpsi  * node_list%node(i)%values(1,3,1) + dTe_dz * node_list%node(i)%x(3,2)
    node_list%node(i)%values(1,4,9) = dTe_dpsi  * node_list%node(i)%values(1,4,1) + dTe_dz * node_list%node(i)%x(4,2) &
...
if (tauIC .ne. 0.d0) then
  call Poisson(my_id,2,node_list,element_list,bnd_node_list,bnd_elm_list, &
               2,4,1, ES%psi_axis,ES%psi_bnd,xpoint2, xcase2,ES%Z_xpoint,freeboundary,refinement,1)      ! inverse Poisson
endif
...
do in=2,n_tor
  ...
  if (my_id .eq. 0) then
    ...
    do i=1,node_list%n_nodes
      ...
      psi = node_list%node(i)%values(1,1,1)
      Z   = node_list%node(i)%x(1,2)
      psi_n = (psi - ES%psi_axis)/(psi_bnd - ES%psi_axis)
  ...
  call Poisson(my_id,1,node_list,element_list,bnd_node_list,bnd_elm_list, &
               4,2,1, ES%psi_axis,ES%psi_bnd,xpoint2, xcase2,ES%Z_xpoint,freeboundary,refinement,1)
enddo
