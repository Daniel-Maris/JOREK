...
real*8,  intent(out) :: dTe_dpsi,  dTe_dz                                             ! 1st order derivatives
real*8,  intent(out) :: dTe_dpsi2, dTe_dz2, dTe_dpsi_dz                               ! 2nd order derivatives
real*8,  intent(out) :: dTe_dpsi3, dTe_dpsi_dz2, dTe_dpsi2_dz,  dTe_dz3               ! 2rd order derivatives
real*8,  intent(out) :: dTe_dpsi4, dTe_dpsi_dz3, dTe_dpsi2_dz2, dTe_dpsi3_dz, dTe_dz4 ! 4th order derivatives
...
if ( .not. num_Te ) then ! use analytical representation
  ...
  dprof0_dpsi  = (Te_0-Te_1)*(Te_coef(1) + 2.d0 * Te_coef(2) * psi_n + 3.d0 * Te_coef(3) * psi_n**2)     / delta_psi
  dprof0_dpsi2 = (Te_0-Te_1)*(2.d0 * Te_coef(2) + 6.d0 * Te_coef(3) * psi_n)                             / delta_psi**2
  dprof0_dpsi3 = (Te_0-Te_1)*(6.d0 * Te_coef(3))                                                         / delta_psi**3
  dprof0_dpsi4 = 0.d0
...
if (xpoint2) then
  ...
  temperature_e_profile = prof1     *	 atn_z * atn_z_u

  dTe_dpsi         = dprof1_dpsi  *    atn_z * atn_z_u
  dTe_dpsi2        = dprof1_dpsi2 *    atn_z * atn_z_u
  dTe_dpsi3        = dprof1_dpsi3 *    atn_z * atn_z_u  
  dTe_dpsi4        = dprof1_dpsi4 *    atn_z * atn_z_u  

  dTe_dz           = prof1        * ( datn_z * atn_z_u +          atn_z * datn_z_u)
  dTe_dz2          = prof1        * (d2atn_z * atn_z_u + 2.d0 *  datn_z * datn_z_u        +   atn_z * d2atn_z_u)  
  dTe_dz3          = prof1        * (d3atn_z * atn_z_u + 4.d0 * d2atn_z * datn_z_u + 4.d0 *  datn_z * d2atn_z_u  + atn_z * d3atn_z_u)  
  dTe_dz4          = prof1        * (d4atn_z * atn_z_u + 4.d0 * d3atn_z * datn_z_u + 6.d0 * d2atn_z * d2atn_z_u &
                                     + 4.d0 * datn_z * d3atn_z_u + atn_z * d4atn_z_u)

  dTe_dpsi_dz      = dprof1_dpsi  * ( datn_z * atn_z_u +         atn_z * datn_z_u)
  dTe_dpsi_dz2     = dprof1_dpsi  * (d2atn_z * atn_z_u + 2.d0 * datn_z * datn_z_u  + atn_z * d2atn_z_u)  
  dTe_dpsi_dz3     = dprof1_dpsi  * (d3atn_z * atn_z_u + 4.d0 * d2atn_z * datn_z_u + 4.d0 *  datn_z * d2atn_z_u  + atn_z * d3atn_z_u)

  dTe_dpsi2_dz     = dprof1_dpsi2 * ( datn_z * atn_z_u +         atn_z * datn_z_u)
  dTe_dpsi2_dz2    = dprof1_dpsi2 * (d2atn_z * atn_z_u + 2.d0 * datn_z * datn_z_u  + atn_z * d2atn_z_u)

  dTe_dpsi3_dz     = dprof1_dpsi3 * ( datn_z * atn_z_u +         atn_z * datn_z_u)

else
  
  temperature_e_profile = prof1
  dTe_dpsi              = dprof1_dpsi
  dTe_dpsi2             = dprof1_dpsi2
  dTe_dpsi3             = dprof1_dpsi3
  dTe_dpsi4             = dprof1_dpsi4
  dTe_dz                = 0.d0
  dTe_dz2               = 0.d0
  dTe_dz3               = 0.d0
  dTe_dz4               = 0.d0
...
if (freeboundary_equil .and. num_Te) then                       !if the temperature profile is given in a file and there is freeboundary equilibrium
  ...
  dTe_dpsi4             = dTe_dpsi4             * current_FB_fact
  dTe_dz                = dTe_dz                * current_FB_fact
  dTe_dz2               = dTe_dz2               * current_FB_fact
  dTe_dz3               = dTe_dz3               * current_FB_fact
  dTe_dz4               = dTe_dz4               * current_FB_fact
  dTe_dpsi_dz           = dTe_dpsi_dz           * current_FB_fact
  dTe_dpsi_dz2          = dTe_dpsi_dz2          * current_FB_fact
  dTe_dpsi_dz3          = dTe_dpsi_dz3          * current_FB_fact
  dTe_dpsi2_dz          = dTe_dpsi2_dz          * current_FB_fact
  dTe_dpsi2_dz2         = dTe_dpsi2_dz2         * current_FB_fact
  dTe_dpsi3_dz          = dTe_dpsi3_dz          * current_FB_fact
