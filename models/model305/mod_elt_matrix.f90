...
#ifdef altcs
       psieq(ms,mt)    = psieq(ms,mt)    + nodes(i)%psi_eq(j)   *esize(i,j) * BasFun  (i,j,ms,mt)
       psieq_s(ms,mt)  = psieq_s(ms,mt)  + nodes(i)%psi_eq(j)   *esize(i,j) * BasFun_s(i,j,ms,mt)
       psieq_t(ms,mt)  = psieq_t(ms,mt)  + nodes(i)%psi_eq(j)   *esize(i,j) * BasFun_t(i,j,ms,mt)
#endif
       do mp=1,n_plane
     ...
     do i=1,n_vertex_max
       ...
       do j=1,n_order+1
         ...
         do im=i_tor_min, i_tor_max
           ...
           v   =  BasFun(i,j,ms,mt) * esize(i,j) * HZ(im,mp)
           v_x = (  y_t(ms,mt) * BasFun_s(i,j,ms,mt) - y_s(ms,mt) * BasFun_t(i,j,ms,mt) ) * esize(i,j) / xjac * HZ(im,mp)
           v_y = (- x_t(ms,mt) * BasFun_s(i,j,ms,mt) + x_s(ms,mt) * BasFun_t(i,j,ms,mt) ) * esize(i,j) / xjac * HZ(im,mp)

           v_s = BasFun_s(i,j,ms,mt) * esize(i,j) * HZ(im,mp)
           v_t = BasFun_t(i,j,ms,mt) * esize(i,j) * HZ(im,mp)
           v_p = BasFun  (i,j,ms,mt) * esize(i,j) * HZ_p(im,mp)

           v_ss = BasFun_ss(i,j,ms,mt) * esize(i,j) * HZ(im,mp)
           v_tt = BasFun_tt(i,j,ms,mt) * esize(i,j) * HZ(im,mp)
           v_st = BasFun_st(i,j,ms,mt) * esize(i,j) * HZ(im,mp)
           ...
           do k=1,n_vertex_max
             ...
             do l=1,n_order+1
               ...
               do in = i_tor_min, i_tor_max
                 ...
                 ! change basis function for axis elements
                 psi   = BasFun(k,l,ms,mt) * esize(k,l) * HZ(in,mp)

                 psi_x = (   y_t(ms,mt) * BasFun_s(k,l,ms,mt) - y_s(ms,mt) * BasFun_t(k,l,ms,mt) ) / xjac * esize(k,l) * HZ(in,mp)
                 psi_y = ( - x_t(ms,mt) * BasFun_s(k,l,ms,mt) + x_s(ms,mt) * BasFun_t(k,l,ms,mt) ) / xjac * esize(k,l) * HZ(in,mp)

                 psi_p  = BasFun   (k,l,ms,mt) * esize(k,l) * HZ_p(in,mp)
                 psi_s  = BasFun_s (k,l,ms,mt) * esize(k,l) * HZ(in,mp)
                 psi_t  = BasFun_t (k,l,ms,mt) * esize(k,l) * HZ(in,mp)
                 psi_ss = BasFun_ss(k,l,ms,mt) * esize(k,l) * HZ(in,mp)
                 psi_tt = BasFun_tt(k,l,ms,mt) * esize(k,l) * HZ(in,mp)
                 psi_st = BasFun_st(k,l,ms,mt) * esize(k,l) * HZ(in,mp)
