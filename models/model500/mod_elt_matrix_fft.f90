         ...
         !--------------------------------------------------------
          ...
          do im=n_tor_start, n_tor_end
            ...
            rhs_ij(1) = v * eta_T  * (zj0 - current_source(ms,mt) - Jb)/ BigR * xjac * tstep * factor(1,1) &
                      + v * (ps0_s * u0_t - ps0_t * u0_s)                            * tstep * factor(1,2) &
                      - v * F0 / BigR  * u0_p                                 * xjac * tstep * factor(1,3) &
                      + eta_num_T * (v_x * zj0_x + v_y * zj0_y)               * xjac * tstep * factor(1,4) &

                      - v * tauIC/(r0_corr*BB2) * F0**2/BigR**2 * (ps0_s * p0_t - ps0_t * p0_s) * tstep * factor(1,5) &
                      + v * tauIC/(r0_corr*BB2) * F0**3/BigR**3 * p0_p        * xjac * tstep * factor(1,5) &

                      + zeta * v * delta_g(mp,1,ms,mt) / BigR                 * xjac* factor(1,6) 
            ...
            rhs_ij(2) =  - 0.5d0 * vv2 * (v_x * r0_y_hat - v_y * r0_x_hat)   * xjac * tstep * factor(2,1) &
                         - r0_hat * BigR**2 * w0 * (v_s * u0_t - v_t * u0_s)        * tstep * factor(2,2) &
                         + v * (ps0_s * zj0_t - ps0_t * zj0_s )                     * tstep * factor(2,3) &
                         - visco_T * BigR * (v_x * w0_x + v_y * w0_y)        * xjac * tstep * factor(2,4) &
                         - v * F0 / BigR * zj0_p                             * xjac * tstep * factor(2,5) &
                         + BigR**2 * (v_s * p0_t - v_t * p0_s)                      * tstep * factor(2,6) &

                         - visco_num_T * (v_xx + v_x/Bigr + v_yy)*(w0_xx + w0_x/Bigr + w0_yy) * xjac * tstep * factor(2,7) &

                         - TG_num2 * 0.25d0 * r0_hat * BigR**3 * (w0_x * u0_y - w0_y * u0_x)  &
                                   * ( v_x * u0_y - v_y * u0_x) * xjac * tstep * tstep * factor(2,8) &

                         - v * tauIC * BigR**4 * (p0_s * w0_t - p0_t * w0_s)        * tstep * factor(2,9) &

                         - tauIC * BigR**3 * p0_y * (v_x* u0_x + v_y * u0_y) * xjac * tstep * factor(2,10) &

                         - v * tauIC * BigR**4 * (u0_xy * (p0_xx - p0_yy) - p0_xy * (u0_xx - u0_yy) ) * xjac * tstep * factor(2,11) &

                         ! --- Diamagnetic viscosity
                         + dvisco_dT * bigR * W_dia * (v_x*T0_x + v_y*T0_y)    * xjac * tstep  * factor(2,12) &
                         + visco_T   * bigR * W_dia * (v_xx + v_x/bigR + v_yy) * xjac * tstep  * factor(2,12) &

                         + BigR**3 * (particle_source(ms,mt) + source_pellet) * (v_x * u0_x + v_y * u0_y) * xjac* tstep * factor(2,13) &
                     
                         + (1.d0 - delta_n_convection) * (   &
                               + BigR**3*(r0_corr*rn0_corr*Sion_T)*(v_x * u0_x + v_y * u0_y)  * xjac * tstep * factor(2,14) &
                               - BigR**3*(r0_corr*r0_corr *Srec_T)*(v_x * u0_x + v_y * u0_y)  * xjac * tstep * factor(2,15) &
                           )  &

                         - zeta * BigR * r0_hat * (v_x * delta_u_x + v_y * delta_u_y) * xjac* factor(2,16) 
            
            if (NEO) then
              rhs_ij(2) = rhs_ij(2)  + amu_neo_prof(ms,mt)*BB2/((Btheta2+epsil)**2)   * factor(2,17)        &
                        * (ps0_x * v_x + ps0_y * v_y) *                                      &  
            ...
            rhs_ij(5)  = v * BigR * (particle_source(ms,mt) + source_pellet)                      * xjac * tstep * factor(5,1) &
                       + v * BigR**2 * ( r0_s * u0_t - r0_t * u0_s)                                      * tstep * factor(5,2) &
                       + v * 2.d0 * BigR * r0 * u0_y                                              * xjac * tstep * factor(5,3) &
                       - (D_par-D_prof) * BigR / BB2 * Bgrad_rho_star * Bgrad_rho                 * xjac * tstep * factor(5,4) &
                       - D_prof * BigR  * (v_x*r0_x + v_y*r0_y                                  ) * xjac * tstep * factor(5,5) &
                       - v * F0 / BigR * Vpar0 * r0_p                                             * xjac * tstep * factor(5,6) &
                       - v * Vpar0 * (r0_s * ps0_t - r0_t * ps0_s)                                       * tstep * factor(5,6) &
                       - v * F0 / BigR * r0 * vpar0_p                                             * xjac * tstep * factor(5,6) &
                       - v * r0 * (vpar0_s * ps0_t - vpar0_t * ps0_s)                                    * tstep * factor(5,6) &

                       + v * 2.d0 * tauIC * p0_y * BigR                                           * xjac * tstep * factor(5,7) &

                       + v * r0_corr * rn0      * BigR * Sion_T                                   * xjac * tstep * factor(5,8) &
                       - v * r0_corr * r0_corr  * BigR * Srec_T                                   * xjac * tstep * factor(5,9) &
                       
                       + zeta * v * delta_g(mp,5,ms,mt) * BigR                                    * xjac       * factor(5,10)   &

                       - D_perp_num * (v_xx + v_x/Bigr + v_yy)*(r0_xx + r0_x/Bigr + r0_yy) * BigR * xjac * tstep * factor(5,11) &

                       - TG_num5 * 0.25d0 * BigR**3 * (r0_x * u0_y - r0_y * u0_x)                                               &
                                                    * ( v_x * u0_y - v_y * u0_x) * xjac * tstep * tstep          * factor(5,12) &
            ...
            rhs_ij(7) = - v * F0 / BigR * P0_p                                             * xjac * tstep * factor(7,1) &
                        - v * (P0_s * ps0_t - P0_t * ps0_s)                                       * tstep * factor(7,1) &

                     - v*(particle_source(ms,mt) + source_pellet) * vpar0 * BB2   * BigR * xjac * tstep * factor(7,2) &

                     - 0.5d0 * r0 * vpar0**2 * BB2 * (ps0_s * v_t - ps0_t * v_s)                * tstep * factor(7,3) &
                     - 0.5d0 * v  * vpar0**2 * BB2 * (ps0_s * r0_t - ps0_t * r0_s)              * tstep * factor(7,3) &
                     + 0.5d0 * v  * vpar0**2 * BB2 * F0 / BigR * r0_p                    * xjac * tstep * factor(7,3) &

                     - visco_par_num * (v_xx + v_x/Bigr + v_yy)*(vpar0_xx + vpar0_x/Bigr + vpar0_yy) * BigR * xjac * tstep * factor(7,4) &

                     + zeta * v * delta_g(mp,7,ms,mt) * R0 * F0**2 / BigR                        * xjac * factor(7,5) &
             ...
             - TG_NUM7 * 0.25d0 * r0 * Vpar0**2 * BB2 &
                       ...
                       * (-(ps0_s * v_t     - ps0_t * v_s)    /xjac                      ) * xjac * tstep * tstep * factor(7,6) &
             - TG_NUM7 * 0.25d0 * v  * Vpar0**2 * BB2 &
                       * (-(ps0_s * vpar0_t - ps0_t * vpar0_s)/xjac + F0 / BigR * vpar0_p) / BigR   &
                       * (-(ps0_s * r0_t    - ps0_t * r0_s)   /xjac + F0 / BigR * r0_p)    * xjac * tstep * tstep * factor(7,6) &
                 
                    + (1.d0 - delta_n_convection) * (     &
                    - v *(r0_corr * rn0_corr * Sion_T) * vpar0 * BB2 * BigR                        * xjac * tstep * factor(7,7) &
                    + v *(r0_corr * r0_corr  * Srec_T) * vpar0 * BB2 * BigR                        * xjac * tstep * factor(7,8) &
                    )
 
            if (normalized_velocity_profile) then
              rhs_ij(7) = rhs_ij(7) - visco_par * (v_x * (vpar0_x-Vt0_x) + v_y * (vpar0_y-Vt0_y)) * BigR* xjac * tstep * factor(7,9) 
            else
            ...
            if (NEO) then
              rhs_ij(7) =  rhs_ij(7)  + amu_neo_prof(ms,mt)*BB2/(Btheta2+epsil)  * factor(7,10) &
            ...
            !###################################################################################################
             rhs_ij(8) = BigR * (- Dn0x * rn0_x * v_x - Dn0y * rn0_y * v_y )   * xjac * tstep     * factor(8,1)         &         
                  ...
                  - BigR * v * r0_corr * rn0_corr * Sion_T                                 * xjac * tstep * factor(8,3) &  
                  + BigR * v * r0_corr * r0_corr  * Srec_T                                 * xjac * tstep * factor(8,4) &
                  + BigR * v * source_neutral                                              * xjac * tstep * factor(8,5) &
                  - Dn_perp_num * (v_xx + v_x/Bigr + v_yy)*(rn0_xx + rn0_x/Bigr + rn0_yy)  * BigR * xjac * tstep * factor(8,6) &

                  + v * delta_g(mp,8,ms,mt) * BigR * xjac * zeta* factor(8,7) 
