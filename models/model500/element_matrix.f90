...
real*8     :: current_source(n_gauss,n_gauss),particle_source(n_gauss,n_gauss),heat_source(n_gauss,n_gauss), source_pellet, source_mgi
...
real*8     :: amat_11, amat_12, amat_21, amat_22, amat_23, amat_24, amat_25, amat_26, amat_28, amat_33, amat_31, amat_44, amat_42
real*8     :: amat_51, amat_52, amat_55, amat_56, amat_57, amat_61, amat_62, amat_65, amat_66, amat_67, amat_16, amat_13
real*8     :: amat_71, amat_72, amat_75, amat_76, amat_77, amat_15, amat_78, amat_58, amat_68, amat_85, amat_86, amat_88
...
do ms=1, n_gauss
 ...
 !                       x_g(ms,mt),y_g(ms,mt),ps0,phi,source_pellet)
     ...
     call mgi_source(mgi_amplitude,mgi_R,mgi_Z,mgi_phi,mgi_radius,mgi_sig,mgi_length, &       
                        x_g(ms,mt),y_g(ms,mt),phi,source_mgi)                                 
...
!###################################################################################################
                 ...
                 amat_15 =  0.d0 
                        !   + v * tauIC/(r0*BB2) * F0**2/BigR**2 * T0  * (ps0_s * rho_t - ps0_t * rho_s) * theta * tstep &
...
!                        + v * tauIC * BigR**4 * (p0_s * w_t - p0_t * w_s)              * theta * tstep 
			 ...
			 -  tauIC * BigR**4 * w * (p0_s * v_t - p0_t * v_s)             * theta * tstep 
                 ...
                 amat_25 = + 0.5d0 * vv2 * (v_x * rho_y_hat - v_y * rho_x_hat)   * xjac * theta * tstep &
                           ...
                           -  tauIC * BigR**4 * w0 * T0 *(rho_s * v_t - rho_t * v_s) * theta * tstep &
                           -  tauIC * BigR**4 * w0 * rho*(T0_s * v_t  - T0_t  * v_s) * theta * tstep &
...
!						 * xjac * theta * tstep 
                 ...
                 amat_26 = - BigR**2 * (v_s * r0_t * T   - v_t * r0_s * T)      * theta * tstep  &
                           ...
                           -  tauIC * BigR**4 * w0 * r0 *(T_s * v_t  - T_t  * v_s) * theta * tstep &
                           -  tauIC * BigR**4 * w0 * T  *(r0_s * v_t - r0_t * v_s) * theta * tstep &
                           
			   - BigR**3 * (r0*rn0*S_ion_T)*(v_x * u0_x + v_y * u0_y) * xjac * theta * tstep 
...
!						 * xjac * theta * tstep 
                 ...
                 amat_28 = - BigR**3 * (r0*rhon*S_ion)*(v_x * u0_x + v_y * u0_y) * xjac * theta * tstep
...
!###################################################################################################
                 ...
                 amat_55 = v * rho * BigR * xjac * (1.d0 + zeta)  &
                         ...
                         - v * rn0 * S_ion * rho * BigR                                                * xjac * theta * tstep &
...
!                        - v * 2.d0 * tauIC * (rho_y * T0 + rho*T0_y) * BigR                           * xjac * theta * tstep &
		 ...
		 amat_56 = - v * BigR * r0 * rn0 * S_ion_T * T                                          * xjac * theta * tstep
		 
!			   - v * 2.d0 * tauIC * (T_y * r0 + T*r0_y) * BigR                           * xjac * theta * tstep 
		 ...
		 amat_58 = - BigR * v * r0 * S_ion * rhon                                             * xjac * theta * tstep
...
!###################################################################################################
			   ...
			   + ZK_perp_num  *                                                          &
                 ...
                 amat_68 = + v * BigR * r0 * rhon * ksiion * S_ion                        * xjac * theta * tstep			   
...
!###################################################################################################
			 ...
			 + v * (P0_s * psi_t - P0_t * psi_s)                                            * theta * tstep &
			   ...
			   + v *(r0 * rn0 * S_ion_T) * vpar0 * BB2 * BigR           * xjac * theta * tstep
                 ...
                 amat_78 = + v *(r0 * rhon * S_ion) * vpar0 * BB2 * BigR         * xjac * theta * tstep
...
!################################################################################################### 
                ...
                amat_85 = + BigR * v * rn0 * S_ion * rho                                              * xjac * theta * tstep     
                 
		 
		amat_86 = + BigR * v * r0 * rn0 * S_ion_T * T                                            * xjac * theta * tstep      

                 
		amat_88 = + v * rhon * BigR * xjac   &
