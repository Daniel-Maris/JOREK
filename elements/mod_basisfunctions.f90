...
!!
!! Vectorized version
pure subroutine basisfunctions5(n_p, s, t, H, H_s, H_t)
implicit none

! --- Routine parameters
integer, intent(in) :: n_p          !< Number of positions to evaluate
real*8, intent(in)  :: s(n_p)       !< s-coordinate in the element
real*8, intent(in)  :: t(n_p)       !< t-coordinate in the element
real*8, intent(out) :: H(n_p,4,4)   !< Basis functions
real*8, intent(out) :: H_s(n_p,4,4) !< Basis functions derived with respect to s
real*8, intent(out) :: H_t(n_p,4,4) !< Basis functions derived with respect to t
...
H_s(:,4,4) =9.d0*(1.d0 - 3.d0*s)*(-1.d0 + s)*(-1.d0 + t)*t**2
H_t(:,4,4) =-9.d0*(-1.d0 + s)**2*s*t*(-2.d0 + 3.d0*t)
end subroutine basisfunctions5
