  ...
  subroutine import_external_fields(filename, my_id)
    ...
    if (.not. starwall_equil_coils) then
      ...
      if ( my_id == 0 ) then
        ...
        if ( n_coils /= n_pf_coils_nml ) then
          write(*,*) 'WARNING: namelist coils number n_pf_coils_nml does not match with external coils number n_coils from coil_field.txt!'
          stop
        end if
        
        if ( .not. allocated(I_coils) ) then
          allocate( I_coils(n_coils) )
          I_coils(1:n_coils) =  pf_coils(1:n_pf_coils_nml)%current 
      ...
      if ( my_id == 0 ) then
        ...
        if ( sr%ncoil /= n_pf_coils_nml ) then
          write(*,*) 'WARNING: number of namelist coils "n_pf_coils_nml" does not match the STARWALL number of coils'
          stop
        end if
        ...
        if ( .not. allocated(I_coils) ) then
          ...
          I_coils(1:sr%ncoil) =  pf_coils(1:n_pf_coils_nml)%current
          n_coils             =  n_pf_coils_nml
          write(*,*) 'I_coils allocated '               
        endif
      endif
    ...
    if ( my_id /= 0 ) then        
      ...
      allocate( I_coils(n_pf_coils_nml) )
    end if
    
    call MPI_bcast(n_coils,                       1, MPI_INTEGER,          0, MPI_COMM_WORLD, err)
    call MPI_bcast(I_coils,          n_pf_coils_nml, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, err)
