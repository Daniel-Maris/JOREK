...
if (my_id .eq. 0) then
  ...
  do i=1,node_list%n_nodes
    ...
    if ( with_TiTe ) then
      call temperature_i(xpoint2, xcase2, Z, Z_xpoint, psi,psi_axis,psi_bnd, zTi,dTi_dpsi,dTi_dz,dTi_dpsi2,dTi_dz2,   &
                                                               dTi_dpsi_dz,dTi_dpsi3,dTi_dpsi_dz2, dTi_dpsi2_dz)
      call temperature_e(xpoint2, xcase2, Z, Z_xpoint, psi,psi_axis,psi_bnd, zTe,dTe_dpsi,dTe_dz,dTe_dpsi2,dTe_dz2,   &
    ...
    if (with_TiTe) then
      ...
      node_list%node(i)%values(1,1,7) = zTe
      node_list%node(i)%values(1,2,7) = dTe_dpsi  * node_list%node(i)%values(1,2,1) + dTe_dz * node_list%node(i)%x(1,2,2)
      node_list%node(i)%values(1,3,7) = dTe_dpsi  * node_list%node(i)%values(1,3,1) + dTe_dz * node_list%node(i)%x(1,3,2)
      node_list%node(i)%values(1,4,7) = dTe_dpsi  * node_list%node(i)%values(1,4,1) + dTe_dz * node_list%node(i)%x(1,4,2) &
