...
type(event)                                       :: D_sputter_event,D2_sputter_event,gas_puff_event ,gas_puff2_event, gas_puff3_event!, partwriter
type(adf11_all)                                   :: adas
...
type(particle_sputter)                            :: D_sputter_source, D2_sputter_source
type(type_edge_domain), allocatable, dimension(:) :: edge_domains
type(edge_elements)                               :: D_edge, D2_edge
...
integer    :: n_sputter_mol
logical    :: restart_from_atoms
...
if (restart_particles) then 
  ...
  !  if (allocated(sim%groups(atoms)%particles)) write(*,*) 'Atoms particles are allocated'
  !  if (allocated(sim%groups)) write(*,*) 'group is allocated'
  !  if (allocated(sim%groups(atoms)%particles)) write(*,*) 'Mol particles are allocated'
   write(*,*) 'n_groups', size(sim%groups)
  if (restart_from_atoms) then
  ...
  ! write(*,*) 'size mol', size(sim%groups(molecules)%particles(:),1)
  ! write(*,*) 'size atom', size(sim%groups(atoms)%particles(:),1)
  ...
  !allocate(particle_kinetic_leapfrog::sim%groups(atoms)%particles(n_particles_local))
	n_particles_local_mol = n_particles_local!/10 !int(100/sim%n_cpu) !< TODO: make new n_particles for molecules or make n_particles(array)
  ! deallocate(sim%groups(molecules)%particles)
  ...
  if (sim%my_id == 0) write(*,*) 'Group 2 = D2'
	...
	n_particles_local = int(n_particles/sim%n_cpu) 
  allocate(particle_kinetic_leapfrog::sim%groups(atoms)%particles(n_particles_local))
  n_particles_local_mol = n_particles_local !int(100/sim%n_cpu) !< TODO: make new n_particles for molecules or make n_particles(array)
...
if (use_sputtering) then  
  ...
  D_sputter_event = new_event_ptr(D_sputter_source)

  ! n_sputter_mol = 0 ! no plasma to molecule
  ! D2_sputter_source = initialise_sputtering_D2(sim%fields%node_list, sim%fields%element_list, n_sputter_mol)
  ! D2_sputter_event = new_event_ptr(D2_sputter_source)
endif
...
if (use_puffing) then  
  ...
  if (puff_t_dependent) then
	t_puff_start = 5000*t_norm !in5000 jorek tijdeenheden begint puffing !restartfile1625?!25000*t_norm !34995*t_norm !5000*t_norm !< start puffing after this amount of seconds, t_SI = t_jorek*t_norm jorek time units
...
if ( restart_particles) then 
    ...
    !  call with(sim, D2_sputter_event)
 ...
 do j=1,size(particles,1)
    ...
    do k=1,n_steps
	  ...
	  if (use_ionisation .and. .not. limits) then
          ...
          if (particle_tmp%weight .le. 1.0d11) then !1.0d9 !1.0d10 1.0d7
 ...
 !$ i_rng = omp_get_thread_num()+1
  ...
  if (particle_tmp%weight .eq. 0.d0) cycle
    ...
    call rng(i_rng)%next(diss_ran)
      ...
      if (particle_tmp%weight .gt. 5.d16) then !weightlimit
      ...
      if (particle_tmp%weight .gt. 1.d16) then !1.d10
        ...
        if (use_nondissionisation) then  
          ...
          if (use_H2plus_dissociation) then 
            ...
            if (ion_diss_ran(1)*sum_probabilities .le. ion_diss_exc_prob) then
              ...
              ion_diss_exc_source(ii) = H2plus_tmp(ii)%weight * 1.d0 !zoveel ioniseren we 
              ...
              ion_diss_ion_source(ii) = H2plus_tmp(ii)%weight * 1.d0 
...
function initialise_sputtering_D2(node_list, element_list, n_reflect) result(D_sputter_source)
  ...
  call D2_edge%prepare(node_list, element_list, edge_domains, nsub=6, nsub_toroidal=1)!,wall_albedo=wall_albedo)

  ! target group, number of particles per mpi task, densities, Zs, basename
  D_sputter_source = particle_sputter(D2_edge, 2, n_reflect, basename='D2_reflect')
  D_sputter_source%use_Yn_func = .false.
  D_sputter_source%n_save = nout !10 ! or nout
  D_sputter_source%albedo_for_neutrals = 1.d0
  D_sputter_source%sputtered_particle_weight_threshold = 1.d0
