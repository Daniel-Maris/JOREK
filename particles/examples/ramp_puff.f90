!> Kinetic neutral detachment program using a flat-ramp-flat puffing input and no pumping
!
...
program ramp_puff
  ...
  !Bot puff
  !define here where to puff from and the shape of the flat-slope-flat puffing profile
  !The first and second puff will do half of the specified flat-slope-flat, while the third
  !location can only be 
  boxpuff = .true. !puff in a poloidal box (true) or puff at a circle, mimicking a valve (false)
  puff_rate = 4.d23 ! puff rate at the end (or always if puff_t_dependent = .false.)

  puff_t_dependent = .true. !< select if you want time dependent puffing (i.e. flat-slope-flat)
  t_puff_start = 5000*t_norm !< start the puff ramp after this amount of seconds, 
  !< t_SI = t_jorek*t_norm jorek time units
  t_puff_slope = 4.d-3 !< linearly ramps up the puffing during this time (in s)
  fueling_rate_start = 4.d23 ! puffrate of the first flat phase  
  
  !> location and radius of puff 1 valve if not using boxpuff
  r_valve     = 0.05d0 
  R_valve_loc = 4.3d0 
  Z_valve     = -3.8d0 
  ...
  if (use_puffing) then
    ...
    if (puff_t_dependent) then
      ...
      gas_puff3 = particle_puffing(n_puff/2, puff_rate3   , r_valve3, R_valve_loc3, Z_valve3, puff_t_dependent=puff_t_dependent,t_puff_start=t_puff_start,t_puff_slope=t_puff_slope, &
          fueling_rate_start=0.d21,poly_R=poly_R3,poly_Z=poly_Z3,boxpuff=boxpuff)
    else
      gas_puff  = particle_puffing(n_puff, puff_rate/2.d0, r_valve, R_valve_loc, Z_valve) 
      gas_puff2 = particle_puffing(n_puff, puff_rate/2.d0, r_valve, R_valve_loc2, Z_valve2)
      gas_puff3 = particle_puffing(n_puff/2, puff_rate3   , r_valve3, R_valve_loc3, Z_valve3) 
    end if
...
contains
  ...
  subroutine do_1particle_recombination(sim,element_list,node_list,rng,particle_step_time)
    ...
    if (sim%my_id .eq. 0) then
      ...
      if(particle_step_time .gt. 0.d0) then !filters out NaNs from initialisation
