...
use nodes_elements
use phys_module, only: n_particles, nstep_particles, nsubstep_particles, tstep_particles, use_ncs, use_pcs, use_ccs, deuterium_adas,sqrt_mu0_over_rho0
...
type(adf11_all)                                   :: adas
!type(ADF11_all)                                   :: ad_deuterium !< imported from mod_atomic_coeff_deuterium
...
adas = read_adf11('12_h')
 if (deuterium_adas) ad_deuterium =  read_adf11('96_h') !< move to core (jorek2_main for particles)
...
use phys_module, only: CENTRAL_MASS, CENTRAL_DENSITY
use constants,   only: MU_ZERO, MASS_PROTON, ATOMIC_MASS_UNIT, K_BOLTZ, EL_CHG
use mod_ionisation_recombination, only : rec_rate_local, rec_rate_global
...
real*8    :: ion_rate, ion_source, ion_prob, ion_ran(1), cx_ran(7),st_ran(2), cx_source, cx_energy 
...
!  jorek_feedback%rhs_gather_time = jorek_feedback%rhs_gather_time + n_steps * timesteps
  ...
  type is (particle_kinetic_leapfrog)
    ...
    !$omp use_cx, use_ionisation, use_sputtering, use_recombination, particles_per_element,          &
    ...
    !$omp particle_source, velocity_par_source, energy_source, v_temp, K_eV, T_eV, cx_ran,st_ran)  &
...
!run_rec = abs(target_time - sim%time) !.lt. particle_step_time  

  !write(*,*) "run_rec", run_rec
  ...
  write(*,*) "sim%time after call,",sim%time, "event(2)%run_at/run_rec", run_rec !events(2)%run_at(rec_time) 
  if (use_recombination .and. run_rec) then !< events(2) = jorek_stepper, sim%time = target_time of next event due to few lines before
...
subroutine do_1particle_recombination(element_list,node_list,jorek_stepper,rng)!,element)
use mod_jorek_timestepping
...
!this to define local_elms    call tr_allocate(local_elms,1,element_list%n_elements,"local_elms",CAT_FEM) !????????????
 
!calculate total recombination
...
! initialise particle in the element with Position, Weight, Energy, Momentum			
	...
	do i = 1, particles_per_element
	    k = k + 1 !< update free particle index ! at begin of loop as k is initialized at k =0
		!write(*,*) 'k,',k,'i_free(k),', i_free(k)
		...
		particles(i_free(k))%st(1:2) = st_ran(2)! [s, t]
...
!< 8 terms '(A,I13,A,E8.2,A,E8.2,A,E8.2 )'
