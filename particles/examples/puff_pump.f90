!> Kinetic neutral detachment program using dynamic puffing from a file and basic neutral pumping
!
! Puffing profile must be specified in file "puff_rate_input" by having points in the t (s) and 
! puffrate (atoms/s) columns, with ascending t, representing a piecewise linear puffing profile 
! 
! The neutral pumping works by selecting elements on the domain boundary and giving those a slightly
! decreased reflectivity for kinetic neutrals, thus effectively pumping neutrals.
!
! Includes recombination, ionisation, recycling and charge exchange for Deuterium. It has major bugfixes 
! compared to sep_puff for the neutral particle balance (conserved) and total particle balance (nearly
! conserved).
! 
! ***WARNING*** this is a dangerous version of the code where the distinction between a particle 
! being not used by the simulation and a particle being lost to the wall but about to be reflected 
! is not made. Therefore you cannot put any code creating new particles between the kinetic loop
! and the sputtering (the sputtering module also handles reflections and recycling) function call
! as you will overwrite to be reflected neutrals by new neutrals, thus effectively losing them. 
! This is likely also the reason why the sanity checks of weight difference before and after a
! function call do not correspond to the change in weight at that function call: unused particles
! are not always correctly reset to 0 weight, 0 location, 0 velocity, 0 element etc.
program puff_pump
  ...
  type(projection), target                          :: jorek_feedback, project_density, write_particle_vtk 
  ...
  integer   :: n_puff
  logical   :: puff_t_dependent,boxpuff, rate_from_file
  character(len=64) :: puff_rate_filename

  !use physics
  logical   :: use_recombination, use_puffing, use_cx, use_ionisation , use_sputtering,use_line_radiation, use_pumping
  logical   :: run_stepper
  ...
  character(len=64) :: projections_dir = './projections/' !< directory to write the particle projection vtk files

  ! Sanity checks on the diagnostics, by taking differences of particle content before and after
  ! a function. Note that the error is larger than in the actual diagnostics because the total 
  ! content is far larger than the difference due to one operation, so the amount of significant 
  ! digits of this difference is small compared to the accuracy of the number it's stored in.
  real*8    :: sanity_ion_pre,sanity_ion_post,sanity_ion_diff_local,sanity_ion_diff_tot
  real*8    :: sanity_recom_pre,sanity_recom_post,sanity_recom_diff_local,sanity_recom_diff_tot
  real*8    :: sanity_sputt_pre,sanity_sputt_post,sanity_sputt_diff_local,sanity_sputt_diff_tot
  real*8    :: sanity_puff_pre,sanity_puff_post,sanity_puff_diff_local,sanity_puff_diff_tot
  ! Numer of free superparticles due to recombination
  integer   :: freesp_pre_recom, freesp_post_recom, usedsp_recom_local, usedsp_recom_tot
  ...
  if (restart_particles) then 
    ...
    call with(sim, partreader) !<defines sim%groups and particles, if more than one group, change num_groups
  ...
  if (use_sputtering) then  
    ...
    D_sputter_source = initialise_sputtering(sim%fields%node_list, sim%fields%element_list, bnd_elm_list, n_reflect, use_pumping)
    D_sputter_event = event(D_sputter_source)
  endif
  
  !Bot puff
  !define here where to puff from
  boxpuff = .true. !puff in a poloidal box (true) or puff at a circle, mimicking a valve (false)
  !> note well, the total puffing rate is the sum of all events, so with 2 events using the input 
  !> file, the total rate is twice the rate in the input file. You can also mimic a constant rate
  !> or a flat-ramp-flat using the input file easily
  rate_from_file = .true. 
  puff_rate_filename = "puff_rate_input"
  !> location of puff 1 if using boxpuff
  ...
  !> location of puff 2 valve if not using boxpuff (radius is same as puff 1 valve)
  R_valve_loc2 = 5.5d0 
  Z_valve2     = -4.35d0
  !> location of puff 2 if using boxpuff
  ...
  !puffing implementation
  if (use_puffing) then
    n_puff      = int(5.d-5*n_particles_local* sim%n_cpu)
    
    gas_puff  = particle_puffing(n_puff, poly_R=poly_R,poly_Z=poly_Z,boxpuff=boxpuff, &
        rate_from_file=rate_from_file,puff_rate_filename=puff_rate_filename, sim_id=sim%my_id)
    gas_puff2 = particle_puffing(n_puff,poly_R=poly_R2,poly_Z=poly_Z2,boxpuff=boxpuff, &
        rate_from_file=rate_from_file,puff_rate_filename=puff_rate_filename, sim_id=sim%my_id)
    gas_puff3 = particle_puffing(n_puff/2, fueling_rate=puff_rate3,poly_R=poly_R3,poly_Z=poly_Z3,boxpuff=boxpuff, sim_id=sim%my_id)
  
    gas_puff_event  = event(gas_puff)
    gas_puff2_event = event(gas_puff2)
    gas_puff3_event = event(gas_puff3)
    
  else !< this might be not well behaved
    n_puff = 0
    gas_puff  = particle_puffing(n_puff, 0.d0, r_valve, R_valve_loc, Z_valve)
    gas_puff2 = particle_puffing(n_puff, 0.d0, r_valve, R_valve_loc, Z_valve)
    gas_puff3 = particle_puffing(n_puff, 0.d0, r_valve, R_valve_loc, Z_valve)
  endif !use_puffing
  ...
  ! make the folder to write the particle projection vtk file to, if it doesn't exist yet
  if(sim%my_id .eq. 0) then
    write(*,*) "making folder to write the particle projection vtk files to: ",trim(projections_dir)
    call system('mkdir -p '//trim(projections_dir))
  end if

  ! Set up feedback, do not write to vtk every timestep
  jorek_feedback = new_projection(sim%fields%node_list, sim%fields%element_list, &
      ...
      do_zonal = .false., calc_integrals=.false., to_vtk=.true., to_h5 = .false., &
      basename=trim(projections_dir)//'projections', nsub=2)
  
  !set up projections which write to vtk
  write_particle_vtk = new_projection(sim%fields%node_list, sim%fields%element_list, &
      filter_n0 = filter_perp_n0, filter_hyper_n0 = filter_hyper_n0, filter_parallel_n0=filter_par_n0,      &
      filter = filter_perp, filter_hyper = filter_hyper, filter_parallel=filter_par, fractional_digits = 9, &
      do_zonal = .false., calc_integrals=.false., to_vtk=.TRUE., to_h5 = .false., &
      basename=trim(projections_dir)//'projections', nsub=2)
  ...
  if (use_ncs) then
    ...
    allocate(write_particle_vtk%rhs(n_order+1, n_vertex_max, sim%fields%element_list%n_elements, n_tor, 4))
  elseif (use_pcs) then  ! not implemented yet!
    ...
    allocate(write_particle_vtk%rhs(n_order+1, n_vertex_max, sim%fields%element_list%n_elements, n_tor, 1))
  ...
  project_density = new_projection(sim%fields%node_list, sim%fields%element_list, &
      ...
      f=[proj_f(proj_one, group = 1)], fractional_digits = 9,  to_vtk=.TRUE., to_h5=.FALSE., &
      basename=trim(projections_dir)//'density', nsub=2)
...
!$ n_stream = omp_get_max_threads()
  ...
  if (use_recombination .and. (.not. restart_particles)) then 
    ...
    call do_1particle_recombination(sim,element_list,node_list,rng,particle_step_time) 
  ...
  do while (.not. sim%stop_now) !< loopstructure itself
    ...
    ! particle loop
    sanity_ion_pre = sum(sim%groups(1)%particles(:)%weight)
    ...
    ! beware not to put any particle creation functions between sputtering and kinetic loop
    
    sanity_ion_post       = sum(sim%groups(1)%particles(:)%weight)
    sanity_ion_diff_local = sanity_ion_post - sanity_ion_pre
    call MPI_REDUCE(sanity_ion_diff_local, sanity_ion_diff_tot, 1, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
    if(sim%my_id .eq. 0) write(*,'(A50,1es24.14)') 'change in neutral content due to ionisation', &
      sanity_ion_diff_tot
    !end particle loop
    ...
    if (run_stepper)then
      ...
      if ( (abs((tstart_jorek +closest_iteration*tstep_si*nout) -projection_time) .le. 1.d-13) .or. sim%stop_now) then !< == true every nout steps
        !something about this doesn't work yet, so as a temporary fix we'll just use the old method
        !and then reduce the number of vtk's afterwards
        !call with(sim, write_particle_vtk) !write a vtk of the particle projection    
      endif !< write projection or diagnostics
      
      if ( (abs((tstart_jorek +nint((projection_time - tstart_jorek)/(tstep_si*500))*tstep_si*500) -projection_time) .le. 1.d-13)) then !< == true every tstep * 500 steps
      ...
      ! beware not to put any particle creation functions between sputtering and kinetic loop
      if (use_sputtering) then  
        sanity_sputt_pre = sum(sim%groups(1)%particles(:)%weight) !< total neutral particle content
        if(abs(sanity_sputt_pre - sanity_ion_post) .gt. 1.d10) write(*,*) 'problem: particle &
            weight getting lost in between kinetic loop and sputtering'

        ! call sputtering
        call with(sim, D_sputter_event) !event(D_sputter_source))
        
        sanity_sputt_post       = sum(sim%groups(1)%particles(:)%weight)
        sanity_sputt_diff_local = sanity_sputt_post - sanity_sputt_pre
        call MPI_REDUCE(sanity_sputt_diff_local, sanity_sputt_diff_tot,1, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
        if(sim%my_id .eq. 0) write(*,'(A50,1es24.14)') 'change in neutral content due to sputtering', &
            sanity_sputt_diff_tot
      endif   !use_sputtering

      if (use_recombination) then
        sanity_recom_pre = sum(sim%groups(1)%particles(:)%weight)
        freesp_pre_recom = count(sim%groups(1)%particles(:)%i_elm .le. 0) 
        
        !call recombination
        call do_1particle_recombination(sim,element_list,node_list,rng,particle_step_time) 
        
        sanity_recom_post = sum(sim%groups(1)%particles(:)%weight)
        freesp_post_recom = count(sim%groups(1)%particles(:)%i_elm .le. 0)
        
        sanity_recom_diff_local = sanity_recom_post - sanity_recom_pre
        usedsp_recom_local      = freesp_pre_recom  - freesp_post_recom
        
        call MPI_REDUCE(sanity_recom_diff_local, sanity_recom_diff_tot, 1, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
        call MPI_REDUCE(usedsp_recom_local, usedsp_recom_tot, 1, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
        if(sim%my_id .eq. 0) write(*,'(A50,1es24.14)') 'change in neutral content due to recombination', &
            sanity_recom_diff_tot
        if(sim%my_id .eq. 0) write(*,'(A50,2I10)') 'new superparticles from recombination (used/elems)', &
            usedsp_recom_tot,element_list%n_elements
        !< amount of new superparticles should be equal to n_elements*particles_per_element
      endif !use_recombination
        
      if (use_puffing) then
        sanity_puff_pre = sum(sim%groups(1)%particles(:)%weight)
        
        !> puffing at three locations
        call with(sim, gas_puff_event) 
        call with(sim, gas_puff2_event)
        call with(sim, gas_puff3_event)
        
        sanity_puff_post       = sum(sim%groups(1)%particles(:)%weight)
        sanity_puff_diff_local = sanity_puff_post - sanity_puff_pre
        call MPI_REDUCE(sanity_puff_diff_local, sanity_puff_diff_tot, 1, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
        if(sim%my_id .eq. 0) write(*,'(A50,1es24.14)') 'change in neutral content due to puffing', &
            sanity_puff_diff_tot
      end if
    endif !run_stepper
...
!omp end parallel do
  ...
  end do ! while timesteps left to do in the simulation
...
contains
  ...
  subroutine loop_particle_kinetic_local(sim, jorek_feedback, rng, timesteps, n_steps, particle_start_time)
    ...
    do j=1,size(particles,1)
      ...
      do k=1,n_steps
        ...
        ! Charge Exchange
        ! It is assumed that we will have an exchange between hydrogen isotopes
    ...
    if (sim%my_id .eq. 0) write(*,'(A46,E16.8,I6)') "Lost superparticles at t due to ionisation: ", sim%time, n_super_ionized_all
    if (sim%my_id .eq. 0) write(*,'(A46,2E16.8)') " Lost particles at t due to ionisation: ", sim%time, n_lost_ion_all
    if (sim%my_id .eq. 0) write(*,'(A46,2E16.8)') " Ionization rate at time t [#/s]: ", sim%time, n_lost_ion_all / (timesteps * n_steps)
    ...
    if (sim%my_id .eq. 0) write(*,'(A46,2E16.8)') "Energy exchange to plasma [W] at t due to ionisation: ", sim%time, p_lost_ion_all ! energy gain
    if (sim%my_id .eq. 0) write(*,'(A46,2E16.8)') "Lost energy [W] at t due to line radiation: ", sim%time, p_plt_lost_all
    if (sim%my_id .eq. 0) write(*,'(A46,2E16.8)') "Energy exchange to plasma [W] at t due to CX radiation: ", sim%time, p_cx_lost_all
    if (sim%my_id .eq. 0) write(*,'(A17,5E16.8)') 'TOTAL Exchange , delta t: ' ,sim%time,p_lost_ion_all, -p_plt_lost_all, p_cx_lost_all, timesteps * n_steps
    if (sim%my_id .eq. 0) write(*,'(A46,2E16.8)') "Total energy exchange to plasma [W]: ", sim%time, p_lost_ion_all -p_plt_lost_all+ p_cx_lost_all
  ...
  !                                 RECOMBINATION
  !================================================================================
  !> creates recombined neutrals by initialising superparticles in every element
  subroutine do_1particle_recombination(sim,element_list,node_list,rng,particle_step_time)
    use mpi
    use mod_atomic_elements
    use mod_particle_io
    ...
    integer             :: particles_per_element !< amount of initiated superparticles per element (>=1)
    ...
    real*8              :: weight_thrown_local, total_weight_thrown    

    particles_per_element = 1 !< must be >= 1, but even with high density cases 1 is usually enough

    weight_thrown_local = 0.d0
    ...
    !< note that to be reflected particles are thus also (unrightly so) considered free
...
!$omp barrier
    ...
    ! ==================
    if(n_free .lt. size(rec_rate_local)*particles_per_element) then
      write(*,*) "recombination problem, not enough superparticles (free/needed)", n_free, &
          size(rec_rate_local)*particles_per_element
    end if
...
!$omp schedule(dynamic,10)      &
!$omp shared(sim, particles, element_list, node_list, rec_v_R,rec_v_Z,rec_v_phi, &
...
!$omp st_ran, i_rng ) &
!$omp reduction(+:weight_thrown_local)
    do ife = 1, size(rec_rate_local) ! loop over all local elements
      
      if(isnan(rec_rate_local(ife)) .or. isnan(weight_thrown_local)) write(*,*) "problem: nans in recombination"
      
      if (isnan(rec_v_R(ife)) .or. isnan(rec_v_Z(ife)) .or. isnan(rec_v_phi(ife))) then !NaN check
        write(*,*) "recombination problem: NaNs in recombination velocities"
        cycle
      end if
      if (rec_rate_local(ife)* central_density* 1.d20 .le. 1.d3) then !this happens a lot due to T > 10 keV for core elements
        weight_thrown_local = weight_thrown_local + rec_rate_local(ife)* central_density* 1.d20
        cycle
      end if

      ! --- Get element
      ielm    = (sim%my_id+1) + sim%n_cpu*(ife - 1) !< loops over all indices, distributing close
      ! indices over the different MPI processes to share the load of heavily recombining elements
      element = element_list%element(ielm)
      
      ! initialise particle in the element with Position, Weight, Energy, Momentum			
      do i = 1, particles_per_element
        k = (i - 1) * size(rec_rate_local) + ife
        ...
        !currently particles are just initialised in the centre of the element
        !call rng(i_rng)%next(st_ran)
        ! sample random st combination
        !particles(i_free(k))%st(1:2) = st_ran(2)! [s, t] !< dummi for later
        s = 0.5d0
        t = 0.5d0
        particles(i_free(k))%st(1) = s
        particles(i_free(k))%st(2) = t
        
        
        !> uses i_elm and s,t to give us R,Z
        call interp_RZ(node_list,element_list,ielm,s,t,R,Z)
        particles(i_free(k))%x(1:3)  = [R, Z, 0.d0]!  = [R, Z, phi] no phi for axisymmetrix particles
        
        !> distribute directly fluid velocity?
        if (particles(i_free(k))%weight .lt. 1.d2) then
          write(*,*) "problem: low recombination weight? ",particles(i_free(k))%weight
        end if
        ...
        if(isnan(sum(particles(i_free(k))%v))) then
          write(*,*) 'problem: NaN in v of recombined particle, v=',particles(i_free(k))%v(1),particles(i_free(k))%v(2),particles(i_free(k))%v(3)
          cycle
        end if
...
!$omp end parallel do
    ...
    call MPI_REDUCE(weight_thrown_local, total_weight_thrown, 1, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
    if (sim%my_id .eq. 0) then
      write(*,'(A40,1es24.14)') 'planned recombination weight thrown', total_weight_thrown
    endif	

  end subroutine !do_1particle_recombination
  
  function initialise_sputtering(node_list, element_list, bnd_elm_list, n_reflect, use_pumping) result(D_sputter_source)
    ...
    type(type_edge_domain), allocatable, dimension(:) :: edge_domains
    logical, optional, intent(in)       :: use_pumping
    
    !local variables:
    integer,parameter :: n_pumps=2
    !normal pump
    integer,parameter :: n_elm_pump(n_pumps)=[11,11]!< number of pump elements for pump 1 & 2
    integer,parameter :: elm_pump_start(n_pumps)=[8430,8480] !< first element
    !larger pump to run global steady state with drifts
    !integer,parameter :: n_elm_pump(n_pumps)=[38,36]!< number of pump elements for pump 1 & 2
    !integer,parameter :: elm_pump_start(n_pumps)=[8422,8460] !< first element
    integer :: pump_elm_arr(sum(n_elm_pump)) !< contains all element numbers at which to pump
    integer :: i_pump, i, ielm, i_bnd_elm, i_node
    !> for logging purposes
    logical :: do_log
    real*8  :: RZ(2,n_pumps,2) !< RZ(1,:,:) = R, RZ(:,1,1) = beginning of pump 1, RZ(:,1,2) is end of pump 1, etc
    real*8  :: dl, Rav, A_pump, A_tot !< length of pump
    ...
    D_sputter_source = particle_sputter(D_edge, 1, n_reflect, basename=trim(projections_dir)//'D_reflect')
    ...
    !> kinetic particle pump using < 1 reflection by having nonzero absorption
    if(present(use_pumping)) then
      if(use_pumping .eq. .true.) then
        D_sputter_source%use_pumping = .true.
        
        !> Kukushkin (2007) JNM https://doi.org/10.1016/j.jnucmat.2007.01.094 eq. 2 
        !> assuming c_p = 75m³/s at room temperature for Deuterium with pump area of 12.55 m²
        !> leads to absoprtion coefficient of 13.6d-3
        !> however, since neutral-neutral collisions are missing, the spreading of our kinetic neutrals
        !> into the PFR is far slower than it should be so the actual removal of particles is lower than
        !> expected for the given divertor conditions and c_p, so we use a multiple of the necessary
        !> coefficient to compensate for the lack of gas expansion
        D_sputter_source%absorption_coeff = 99.d-2 !13.6d-3

        i=1
        do i_pump=1,n_pumps
          do ielm=elm_pump_start(i_pump),elm_pump_start(i_pump)+n_elm_pump(i_pump)-1
            pump_elm_arr(i)=ielm
            i=i+1
          end do
        end do
        D_sputter_source%pump_elems = pump_elm_arr

        !Everything is set now, the rest is just a diagnostic for the output file:
        RZ(:,:,:)=-1.d99
        A_tot = 0.d0

        do i_bnd_elm=1,bnd_elm_list%n_bnd_elements
          ielm = bnd_elm_list%bnd_element(i_bnd_elm)%element
          do i_pump=1,n_pumps
            if(ielm .eq. elm_pump_start(i_pump)) then !first element
              i_node = bnd_elm_list%bnd_element(i_bnd_elm)%vertex(2)
              RZ(:,i_pump,1) = node_list%node(i_node)%x(1,1:2)
            else if (ielm .eq. elm_pump_start(i_pump) + n_elm_pump(i_pump) - 1) then !last element of pump
              i_node = bnd_elm_list%bnd_element(i_bnd_elm)%vertex(1)
              RZ(:,i_pump,2) = node_list%node(i_node)%x(1,1:2)
            end if
          end do
        end do
        
        if(sim%my_id .eq. 0) then
          write(*,*) 'using pumping at (global) elements: ',D_sputter_source%pump_elems
          do i=1,n_pumps
            write(*,'(A5,I2,A16,2F10.6,A13,2F10.6)') 'pump ',i,' starts at RZ = ',RZ(:,i,1),' and ends at ',RZ(:,i,2)
            dl = sqrt((RZ(1,i,1)-RZ(1,i,2))**2 + (RZ(2,i,1)-RZ(2,i,2))**2)
            Rav = 0.5d0*(RZ(1,i,1)+RZ(1,i,2))
            A_pump = TWOPI*Rav*dl
            A_tot = A_tot+A_pump
            write(*,'(A21,I2,A3,F6.1)') 'pumping area of pump ',i,' = ',A_pump
          end do
          write(*,*) 'total pumping area',A_tot
          write(*,*) 'pumping absorption coefficient: ',D_sputter_source%absorption_coeff
          
        end if !sim_id .eq. 0
      else
        if(sim%my_id .eq. 0) write(*,*) 'not using pumping'
      end if !use_pumping
    else
      if(sim%my_id .eq. 0) write(*,*) 'not using pumping'
    end if !present(use_pumping)
