...
use phys_module, only: controller_K_i, controller_K_d, setpoint, max_value, min_value, controller_type, F0
...
if (restart_particles) then 
   ...
   !may be similar to phi_zero_whrite to a sim_in and sim_out but with different allocation size.
	...
	adas = read_adf11('12_h',directory='/home/ITER/vanhooe/Documents/Usefulscripts/atomic_and_bnd_data/')
...
! Read Open ADAS data for plasma fluid
 if (deuterium_adas .and. use_recombination) ad_deuterium =  read_adf11('96_h',directory='/home/ITER/vanhooe/Documents/Usefulscripts/atomic_and_bnd_data/') !< move to core (jorek2_main for particles)
...
R_valve_loc = 4.9d0 !in meters, aangepast voor iterlikesimpleplasma ! 4.3d0 !4.27d0!               4.4d0 !4.42787 !4.42787!2.33!2.6!2.1 !< for JET test !1.98991!2.58888  or 1.98991
Z_valve     = -3.63d0 ! in meters, angepast voor iterlikesimpleplasma ! -3.8d0 !-3.74d0!             -3.8d0 !-3.7 !-3.77948! -1.86 !-1.0!-1.75 !-0.550736!1.86579   or -0.550736
...
!poly_R2 = (/5.426d0 ,5.559d0 ,5.455d0 ,5.5586d0 /) ! coordinates of quadrangular valve shape, in meters
!poly_Z2= (/-4.155d0 ,-4.4005d0 ,-4.0803d0 ,-4.330d0 /) ! coordinates of quadrangular valve shape, in meters


!puff_rate3 = 0.d21 !136.d21 ! 109.d21 !72.d21 !160.d21 !160.d21!85.d21
!R_valve_loc3 = 2.d0  !aangepast 6.05d0!                  5.4d0 !5.46d0
!Z_valve3     = 2.d0 ! aangepast 4.15d0! 
!r_valve3    = 0.10d0!  .12
...
!Z_valve     = -3.7898!
if (use_puffing) then  
  n_puff      = int(5.d-5*n_particles_local* sim%n_cpu) !0.25 0.5d-4 !< now total n_puff ! amount of superparticles
  if (puff_t_dependent) then
	t_puff_start = 100*t_norm !was 5000 !25000*t_norm !34995*t_norm !5000*t_norm !< start puffing after this amount of seconds, t_SI = t_jorek*t_norm jorek time units
	t_puff_slope = 4.d-3 !4.d-3 !< linearly ramps up the puffing during this time in seconds
...
if (sim%my_id .eq. 0) write(*,'(A46,2E14.6)') "Energy exchange to plasme [W] at t due to ionisation: ", sim%time, p_lost_ion_all ! energy gain
if (sim%my_id .eq. 0) write(*,'(A46,2E14.6)') "Lost energy [W] at t due to line radiation: ", sim%time, p_plt_lost_all
if (sim%my_id .eq. 0) write(*,'(A46,2E14.6)') "Energy exchange to plasme [W] at t due to CX radiation: ", sim%time, p_cx_lost_all
...
if (sim%my_id .eq. 0) then
	...
	write(*,*) 'total power to recombined neutrals [MW]: ' , total_Erec_neutral_all *1.5d0 / MU_ZERO/(particle_step_time+1.d-20) /1.d6
	write(*,*) 'total energy lost to Prb [J]: ' , total_Erec_rad_all *1.5d0 / MU_ZERO
	write(*,*) 'total power lost to Prb [MW]: ' , total_Erec_rad_all *1.5d0 / MU_ZERO/(particle_step_time+1.d-20) /1.d6
	write(*,*) 'total volume : ' , total_volume_all
	write(*,'(A15,6E14.6)') 'TOTAL RECOMB: ',sim%time, total_rec_all* central_density* 1.d20 , total_Erec_neutral_all *1.5d0 / MU_ZERO, total_Erec_neutral_all *1.5d0 / MU_ZERO/(particle_step_time+1.d-20) /1.d6, &
	                           total_Erec_rad_all *1.5d0 / MU_ZERO, total_Erec_rad_all *1.5d0 / MU_ZERO/(particle_step_time+1.d-20) /1.d6
endif
