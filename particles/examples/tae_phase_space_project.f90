...
!$ real*8 :: w0, w1, mmm(3)
 ...
 ! A few examples of phase space projections.
  ...
  ! Full tensor, but is not needed in this example.
...
!$ n_stream = omp_get_max_threads()
  ...
  do while (.not. sim%stop_now)
    ...
    ! For this example we can abort here, but it could be extended to include multiple fluid timesteps.
...
!$      i_rng = omp_get_thread_num()+1
        ...
        do k=1,n_steps
          ...
          ! but for now it's just the v(t-1/2) velocity in the r(t) reference frame, as this is when E was calculated so we have to use the non-pushed particle velocity.
