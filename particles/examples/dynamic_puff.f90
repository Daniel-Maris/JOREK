!> Kinetic neutral detachment program using dynamic puffing from a file and no pumping
!
...
! 
! Includes recombination, ionisation, recycling and charge exchange for Deuterium only with no 
! pumping. It has major bugfixes compared to sep_puff for the neutral particle balance
! 
...
program ramp_puff
  ...
  if (restart_particles) then 
    ...
    write(*,*) "trying to read part_restart.h5"
  ...
  jorek_feedback = new_projection(sim%fields%node_list, sim%fields%element_list, &
      ...
      do_zonal = .false., calc_integrals=.false., to_vtk=.false., to_h5 = .false., &
