...
def read_datasets_from_hdf5(filenames,filepaths,separator):
  from h5py import File
  from numpy import array,transpose
  stddev   = []
  variance = []
  average  = []
  for filenameid,filename in enumerate(filenames):
    fhandler = File("".join([filepaths[filenameid],separator,filename]))
...
def plot_convergence_3d(ax,x,values,title='',fontsize=12,xlabel='x',\
ylabel='error',loglog=True,linewidth=3,markersize=10,markertype='s'):
  if(loglog):
    for valuesy in values:
...
def plot_per_pixel_quantities(x,data,title='',fontsize=12,xlabel='x',\
ylabel='error',loglog=True,linewidth=3,markersize=10,markertype='s',ncols=3):
  ...
  for dataid,dat in enumerate(data):
    plot_convergence_3d(axs[dataid],x,dat,title="".join([title,' for spectrum id: ',str(dataid)]),\
...
def plot_variance_stdded_covergence_per_pixel(x,variance,stddev,fontsize=12,\
ncols=3,xlabel='x',loglog=True,linewidth=linewidth,markersize=markersize,\
markertype=markertype,ncols=3):
  plot_per_pixel_quantities(x,stddev,title='Standard deviation convergence',\
  fontsize=fontsize,xlabel=xlabel,ylabel='\sigma',loglog=loglog,\
  linewidth=linewidth,markersize=markersize,markertype=markertyoe,ncols=ncols)
  plot_per_pixel_quantities(x,variance,title='Variance convergence',\
...
# main functions
def read_and_plot_data(x,filenames,filepaths,separator,fontsize=12,\
xlabel='x',loglog=True,linewidth=3,markersize=10,markertype='s',ncols=3):
  stddev,variance,average = read_datasets_from_hdf5(filenames,filepaths,separator)
  plot_average_variance_stddev(average,variance,stddev,ncols=ncols,fontsize=fontsize)
  plot_variance_stdded_covergence_per_pixel(x,variance,stddev,fontsize=fontsize,\
  ncols=ncols,xlabel=xlabel,loglog=True,linewidth=linewidth,markersize=markersize,\
...
if __name__ == '__main__':
  ...
  read_and_plot_data(args.x,args.filenames,args.filepaths,args.separator,xlabel=args.xlabel,\
  loglog=args.loglog,linewidth=args.linewidth,markersize=args.markersize,\
