...
type,extends(spectrum_base) :: spectrum_integrator_1st
  real*8,dimension(:),allocatable :: min_wlen  !< minimum wavelength
  real*8,dimension(:),allocatable :: wbin_size !< wlenght interval size
  contains
  procedure,pass(spectrum) :: allocate_spectrum     => allocate_spectrum_integrator_1st
...
!> Constructors--------------------------------------------------
!> constructor of the spectrum_integrator_1st class
!> inputs:
...
function construct_spectrum_integarator_1st(n_points,n_spectra,&
...
end function construct_spectrum_integarator_1st

!> Procedures first order integrator-----------------------------
!> allocate spectrum integrator 1st order datatype
!> inputs:
!>   spectrum:    (spectrum_integrator_1st) generate and integrated
!>                variables using the central point rule
!>   n_points:    (integer) number of spectral points
...
!>   spectrum: (spectrum_integrator_1st) generate and integrated
!>             variables using the central point rule
subroutine allocate_spectrum_integrator_1st(spectrum,n_points,&
...
!>   spectrum: (spectrum_integrator_1st) generate and integrated
!>             variables using the central point rule
subroutine set_spectrum_int_1st_properties(spectrum,n_spectra,&
min_wlen,max_wlen)
  implicit none
  !> inputs
