    ...
    do j=1,size(sim%groups(group_num)%particles,1)
      ...
      do k=1,nstep_part_adj
        ...
        if (sim%groups(group_num)%coupling_scheme == 'ncs') then
          ...
          if (sim%groups(group_num)%use_kin_cx  .and. particle_tmp%weight .gt. 0.d0) then 
            !< CX uses adas as well, but stopping it under a limit is destabilizing rather than stabilizing, 
            !< so we don't take the limits into account.
