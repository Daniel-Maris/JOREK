...
contains
  ...
  subroutine evolve_particle_group(sim, group_num, jorek_feedback, rng, tstep_part_adj)
    ...
    if (part_group%coupling_scheme == 'ncs') then
      ...
      call evolve_particle_group_ics(sim, part_group, feedback_rhs, feedback_nodelist, feedback_element_list, rng, tstep_part_adj, imp_q_idx)
    endif
  ...
  subroutine evolve_particle_group_ncs(sim, part_group, feedback_rhs, feedback_nodelist, feedback_element_list, rng, tstep_part_adj)
    implicit none
  ...
  subroutine evolve_particle_group_ics(sim, part_group, feedback_rhs, feedback_nodelist, feedback_element_list, rng, tstep_part_adj, imp_q_idx)
    use mod_collisions
    ...
    !      i_rng = 1
      !$ i_rng = omp_get_thread_num()+1
        ...
        !   call sim%fields%calc_vvector(t, particle_tmp%i_elm, particle_tmp%st, particle_tmp%x(3), vvector)
          ...
          if (part_group%use_kin_ionisation .and. .not. limits) then
              call rng(i_rng)%next(ion_ran)
              particle_tmp%q = int(new_charge(int(q_old,4), part_group%ad, log10(n_e), log10(T_e), tstep_part_adj, ion_ran(1:2)),1)
