...
real*8                    :: R, R_s, R_t, Z, Z_s, Z_t, st_jac, st_step(2)
real*8                    :: fE, fB, t_norm
real*8                    :: R_out, Z_out, s_out, t_out
integer                   :: i, j, k, m, i_elm, i_var(2), n_done, ifail, ielm_out, n_lost
...
!$omp   private(i, j, k, particle, x, v, st, i_elm, psi, psi_s, psi_t, psi_R, psi_Z, psi_prev, U_prev, R, R_s, R_t, Z, Z_s, Z_t,st_jac, &
!$omp           qom, B0, B02, E0, v_tmp, fE, fB, changed, lost, search,            &
!$omp           U, U_s, U_t, U_R, U_Z, U_phi, x_prev, v_prev, R_update, RPhi_update, P, P_s, P_t, R_out ,Z_out, ielm_out, s_out, &
!$omp           t_out, ifail, st_step)
...
do i = 1, particle_list%n_particles
  ...
  call interp_PRZ(node_list,element_list,i_elm,i_var,2,st(1),st(2),x(3),P,P_s,P_t,R,R_s,R_t,Z,Z_s,Z_t)
