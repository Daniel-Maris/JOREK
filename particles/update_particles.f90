...
real*8                    :: x(3), v(3), v0(3), v_up(3), delta_v(3), delta_phi
...
!    if (((R - R_axis)**2 + (Z - Z_axis)**2) .lt. r_crit2) then
!      psi_R =  CR * (R - R_axis)
!      psi_Z =  CZ * (Z - Z_axis)
!      write(*,'(A,8e16.8)') ' SMALL JACOBIAN : ', (  psi_s * Z_t - psi_t * Z_s ) / st_jac,psi_R,(- psi_s * R_t + psi_t * R_s ) / st_jac,psi_Z,psi_prev,psi
!    endif
    ...
    E0     = (/ - F0 * U_R, - F0 * U_Z, - F0 * U_phi /) * qom * t_step / 2.

    v0       = v + E0

    delta_v  = ( cross_product(v0,B0) + dot_product(v0,B0) * B0 - B02 * v0 ) * 2.d0 / (1.d0 + B02)
