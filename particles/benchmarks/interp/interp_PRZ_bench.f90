...
real*8, dimension(n_vector,1) :: P, P_s, P_t, P_phi
...
do i=1,N_loops
  ...
  call interp_PRZ_vec(node_list, element_list, i_elm, [1], 1, n_vector, s, t, phi, P, P_s, P_t, P_phi, R, R_s, R_t, Z, Z_s, Z_t)
  !do j=1,n_vector
    !call interp_PRZ(node_list, element_list, i_elm, [1], 1, s(j), t(j), phi(j), &
...
write(*,*) "Time per iteration: ", ((t1-t0)/real(N_loops*N_vector)) *1d6, " ms"
