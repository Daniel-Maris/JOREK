...
real*8  :: psi_s, psi_t, psi_R, psi_Z, s_elm, t_elm, mass_ion, ran3(3), ran4(4), P(4), P_s(4), P_t(4)
...
particle_list%n_particles = 1000000
...
!$omp           omp_tid, n_particles_thread)
...
!  if ( sqrt((R_in-3.3)**2 + Z_in**2) .gt. 0.4 ) cycle
  ...
  if (ifail .eq. 0) then
    ...
    i_var = (/ 1, 5, 6, 7 /)
    call interp_PRZ(node_list,element_list,i_elm,i_var,size(i_var),s_elm,t_elm,phi_in,P,P_s,P_t,R,R_s,R_t,Z,Z_s,Z_t)
