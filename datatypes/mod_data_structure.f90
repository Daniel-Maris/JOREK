  ...
  subroutine new_thread_buffers()
    ...
    if (.not. associated(thread_struct)) then
       ...
       do i = 1, nbthreads
          ...
          call tr_allocatep(thread_struct(i)%ELM_p, 1,n_plane,1,n_vertex_max*n_var*(n_order+1),1,n_vertex_max*n_var*(n_order+1)*n_tor,"ELM_p",CAT_MATELEM,.false.)
          call tr_allocatep(thread_struct(i)%ELM_n, 1,n_plane,1,n_vertex_max*n_var*(n_order+1),1,n_vertex_max*n_var*(n_order+1)*n_tor,"ELM_n",CAT_MATELEM,.false.)
          call tr_allocatep(thread_struct(i)%ELM_k, 1,n_plane,1,n_vertex_max*n_var*(n_order+1),1,n_vertex_max*n_var*(n_order+1)*n_tor,"ELM_k",CAT_MATELEM,.false.)
          call tr_allocatep(thread_struct(i)%ELM_kn,1,n_plane,1,n_vertex_max*n_var*(n_order+1),1,n_vertex_max*n_var*(n_order+1)*n_tor,"ELM_kn",CAT_MATELEM,.false.)
