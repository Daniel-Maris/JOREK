...
module mod_four_filter
  ...
  subroutine perform_four_trafo(re, cp, trafo_type, ierr)
    ...
    real*8,     allocatable, intent(inout) :: re(:,:,:,:)
    complex*16, allocatable, intent(inout) :: cp(:,:,:,:)
    integer,                 intent(in)    :: trafo_type !< see constants TOROIDAL_TRAFO etc above.
    integer,                 intent(inout) :: ierr
    ...
    integer*8 :: plan, n(4)
    logical   :: forward
    real*8, allocatable :: re2(:)
    complex*16, allocatable :: cp2(:)
    ...
    if ( allocated(re) .and. (.not. allocated(cp)) ) then
      ...
      ierr = 100
      write(*,*) 'ERROR in '//trim(THIS_ROUTINE_NAME)//': Forward transform requires re to be ' // &
        'allocated; backward transform requires cp to be allocated; never both.'
      return
    end if
    if ( (trafo_type /= POLTOR_TRAFO) .and. (trafo_type /= TOROIDAL_TRAFO) .and.                   &
      (trafo_type /= POLOIDAL_TRAFO) ) then
      ierr = 105
      write(*,*) 'ERROR in '//trim(THIS_ROUTINE_NAME)//': Illegal trafo_type.'
      return
    end if
    
#ifdef USE_FFTW
    
    ! --- Forward transformation (real to complex)
    if ( forward ) then
      
      n(:) = (/ size(re,1), size(re,2), size(re,3), size(re,4) /)
      if ( trafo_type==POLOIDAL_TRAFO ) then
        allocate( cp(n(1),n(2)/2+1,n(3),n(4)), re2(n(2)), cp2(n(2)/2+1) )
      else
      ...
      ! --- Tor forward
        ...
        call dfftw_plan_dft_r2c_1d(plan, n(1), re, cp, FFTW_ESTIMATE)
        do i = 1, n(4)
          do j = 1, n(3)
            do k = 1, n(2)
              call dfftw_execute_dft_r2c(plan, re(:,k,j,i), cp(:,k,j,i))
            end do
          end do
        end do
        cp = cp / real(n(1))
      ...
      ! --- Pol forward
        ...
        call dfftw_plan_dft_r2c_1d(plan, n(2), re2, cp2, FFTW_ESTIMATE)
        
        do i = 1, n(4)
          do j = 1, n(3)
            do k = 1, n(1)
              re2(:) = re(k,:,j,i)
              call dfftw_execute_dft_r2c(plan, re2(:), cp2(:))
              cp(k,:,j,i) = cp2(:)
            end do
          end do
        end do
    ...
    ! --- Backward transformation (complex to real)
    else
    
      n(:) = (/ size(cp,1), size(cp,2), size(cp,3), size(cp,4) /)
      if ( trafo_type==POLOIDAL_TRAFO ) then
        allocate( re(n(1),2*(n(2)-1),n(3),n(4)), re2(2*(n(2)-1)), cp2(n(2)) )
      else
      ...
      if ( trafo_type == POLTOR_TRAFO ) then
        ...
        call dfftw_plan_dft_c2r_2d(plan, 2*(n(1)-1), n(2), cp, re, FFTW_ESTIMATE)
        do i = 1, n(4)
          do j = 1, n(3)
            call dfftw_execute_dft_c2r(plan, cp(:,:,j,i), re(:,:,j,i))
          end do
        end do
      ...
      ! --- Tor backward
        ...
        call dfftw_plan_dft_c2r_1d(plan, 2*(n(1)-1), cp, re, FFTW_ESTIMATE)
        do i = 1, n(4)
          do j = 1, n(3)
            do k = 1, n(2)
              call dfftw_execute_dft_c2r(plan, cp(:,k,j,i), re(:,k,j,i))
            end do
          end do
        end do
      ...
      ! --- Pol backward
        ...
        call dfftw_plan_dft_c2r_1d(plan, 2*(n(2)-1), cp2, re2, FFTW_ESTIMATE)
        
        do i = 1, n(4)
          do j = 1, n(3)
            do k = 1, n(1)
              cp2(:) = cp(k,:,j,i)
              call dfftw_execute_dft_c2r(plan, cp2(:), re2(:))
              re(k,:,j,i) = re2(:)
            end do
          end do
        end do
...
#error "You need fftw library and to define -DUSE_FFTW"
  ...
  recursive subroutine filter_add(filter, ierr, m, n, m_start, m_end, n_start, n_end, mabs,        &
    mabs_start, mabs_end)
    ...
    type (t_four_filter), intent(inout) :: filter
    integer,              intent(inout) :: ierr
    integer, optional,    intent(in)    :: m, n, m_start, m_end, n_start, n_end, mabs, mabs_start, &
      mabs_end
  ...
  function simple_filter(m, n, m_start, m_end, n_start, n_end, mabs, mabs_start, mabs_end)         &
    result(filter)
    ...
    call filter_add(filter, ierr, m, n, m_start, m_end, n_start, n_end, mabs, mabs_start, mabs_end)
  ...
  !> Apply a Fourier filter: Transform, modify coefficients, transform back.
